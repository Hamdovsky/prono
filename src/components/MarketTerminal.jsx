...
const MarketBar = ({ prob }) => {
  ...
  return (
    <div style={{ width: '100%', height: 3, background: 'rgba(148,163,184,0.15)', borderRadius: 2, marginTop: 2 }}>
...
const EvBadge = ({ ev }) => {
  ...
  const color = cls === 'green' ? '#00ffaa' : cls === 'yellow' ? '#fbbf24' : cls === 'red' ? '#ef4444' : '#64748b'
  return <span style={{ fontSize: 9, fontWeight: 900, color, fontFamily: "'JetBrains Mono', monospace" }}>{val}</span>
}
...
const MarketCell = ({ label, prob, odds, ev, invert }) => {
  ...
  const color = p >= 65 ? '#00ffaa' : p >= 45 ? '#fbbf24' : '#ef4444'
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 1, padding: '2px 4px', borderRadius: 3, background: 'rgba(0,0,0,0.15)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 9, fontWeight: 700, color: '#94a3b8' }}>{label}</span>
        <span style={{ fontSize: 9, fontWeight: 900, color, fontFamily: "'JetBrains Mono', monospace" }}>{p}%</span>
      </div>
      <MarketBar prob={p} />
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 8, color: '#64748b', fontFamily: "'JetBrains Mono', monospace" }}>@{fmtOdds(odds)}</span>
        <EvBadge ev={ev} />
      </div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', gap: 3, flexDirection: 'column' }}>
      {Object.entries(markets || {}).map(([key, m]) => (
        <MarketCell key={key} label={prefix ? `${prefix} ${key}` : key} prob={m.prob} odds={m.odds} ev={m.ev} />
      ))}
    </div>
  )
}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#f8fafc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
          {isLive && <span style={{ fontSize: 8, color: '#ef4444', fontWeight: 900 }}>🔴 LIVE</span>}
          {isInsufficient && <span style={{ fontSize: 8, color: '#f59e0b', fontWeight: 900 }}>⚠️ ATTENTE</span>}
          <span style={{ fontSize: 10, fontWeight: 900, color: m.confidence >= 70 ? '#00ffaa' : m.confidence >= 55 ? '#fbbf24' : '#64748b' }}>
            {m.confidence || 0}%
          </span>
        </div>
      </div>
      ...
      {isInsufficient ? (
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 120px', minWidth: 100 }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: 2 }}>1X2</div>
            <div style={{ display: 'flex', gap: 2 }}>
              {['1','X','2'].map(k => {
                ...
                return (
                  <div key={k} style={{
                    flex: 1, textAlign: 'center', padding: '2px 0', borderRadius: 3,
                    background: k === (quant.main_pick || '').replace(/[^1X2]/g, '') ? 'rgba(0,255,170,0.12)' : 'rgba(0,0,0,0.15)',
                    border: k === (quant.main_pick || '').replace(/[^1X2]/g, '') ? '1px solid rgba(0,255,170,0.3)' : '1px solid transparent'
                  }}>
                    <div style={{ fontSize: 10, fontWeight: 900, color: pct >= 65 ? '#00ffaa' : pct >= 45 ? '#fbbf24' : '#ef4444' }}>{pct}%</div>
          ...
          <div style={{ flex: '0 0 auto', minWidth: 70 }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: 2 }}>BTTS</div>
            <div style={{ display: 'flex', gap: 2 }}>
              {['YES','NO'].map(k => {
                const mkt = mkts.btts?.[k]
                const pct = fmtPct(mkt?.prob)
                return (
                  <div key={k} style={{ flex: 1, textAlign: 'center', padding: '2px 4px', borderRadius: 3, background: 'rgba(0,0,0,0.15)', minWidth: 30 }}>
                    <div style={{ fontSize: 9, fontWeight: 900, color: pct >= 60 ? '#00ffaa' : '#ef4444' }}>{k === 'YES' ? 'O' : 'N'}</div>
          ...
          {/* O/U 2.5 */}
          <div style={{ flex: '0 0 auto', minWidth: 70 }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: 2 }}>O/U 2.5</div>
            <div style={{ display: 'flex', gap: 2 }}>
              {[['O2.5','O'],['U2.5','U']].map(([k, l]) => {
                const mkt = mkts.over_under?.[k]
                const pct = fmtPct(mkt?.prob)
                return (
                  <div key={k} style={{ flex: 1, textAlign: 'center', padding: '2px 4px', borderRadius: 3, background: 'rgba(0,0,0,0.15)', minWidth: 30 }}>
                    <div style={{ fontSize: 9, fontWeight: 900, color: pct >= 60 ? '#00ffaa' : '#ef4444' }}>{l}</div>
                    <div style={{ fontSize: 8, color: '#64748b' }}>{pct}%</div>
                    <EvBadge ev={mkt?.ev} />
                  </div>
                )
          ...
          <div style={{ flex: '0 0 auto', minWidth: 80 }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: 2 }}>DC</div>
            <div style={{ display: 'flex', gap: 2 }}>
              {['1X','12','X2'].map(k => {
                const mkt = mkts.double_chance?.[k]
                const pct = fmtPct(mkt?.prob)
                return (
                  <div key={k} style={{ flex: 1, textAlign: 'center', padding: '2px 2px', borderRadius: 3, background: 'rgba(0,0,0,0.15)', minWidth: 24 }}>
                    <div style={{ fontSize: 7, fontWeight: 900, color: '#94a3b8' }}>{k}</div>
                    <div style={{ fontSize: 8, color: pct >= 70 ? '#00ffaa' : '#64748b', fontWeight: 700 }}>{pct}%</div>
                  </div>
                )
              })}
            </div>
          ...
          <div style={{ flex: '0 0 auto', minWidth: 60 }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: 2 }}>HT</div>
            <div style={{ display: 'flex', gap: 2 }}>
              {['O0.5'].map(k => {
                const mkt = mkts.first_half?.[k]
                const pct = fmtPct(mkt?.prob)
                return (
                  <div key={k} style={{ flex: 1, textAlign: 'center', padding: '2px 4px', borderRadius: 3, background: 'rgba(0,0,0,0.15)' }}>
                    <div style={{ fontSize: 9, fontWeight: 900, color: pct >= 65 ? '#00ffaa' : '#fbbf24' }}>{pct}%</div>
                    <EvBadge ev={mkt?.ev} />
                  </div>
                )
              })}
          ...
          {/* CS */}
          <div style={{ flex: '0 0 auto', minWidth: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>CS</div>
            <div style={{ fontSize: 13, fontWeight: 900, color: '#00ffaa', fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          {/* EV */}
          <div style={{ flex: '0 0 auto', minWidth: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>EV</div>
...
const MatchTableRow = ({ m }) => {
  ...
  const EvNum = ({ ev }) => {
    const { val, cls } = fmtEv(ev)
    const color = cls === 'green' ? '#00ffaa' : cls === 'yellow' ? '#fbbf24' : cls === 'red' ? '#ef4444' : '#64748b'
    return <span style={{ fontSize: 10, fontWeight: 900, color, fontFamily: "'JetBrains Mono', monospace" }}>{val}</span>
  }
  ...
  const MktMini = ({ label, prob, odds, ev, isMain }) => {
    ...
    return (
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 900, color: p >= 60 ? '#00ffaa' : p >= 45 ? '#fbbf24' : '#64748b' }}>{p}%</div>
  ...
  return (
    <div style={{
      ...
      fontSize: 11, minWidth: 'fit-content'
    }}>
      {/* Match */}
      ...
      <div style={{ display: 'flex', gap: 2, flex: '0 0 150px' }}>
        {['1','X','2'].map(k => {
          const mkt = mkts.match_result?.[k]
          return <MktMini key={k} label={k} prob={mkt?.prob} odds={mkt?.odds} ev={mkt?.ev} isMain={k === mainPick} />
        })}
      </div>
        ...
        {['1X','12','X2'].map(k => {
          ...
          return (
            <div key={k} style={{ textAlign: 'center', padding: '3px 2px', minWidth: 36 }}>
              ...
              <div style={{ fontSize: 10, fontWeight: 900, color: p >= 75 ? '#00ffaa' : '#64748b' }}>{p}%</div>
            </div>
          )
        })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flex: '0 0 100px' }}>
        <div style={{ fontSize: 12, fontWeight: 900, color: '#00ffaa', fontFamily: "'JetBrains Mono', monospace", minWidth: 40 }}>
          {quant.expected_score || m.expected_score || '-'}
        </div>
...
const MarketTerminal = ({ matches }) => {
  ...
  const stats = useMemo(() => {
    ...
    const avgEv = withEv.length > 0
      ? withEv.reduce((s, m) => s + parseFloat(m.quant.ev_score || 0), 0) / withEv.length
      : 0
    const bestEv = withEv.sort((a, b) => parseFloat(b.quant?.ev_score || 0) - parseFloat(a.quant?.ev_score || 0))[0]
  ...
  return (
    <div style={{ padding: '8px 12px' }}>
      ...
      <div style={{
        ...
        <span style={{ color: '#64748b' }}>📊 <b style={{ color: '#f8fafc' }}>{stats.total}</b> matchs</span>
        {stats.live > 0 && <span style={{ color: '#ef4444' }}>🔴 <b>{stats.live}</b> live</span>}
        <span style={{ color: '#64748b' }}>EV moyen: <b style={{ color: stats.avgEv > 0 ? '#00ffaa' : '#ef4444' }}>
          {stats.avgEv > 0 ? '+' : ''}{stats.avgEv.toFixed(3)}
        </b></span>
        {stats.bestEv && (
          <span style={{ color: '#64748b' }}>
            🏆 Meilleur EV: <b style={{ color: '#fbbf24' }}>{stats.bestEv.homeTeam} vs {stats.bestEv.awayTeam}</b>
      ...
      {liveMatches.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 10, fontWeight: 900, color: '#ef4444', textTransform: 'uppercase', marginBottom: 6, letterSpacing: 1 }}>
      ...
      {scheduledMatches.length > 0 && (
        <>
          <div style={{ fontSize: 10, fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase', marginBottom: 6, letterSpacing: 1 }}>
            📅 MATCHS PROGRAMMÉS ({scheduledMatches.length})
          </div>
          {viewMode === 'table' ? (
            /* Table view */
            <div style={{ overflowX: 'auto', borderRadius: 6, border: '1px solid rgba(148,163,184,0.1)' }}>
              {/* Header */}
              <div style={{
                display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px',
                background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(148,163,184,0.15)',
                fontSize: 8, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.5
              }}>
              ...
              {scheduledMatches.map(m => <MatchTableRow key={m.id} m={m} />)}
              {scheduledMatches.length === 0 && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: 6 }}>
              {scheduledMatches.map(m => <MatchCard key={m.id} m={m} />)}
            </div>
          )}
        </>
      )}
