...
class AutopsyService {
  ...
  async diagnoseMatch(matchId) {
    try {
      ...
      const prono =
        fullData.enriched && fullData.enriched.main_predictions
          ? fullData.enriched.main_predictions[0]
          : null
    ...
    try {
      ...
      for (const m of matches) {
        try {
          ...
          const isCorrect = this._checkIfPredictionCorrect(pronos, m, actualWinner, totalGoals)

          if (!isCorrect) {
            // Prediction Failed: Run Autopsy
            const diagnosisData = this._diagnoseFailure(
              m,
              fullData,
              scoreH,
              scoreA,
            ...
            failedMatches.push({
              ...
              confidence:
                pronos[0]?.probability ||
                pronos[0]?.confidence ||
                (fullData.xgboost_confidence
                  ? Math.round(fullData.xgboost_confidence * 100)
                  : 'N/A'),
      ...
      console.log(
        `🔬 [AUTOPSY] Found ${failedMatches.length} failed predictions out of ${matches.length} matches.`
      )
  ...
  _checkIfPredictionCorrect(pronos, match, actualWinner, totalGoals) {
    ...
    pronos.forEach((p) => {
      ...
      const is1X2 =
        ...
        label.includes('1X2') ||
        label.includes('Choix') ||
        label.includes('Analyst') ||
        label.includes('Favori')
      const isGoals =
        label.includes('Goals') ||
      ...
      if (is1X2) {
        ...
        const isHomePred =
          valLower.includes(homeLower) ||
          val.includes('🏠') ||
          val.includes('Home') ||
          val.includes('1')
        const isAwayPred =
          valLower.includes(awayLower) ||
          val.includes('✈️') ||
          val.includes('Away') ||
          val.includes('2')
        ...
        if (label.includes('Double') || label.includes('فرصة')) {
          if (val.includes('1X') && (actualWinner === 'H' || actualWinner === 'D'))
            anyCorrect = true
          if (val.includes('X2') && (actualWinner === 'A' || actualWinner === 'D'))
            anyCorrect = true
          if (val.includes('12') && (actualWinner === 'H' || actualWinner === 'A'))
            anyCorrect = true
        }
      }
      ...
      if (isGoals) {
        ...
        const isOver =
          val.toLowerCase().includes('over') || val.includes('أكثر') || val.includes('+')
  ...
  _normalizeStats(rawStats) {
    ...
    const processItem = (item) => {
      ...
      if (cat.includes('red card')) {
        normalized.redCards.home = h
        normalized.redCards.away = a
      }
      if (cat.includes('expected goal')) {
        normalized.expectedGoals.home = h
        normalized.expectedGoals.away = a
      }
      if (cat.includes('shots on target')) {
        normalized.shotsOnTarget.home = h
        normalized.shotsOnTarget.away = a
      }
      if (cat.includes('big chance')) {
        normalized.bigChances.home = h
        normalized.bigChances.away = a
      }
      if (cat.includes('possession')) {
        normalized.possession.home = h
        normalized.possession.away = a
      }
      if (cat.includes('total shots') || cat === 'shots') {
        normalized.totalShots.home = h
        normalized.totalShots.away = a
      }
      if (cat.includes('corner')) {
        normalized.corners.home = h
        normalized.corners.away = a
      }
    }
  ...
  _diagnoseFailure(matchRow, fullData, scoreH, scoreA, actualWinner, mainPrediction) {
    ...
    let earlyGoalConcededH = false,
      earlyGoalConcededA = false
    let penaltyConcededH = false,
      penaltyConcededA = false
    let redCardBefore70H = false,
      redCardBefore70A = false
    ...
    incidents.forEach((inc) => {
      ...
      if (type === 'goal' || inc.incidentClass === 'regular' || inc.incidentClass === 'penalty') {
        const isPenalty =
          inc.incidentClass === 'penalty' || text.includes('penalty') || text.includes('جزاء')
        if (inc.isHome === true || inc.isHome === 'true') {
    ...
    if (!redCardBefore70H && stats.redCards.home > 0 && incidents.length === 0)
      redCardBefore70H = true
    if (!redCardBefore70A && stats.redCards.away > 0 && incidents.length === 0)
      redCardBefore70A = true

    if (redCardBefore70H && scoreH < scoreA) {
      diagnosis = {
        ar: `خسر الفريق المضيف (${matchRow.homeTeam}) بسبب طرد مبكر (قبل الدقيقة 70) دمر الهيكل الدفاعي.`,
        icon: '🛑',
        type: 'PERSONNEL_DEFICIT_DISRUPTION',
      }
    } else if (redCardBefore70A && scoreA < scoreH) {
      diagnosis = {
        ar: `خسر الفريق الضيف (${matchRow.awayTeam}) بسبب طرد مبكر (قبل الدقيقة 70) دمر الخطة التكتيكية.`,
        icon: '🛑',
        type: 'PERSONNEL_DEFICIT_DISRUPTION',
      }
    }
    ...
    if (earlyGoalConcededH && scoreH < scoreA) {
      diagnosis = {
        ar: `هدف مبكر بعثر الأوراق التكتيكية للفريق المضيف في أول 15 دقيقة، ودفع الخصم للركون للدفاع.`,
        icon: '⏱️',
        type: 'EARLY_TACTICAL_DISRUPTION',
      }
    } else if (earlyGoalConcededA && scoreA < scoreH) {
      diagnosis = {
        ar: `هدف مبكر مباغت في أول 15 دقيقة دمر خطة لعب الفريق الضيف ولخبط الحسابات.`,
        icon: '⏱️',
        type: 'EARLY_TACTICAL_DISRUPTION',
      }
    }
    ...
    if (penaltyConcededH && scoreH < scoreA && scoreA - scoreH === 1) {
      diagnosis = {
        ar: `ركلة جزاء فارقة منحت الأفضلية للخصم وحسمت المباراة بصعوبة للفريق الضيف.`,
        icon: '🎯',
        type: 'SET_PIECE_DECIDER',
      }
    } else if (penaltyConcededA && scoreA < scoreH && scoreH - scoreA === 1) {
      diagnosis = {
        ar: `ركلة جزاء فارقة منحت الأفضلية لصاحب الأرض وحسمت المباراة بصعوبة.`,
        icon: '🎯',
        type: 'SET_PIECE_DECIDER',
      }
    }
    ...
    if (scoreH < scoreA && scoreA >= 3) {
      diagnosis = {
        ar: `انهيار دفاعي كارثي غير متوقع: تلقى الفريق المضيف ${scoreA} أهداف في شباكه.`,
        icon: '📉',
        type: 'SYSTEMIC_DEFENSIVE_FAILURE',
      }
    } else if (scoreA < scoreH && scoreH >= 3) {
      diagnosis = {
        ar: `انهيار دفاعي كارثي غير متوقع: تلقى الفريق الضيف ${scoreH} أهداف في شباكه.`,
        icon: '📉',
        type: 'SYSTEMIC_DEFENSIVE_FAILURE',
      }
    }
    ...
    if (
      actualWinner === 'D' &&
      (totalGoals === 0 || totalGoals === 2) &&
      stats.shotsOnTarget.home < 3 &&
      stats.shotsOnTarget.away < 3
    ) {
      diagnosis = {
        ar: `غياب تام للشراسة الهجومية وأداء باهت جداً (تسديدات نادرة من الطرفين وعقم هجومي واضح).`,
        icon: '🥱',
        type: 'LOW_INTENSITY_OFFENSE',
      }
    }
    ...
    if (xgH > xgA + 0.85 && scoreH <= scoreA) {
      diagnosis = {
        ar: `رعونة هجومية: ${matchRow.homeTeam} تفوق في الأهداف المتوقعة (xG: ${xgH.toFixed(2)}) ولكنه تفنن في الإهدار.`,
        icon: '🥅',
        type: 'XG_WASTE',
      }
    } else if (xgA > xgH + 0.85 && scoreA <= scoreH) {
      diagnosis = {
        ar: `رعونة هجومية: ${matchRow.awayTeam} تفوق في الأهداف المتوقعة (xG: ${xgA.toFixed(2)}) ولم يحولها إلى أهداف.`,
        icon: '🥅',
        type: 'XG_WASTE',
      }
    }
    ...
    if (stats.shotsOnTarget.home >= 4 && scoreH <= 1 && actualWinner !== 'H') {
      diagnosis = {
        ar: `تألق إعجازي لحارس مرمى الخصم الذي تصدى لتسديدات ${matchRow.homeTeam} الكثيرة وحرمه من الفوز.`,
        icon: '🧤',
        type: 'GK_WALL',
      }
    } else if (stats.shotsOnTarget.away >= 4 && scoreA <= 1 && actualWinner !== 'A') {
      diagnosis = {
        ar: `جدار دفاعي وتألق مبهر للحارس الخصم حرم ${matchRow.awayTeam} من التسجيل رغم كثرة التسديدات.`,
        icon: '🧤',
        type: 'GK_WALL',
      }
    }
    ...
    if (stats.bigChances.home >= 2 && scoreH < scoreA) {
      diagnosis = {
        ar: `إهدار فرص محققة: فريق ${matchRow.homeTeam} أضاع ${stats.bigChances.home} فرص مؤكدة للتسجيل كانت كفيلة بحسم اللقاء.`,
        icon: '🚫',
        type: 'BIG_CHANCE_WASTE',
      }
    } else if (stats.bigChances.away >= 2 && scoreA < scoreH) {
      diagnosis = {
        ar: `إهدار فرص محققة: فريق ${matchRow.awayTeam} أضاع ${stats.bigChances.away} فرص مؤكدة للتسجيل أمام المرمى.`,
        icon: '🚫',
        type: 'BIG_CHANCE_WASTE',
      }
    }
    ...
    if (stats.totalShots.home > stats.totalShots.away + 6 && scoreH <= scoreA) {
      diagnosis = {
        ar: `سيطرة ميدانية عقيمة: ${matchRow.homeTeam} سدد أكثر من الخصم بكثير، لكنه فشل في الحسم.`,
        icon: '🏹',
        type: 'SHOT_DOMINANCE',
      }
    } else if (stats.totalShots.away > stats.totalShots.home + 6 && scoreA <= scoreH) {
      diagnosis = {
        ar: `ضغط هجومي غير مستغل: ${matchRow.awayTeam} سدد أكثر من المنافس ومع ذلك خسر اللقاء.`,
        icon: '🏹',
        type: 'SHOT_DOMINANCE',
      }
    }
    ...
    if (stats.corners.home > stats.corners.away + 7 && scoreH <= scoreA) {
      diagnosis = {
        ar: `حصار هجومي فاشل: تفوق ${matchRow.homeTeam} الكبير في الركنيات يوضح سيطرته العقيمة.`,
        icon: '🚩',
        type: 'CORNER_DOMINANCE',
      }
    } else if (stats.corners.away > stats.corners.home + 7 && scoreA <= scoreH) {
      diagnosis = {
        ar: `سيطرة كلية على الكرات الثابتة: ${matchRow.awayTeam} لم يستغل الركنيات لهز الشباك الخصم.`,
        icon: '🚩',
        type: 'CORNER_DOMINANCE',
      }
    }
    ...
    const lateGoal = incidents.find(
      (inc) => (inc.type === 'goal' || inc.incidentClass === 'regular') && parseInt(inc.time) > 85
    )
    if (lateGoal) {
      diagnosis = {
        ar: `تحطم التوقع بسبب "هدف قاتل" في الدقيقة ${lateGoal.time} قلب موازين النتيجة بالكامل.`,
        icon: '⏱️',
        type: 'LATE_GOAL',
      }
    }
    ...
    if (stats.possession.home > 61 && scoreH < scoreA) {
      diagnosis = {
        ar: `استحواذ سلبي: ${matchRow.homeTeam} سيطر بنسبة ${stats.possession.home}% لكنه اُصيب بمرتدة قاتلة.`,
        icon: '🔄',
        type: 'POSSESSION_FAIL',
      }
    }
    ...
    return {
      ...
      incidents: incidents.filter((i) =>
        ['goal', 'card', 'red', 'penalty'].some((t) =>
          (i.type || i.incidentClass || '').toLowerCase().includes(t)
        )
      ),
    }
  }
