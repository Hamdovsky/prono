...
class DeepFormService {
    constructor() {
        ...
        this.ELITE_CLUBS = new Set([
            ...
            'bayern munich', 'dortmund', 'leverkusen', 'juventus', 'inter milan',
            'ac milan', 'napoli', 'paris saint-germain', 'marseille', 'lyon',
            'psv eindhoven', 'ajax', 'porto', 'benfica', 'sporting cp',
            'galatasaray', 'fenerbahce', 'al ahly', 'al hilal', 'al ittihad'
        ]);

        // League average goals per match (used for normalization)
        this.LEAGUE_AVG_GOALS = {
            'premier league': 2.7, 'la liga': 2.5, 'bundesliga': 3.0,
            'serie a': 2.6, 'ligue 1': 2.4, 'eredivisie': 3.1,
            'super lig': 2.8, 'eredivisie': 3.1, 'primeira liga': 2.5,
            'champions league': 2.8, 'europa league': 2.7,
            'default': 2.6
        };
    }
    ...
    analyzeForm(events, teamId) {
        ...
        recentEvents.forEach((ev, index) => {
            ...
            const DRAW_MULT = isHome ? 0.90 : 1.10;
        ...
        let strength_of_schedule = opponentCount > 0
            ? Math.min(100, Math.max(0, opponentStrengthSum / opponentCount))
            : 50;
