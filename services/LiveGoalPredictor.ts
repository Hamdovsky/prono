...
class LiveGoalPredictor {
  ...
  calculateOddsMovementFactor(match) {
    ...
    if (odds.over25 && odds.over25 < 1.4) factor += 30
    else if (odds.over25 && odds.over25 < 1.6) factor += 22
    else if (odds.over25 && odds.over25 < 1.8) factor += 15
    else if (odds.over25 && odds.over25 < 2.0) factor += 8
  ...
  async calculatePatternMatchFactor(match) {
    ...
    const matches = patterns.filter((p) => {
      ...
      return (
        p.scoreState === currentState.score &&
  ...
  async findMatchingPatterns(match) {
    ...
    return patterns
      .filter(
        (p) =>
          p.scoreState === currentState && Math.abs(p.minute - (parseInt(match.minute) || 0)) < 10
      )
      .slice(0, 5)
      .map((p) => ({
        minute: p.minute,
  ...
  getActiveMatches() {
    this._cleanup()
    return Array.from(this.activeMatches.values()).sort(
      (a, b) => b.probabilities.next10min - a.probabilities.next10min
    )
  }
