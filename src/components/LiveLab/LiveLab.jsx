...
const AnimatedStat = ({ val, prevVal, className, isScore, style }) => {
  ...
  return (
    <span className={`${className} ${flashClass}`} style={style} key={val}>
      {val}
    </span>
  )
}
  ...
  return (
    <div className="ll-card">
      <div className="ll-card-top">
        ...
        <span
          className={`ll-cat ${m.category === 'LIVE' ? 'pulse' : ''}`}
      ...
      <div className="ll-ai-strip">
        <div className="ll-ai-item">
          ...
          <div>
            <AnimatedStat val={m.homeWinP} prevVal={prevM?.homeWinP} className="ll-ai-val" />%
          </div>
        </div>
        <div className="ll-ai-item">
          <span className="ll-ai-lbl">🤝 تعادل</span>
          <div>
            <AnimatedStat val={m.drawP} prevVal={prevM?.drawP} className="ll-ai-val" />%
          </div>
        </div>
        <div className="ll-ai-item">
          <span className="ll-ai-lbl">✈️ فوز ض</span>
          <div>
            <AnimatedStat val={m.awayWinP} prevVal={prevM?.awayWinP} className="ll-ai-val" />%
          </div>
        </div>
        <div className="ll-ai-item highlight">
          <span className="ll-ai-lbl">🎯 ثقة</span>
          <div>
            <AnimatedStat val={m.confidence} prevVal={prevM?.confidence} className="ll-ai-val" />%
          </div>
        </div>
      </div>

      {m.dnaInsight && <div className="ll-dna-insight">🧬 {m.dnaInsight}</div>}
      ...
      {m.statsbombInsight && (m.statsbombInsight.home || m.statsbombInsight.away) && (
        ...
        >
          <strong style={{ display: 'block', marginBottom: '6px' }}>
            📊 التحليل التاريخي العميق (StatsBomb):
          </strong>
          {m.statsbombInsight.home && (
            <div style={{ marginBottom: '4px' }}>
              <strong>{m.homeTeam}:</strong> {m.statsbombInsight.home}
            </div>
          )}
          {m.statsbombInsight.away && (
            <div>
              <strong>{m.awayTeam}:</strong> {m.statsbombInsight.away}
            </div>
          )}
        </div>
      )}
      ...
      {hasLiveStats && (
        <div className="ll-momentum-wave">
          ...
          <div className="ll-wave-container">
            <div
              className="ll-wave-bar home"
              style={{ height: `${m.momentum.homePercent * 0.6}%` }}
            ></div>
            <div
              className="ll-wave-bar away"
              style={{ height: `${m.momentum.awayPercent * 0.6}%` }}
            ></div>
          </div>
        </div>
      )}
      ...
      {hasLiveStats && (
        <div className="ll-stats">
          <div className="ll-stat-row">
            <AnimatedStat
              val={m.stats.dangerousAttacks.home}
              prevVal={prevM?.stats.dangerousAttacks.home}
              className="ll-sv home"
            />
            <span className="ll-sl">⚔️ هجمات خطيرة</span>
            <AnimatedStat
              val={m.stats.dangerousAttacks.away}
              prevVal={prevM?.stats.dangerousAttacks.away}
              className="ll-sv away"
            />
          </div>
          <div className="ll-stat-row">
            <AnimatedStat
              val={m.stats.shotsOnTarget.home}
              prevVal={prevM?.stats.shotsOnTarget.home}
              className="ll-sv home"
            />
            <span className="ll-sl">🎯 تسديدات</span>
            <AnimatedStat
              val={m.stats.shotsOnTarget.away}
              prevVal={prevM?.stats.shotsOnTarget.away}
              className="ll-sv away"
            />
          </div>
          <div className="ll-stat-row xg-row">
            <AnimatedStat
              val={m.stats.xg.home}
              prevVal={prevM?.stats.xg.home}
              className="ll-sv home"
            />
            <span className="ll-sl">📈 الأهداف المتوقعة (xG)</span>
            <AnimatedStat
              val={m.stats.xg.away}
              prevVal={prevM?.stats.xg.away}
              className="ll-sv away"
            />
          </div>
          ...
          <div className="ll-6layer-header">
            <div className="ll-ti-grid">
              ...
              <div className="ll-ti-box">
                ...
                <span className="ll-ti-value deviation">
                  {m.xgDeviation.home > 0 ? `+${m.xgDeviation.home}` : m.xgDeviation.home}H
                </span>
                <div
                  className={`luck-indicator ${m.xgDeviation.verdict.includes('Over') ? 'luck' : m.xgDeviation.verdict.includes('Under') ? 'unluck' : 'normal'}`}
                >
                  {m.xgDeviation.verdict.includes('Over')
                    ? '⚠️ محظوظ'
                    : m.xgDeviation.verdict.includes('Under')
                      ? '❌ منحوس'
                      : '✅ واقعي'}
                </div>
              </div>
          ...
          {m.nextGoalEstimate && (
            <div className="ll-goal-estimate">⚠️ هدف متوقع خلال {m.nextGoalEstimate} دقائق</div>
          )}
        </div>
      )}
      ...
      {m.pronostics && (m.pronostics.bets || Array.isArray(m.pronostics)) && (
        <div className="ll-pronostics">
          ...
          {m.pronostics.strategicOrientation && (
            <div className="ll-orient-table">
              <div className="ll-orient-row">
                ...
                <span className="ll-orient-val">
                  {m.pronostics.strategicOrientation.confidenceScore}%
                </span>
              </div>
              <div className="ll-orient-row">
                <span className="ll-orient-key">Risk Level</span>
                <span
                  className={`ll-orient-val ${m.pronostics.strategicOrientation.riskLevel.toLowerCase()}`}
                >
              ...
              <div className="ll-orient-row">
                ...
                <span className="ll-orient-val">
                  {m.pronostics.strategicOrientation.suggestedMarket}
                </span>
              </div>
            </div>
          )}
          ...
          {(m.pronostics.bets || m.pronostics).map((p, i) => (
            <div
              key={i}
              className="ll-prono-item"
              style={{ borderRight: `3px solid ${p.color || '#475569'}` }}
            >
              <div className="ll-prono-head">
                ...
                <div className="ll-prono-market-group">
                  ...
                  <div className="ll-prono-tags">
                    {p.status && (
                      <span
                        className={`ll-prono-status ${p.status.includes('نخبة') ? 'elite' : p.status.includes('قيمة') ? 'value' : ''}`}
                        style={{
                          color:
                            p.status.includes('نخبة') || p.status.includes('قيمة')
                              ? undefined
                              : p.color || '#94a3b8',
                        }}
                      >
                    ...
                    {p.matchState && <span className="ll-prono-state">{p.matchState}</span>}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <AnimatedStat
                    val={p.probability}
                    prevVal={
                      prevM?.pronostics?.bets?.[i]?.probability ||
                      prevM?.pronostics?.[i]?.probability
                    }
                    className="ll-prono-prob"
                    style={{ color: p.color || '#f8fafc' }}
                  />
                  %
                </div>
              </div>
...
const LiveLab = () => {
  ...
  const filteredMatches = (Array.isArray(matches) ? matches : []).filter((m) => {
    ...
    if (filter === 'GOALS')
      return m.pronostics?.some((p) => p.type === 'GOALS' || p.market?.includes('هدف'))
    return true
  })
    ...
    <div className="live-lab-container" dir="rtl">
      <div className="ll-header">
        <div className="ll-title-row">
          <h1>
            مختبر التحليل الكوانتي <span>Elite V38</span>
          </h1>
        ...
        <div className="ll-filters">
          ...
          <button className={filter === 'ELITE' ? 'active' : ''} onClick={() => setFilter('ELITE')}>
            🔥 النخبة
          </button>
          <button className={filter === 'VALUE' ? 'active' : ''} onClick={() => setFilter('VALUE')}>
            💰 القيمة
          </button>
          <button className={filter === 'GOALS' ? 'active' : ''} onClick={() => setFilter('GOALS')}>
            ⚽ أهداف
          </button>
        </div>
      </div>
