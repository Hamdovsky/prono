class AdvancedAnalyticsEngine {
  ...
  static calculateDisciplineConflict(homeStats, awayStats, refYellows = 0, refReds = 0) {
    ...
    const homeYellows = homeStats?.yellowCards
      ? homeStats.yellowCards / (homeStats.matchesPlayed || 1)
      : 2
    const awayYellows = awayStats?.yellowCards
      ? awayStats.yellowCards / (awayStats.matchesPlayed || 1)
      : 2
  ...
  static calculateOddsVelocity(openingOdds, currentOdds) {
    ...
    const dropDiff =
      ((currentOdds.home - openingOdds.odds_home_open) / openingOdds.odds_home_open) * 100
