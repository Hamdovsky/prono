...
async function runBackfill() {
  ...
  if (fs.existsSync(PROGRESS_FILE)) {
    try {
      ...
      if (saved.lastDate) currentDate = saved.lastDate
    } catch (e) {}
  }
  ...
  while (currentDate <= END_DATE) {
    ...
    try {
      ...
      console.error(`❌ API fail for ${currentDate}. Retrying...`)
      await new Promise((r) => setTimeout(r, 5000))
      continue
    }

    const targetMatches = allEvents
      ...
      .map((ev) => {
        ...
        m._homeId = ev.homeTeam.id
        m._awayId = ev.awayTeam.id
        m._utid = ev.tournament?.uniqueTournament?.id
        m._sid = ev.season?.id
        return m
      })
      .filter((m) => m !== null)
    ...
    const processMatch = async (match) => {
      try {
        ...
        await learningEngine.learn({
          ...
          homeTeam: match.home,
          awayTeam: match.away,
          prediction: detectPrediction(match),
          ...
          matchStats: {
            ...
            red_cards_home: hS?.redCards || 0,
            red_cards_away: aS?.redCards || 0,
          },
        ...
        if (done % 20 === 0)
          process.stdout.write(`\r  🚀 Progress: ${done}/${targetMatches.length} `)
      }
    }
