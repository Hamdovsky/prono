...
import autopsyService from './autopsyService'
import lineupService from './LineupService'
import SmartOddsAnalyzer from './SmartOddsAnalyzer'
...
class AdaptiveLearningEngine {
  ...
  async learn(input) {
    try {
      ...
      const oddsUsed =
        errorAnalysis.predictedSide === 'H'
          ? oddsData.home
          : errorAnalysis.predictedSide === 'A'
            ? oddsData.away
            : null
      ...
      if (!errorAnalysis.isCorrect && rootCause !== ROOT_CAUSES.VARIANCE) {
        ...
        try {
          const arabic = this._generateArabicInsight(
            homeTeam,
            awayTeam,
            errorAnalysis,
            rootCause,
  ...
  async processBatch(matches) {
    if (!matches || !Array.isArray(matches))
      return { success: false, error: 'Invalid matches array' }
  ...
  async getSimulationContext(league) {
    try {
      ...
      const meta = database.db
        .prepare(`SELECT confidence_adj, accuracy FROM league_weights WHERE league = ?`)
        .get(league) || { confidence_adj: 0, accuracy: 0.5 }
  ...
  async getConfidenceAdjustment(league) {
    try {
      const row = database.db
        .prepare('SELECT confidence_adj FROM league_weights WHERE league = ?')
        .get(league)
  ...
  _getOddsRiskTier(odds) {
    ...
    if (o < 1.7) return { tier: 'STRONG_FAV', mult: 1.5 }
    if (o < 2.2) return { tier: 'MODERATE', mult: 1.0 }
  ...
  _getSisterLeagueWeights(league) {
    for (const [parent, sisters] of Object.entries(DNA_SIMILARITY_MAP)) {
      if (sisters.includes(league)) {
        try {
          const row = database.db
            .prepare(`SELECT weights FROM league_weights WHERE league = ?`)
            .get(parent)
  ...
  _generateArabicInsight(h, a, analysis, cause, ctx) {
    ...
    if (cause === ROOT_CAUSES.ODDS_MISREAD)
      detail += ' حركة السوق كانت تشير لاتجاه مختلف عن الأداء الفني.'
  ...
  _getLeagueXgConvRate(league) {
    try {
      const data = database.db
        .prepare(`SELECT avg_conv_rate FROM league_xg_conversion WHERE league = ?`)
        .get(league)
  ...
  _updateXgConvRate(league, totalGoals, matchStats) {
    try {
      const totalXgStr = matchStats.xg_home
        ...
        : matchStats.xG
          ? parseFloat(matchStats.xG.home) + parseFloat(matchStats.xG.away)
          : 0

      if (totalXgStr <= 0.1) return // Prevent div by 0 bounds

      const rate = Math.min(3.0, totalGoals / totalXgStr) // Clip anomaly
  ...
  async _detectRootCause(
    ...
    if (
      (sotH >= 5 && errorAnalysis.errorType !== ERROR_TYPES.CORRECT) ||
    ...
    if (deepDiagnosis && deepDiagnosis.type !== 'UNKNOWN') {
      ...
      if (rootMap) {
        logger.info(
          `🔬 [V5 AUTOPSY] Overriding Root Cause for ${homeTeam} vs ${awayTeam}: ${rootMap}`
        )
        finalCause = rootMap
    ...
    const lineupDeficit = await lineupService.calculateLineupDeficit(
      matchStats.matchId,
      matchStats.homeTeamId,
      matchStats.awayTeamId
    )
    if (lineupDeficit && lineupDeficit.isFetched) {
      ...
      if (hDef > 0.2 || aDef > 0.2) {
        logger.info(
          `🏃‍♂️ [V6 LINEUP] Significant deficit detected during learning for ${homeTeam} vs ${awayTeam}`
        )
    ...
    if (failingTeam) {
      ...
      if (causeCount >= 2 && finalCause !== ROOT_CAUSES.VARIANCE) {
        logger.warn(
          `[LEARN V2] Pattern detected: ${failingTeam} repeatedly failing on ${finalCause}`
        )
  ...
  async _applyWeightAdjustments(league, adjustments, isCorrect) {
    try {
      ...
      const db = database.db
      const meta = db
        .prepare(`SELECT total_cases, last_updated FROM league_weights WHERE league = ?`)
        .get(league)
      if (meta) {
        totalCases = meta.total_cases || 0
        lastUpdateDays = Math.floor(
          (Date.now() - new Date(meta.last_updated).getTime()) / (1000 * 3600 * 24)
        )
      }
      ...
      db.prepare(
        `
                ...
                ON CONFLICT(league) DO UPDATE SET
                    ...
                    last_updated   = CURRENT_TIMESTAMP
            `
      ).run(league, JSON.stringify(DEFAULT_WEIGHTS), adjustment * 100, adjustment * 100)
    } catch (_) {}
  }
  ...
  _deriveRule(league, errorType, rootCause, adjustments) {
    const rulesMap = {
      [ROOT_CAUSES.RED_CARD]: {
        rule_type: 'FEATURE_BUMP',
        condition: 'red_card_detected',
        action: 'increase_red_card_weight_by_10pct',
        confidence: 0.8,
      },
      [ROOT_CAUSES.XG_ANOMALY]: {
        rule_type: 'XG_THRESHOLD',
        condition: 'xg_diff > dynamic_conv and upset',
        action: 'flag_as_xg_anomaly_and_reduce_confidence',
        confidence: 0.72,
      },
      [ROOT_CAUSES.ODDS_MISREAD]: {
        rule_type: 'ODDS_MOVEMENT',
        condition: 'odds_drift > 0.3',
        action: 'shift_prediction_weight_toward_odds',
        confidence: 0.68,
      },
      [ROOT_CAUSES.LATE_GOAL]: {
        rule_type: 'LATE_GOAL_RISK',
        condition: 'minute > 85 and tied',
        action: 'add_late_goal_risk_signal_to_ou25',
        confidence: 0.65,
      },
      [ROOT_CAUSES.POSSESSION_TRAP]: {
        rule_type: 'POSSESSION_TRAP',
        condition: 'possession_home > 62 and result_away',
        action: 'reduce_possession_weight',
        confidence: 0.7,
      },
      [ROOT_CAUSES.GK_MASTERCLASS]: {
        rule_type: 'GK_SAVE_BOOST',
        condition: 'shots_on_target >= 5 and goals <= 1',
        action: 'reduce_confidence_by_8pct',
        confidence: 0.62,
      },
      [ROOT_CAUSES.DEFENSIVE_COLLAPSE]: {
        rule_type: 'DEF_COLLAPSE',
        condition: 'total_goals >= 5',
        action: 'downgrade_form_weight',
        confidence: 0.75,
      },
      [ROOT_CAUSES.STRUCTURAL_WEAKNESS]: {
        rule_type: 'TEAM_PATTERN_CRISIS',
        condition: 'recurring_failure > 2 matches',
        action: 'override_root_cause_and_decay_elo',
        confidence: 0.88,
      },
    }
    ...
    return (
      rulesMap[rootCause] || {
        rule_type: 'GENERAL_NOISE',
        condition: 'variance_event',
        action: 'no_weight_change_required',
        confidence: 0.4,
      }
    )
  }
  ...
  async _updateLeagueAccuracy(league) {
    try {
      ...
      db.prepare(`UPDATE league_weights SET accuracy = ? WHERE league = ?`).run(
  ...
  async getLeagueReport(league, dateFilter = null) {
    ...
    try {
      ...
      const errorRows = db
        .prepare(
          `SELECT error_type, COUNT(*) as count FROM learning_memory ${whereClause} GROUP BY error_type`
        )
        .all(...params)
      const causeRows = db
        .prepare(
          `SELECT root_cause, COUNT(*) as count FROM learning_memory ${whereClause} GROUP BY root_cause`
        )
        .all(...params)

      const errorDistrib = {}
      errorRows.forEach((r) => (errorDistrib[r.error_type] = r.count))
      const causeDistrib = {}
      causeRows.forEach((r) => (causeDistrib[r.root_cause] = r.count))

      // Recent Cases
      const recentCases = db
        .prepare(`SELECT * FROM learning_memory ${whereClause} ORDER BY match_date DESC LIMIT 50`)
        .all(...params)
      ...
      if (dateFilter) {
        ...
        allRecentCases = db
          .prepare(`SELECT * FROM learning_memory ${globalWhere} ORDER BY match_date DESC LIMIT 10`)
          .all(...globalParams)
      }
      ...
      const meta = db.prepare(metaQuery).get(...(league !== 'ALL' ? [league] : [])) || {
        accuracy: 0.5,
        total_cases: 0,
        confidence_adj: 0,
      }
      ...
      return {
        ...
        totalCases:
          meta.total_cases ||
          (league === 'ALL'
            ? db.prepare('SELECT COUNT(*) as c FROM learning_memory').get()?.c || 0
            : 0),
        accuracy: meta.accuracy || 0.5,
        errorDistrib,
        causeDistrib,
  ...
  async _processAdversarialLearning(matchId, league, input, champAnalysis) {
    try {
      ...
      db.prepare(
        ...
        league,
        matchId,
        champAnalysis.isCorrect ? 'WIN' : 'LOSS',
        challCorrect ? 'WIN' : 'LOSS'
      )
  ...
  async getChallengerWeights(league) {
    try {
      const row = database.db
        .prepare(`SELECT weights FROM league_challenger_weights WHERE league = ?`)
        .get(league)
      if (row) return JSON.parse(row.weights)
    ...
    try {
      ...
      if (stats && stats.total >= 20) {
        ...
        if (challAcc > champAcc + 0.05 || (champAcc < 0.45 && challAcc > champAcc)) {
          logger.info(
            `🏆 [V5 EVOLUTION] Challenger promoted for ${league}! (Accuracy: ${challAcc.toFixed(2)} vs ${champAcc.toFixed(2)})`
          )
...
export = new AdaptiveLearningEngine()
