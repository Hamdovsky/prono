...
import scrapedOddsService from './scrapedOddsService'
...
async function scanOverUnder(match, prediction, line = 2.5) {
  ...
  const marketOU =
    odds?.odds_over_25 || parseFloat(match.odds_over || match[`odds_over_${line}`] || 0)
  if (marketOU > 1) {
...
export = { calculateEV, scanMatch, scanOverUnder, scanAll }
