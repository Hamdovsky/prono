...
jest.mock('../services/doubleOptimizerService', () => ({
  ...
  selectOptimalDoubles: jest
    .fn()
    .mockReturnValue({ expectedCorrect: { withDoubles: 8, allSingles: 7 }, ranked: [] }),
}))
...
describe('Additional API Routes', () => {
  ...
  describe('Config API (/api/config)', () => {
    ...
    it('should not expose config endpoint', async () => {
      const response = await request(app).post('/api/config').send({ scraperUrl: 'http://new-url' })
