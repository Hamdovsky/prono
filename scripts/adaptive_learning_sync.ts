...
async function syncLearning() {
  ...
  try {
    ...
    logger.info(
      `📈 [TITANIUM-LEARN] Found ${candidateMatches.length} unindexed matches. Ingesting...`
    )
    ...
    if (report.success) {
      logger.info(
        `🎉 [TITANIUM-LEARN] Batch complete! Correct: ${report.correctCount}/${report.totalProcessed}`
      )
    } else {
