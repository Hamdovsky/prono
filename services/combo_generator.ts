...
class ComboStrategy {
  ...
  evaluate(matches) {
    ...
    return matches.filter((m) => {
      ...
      mCopy.probThreshold = 0.7 * probMult
      mCopy.confThreshold = 0.7 * confMult
...
class ComboGenerator {
  constructor() {
    this.strategies = [
      ...
      new ComboStrategy('Bankroll Builder', 2, 3, 1.3, 3.0, (m) => {
        const prob = Math.max(
          m.home_win_probability || 0,
          m.away_win_probability || 0,
          m.draw_probability || 0
        )
        return prob >= (m.probThreshold || 0.7)
      }),
      // 2. ⚖️ Elite Double (Balanced)
      new ComboStrategy('Elite Double', 2, 2, 2.2, 6.0, (m) => {
        const prob = Math.max(
          m.home_win_probability || 0,
          m.away_win_probability || 0,
          m.draw_probability || 0
        )
        return prob >= (m.probThreshold || 0.65) && m.xgboost_confidence >= (m.confThreshold || 0.7)
      }),
      // 3. 🚀 Moonshot (High Yield)
      new ComboStrategy('Moonshot', 3, 5, 5.0, 50.0, (m) => {
        const prob = Math.max(
          m.home_win_probability || 0,
          m.away_win_probability || 0,
          m.draw_probability || 0
      ...
      new ComboStrategy('Millionaire Selection', 1, 2, 1.4, 4.0, (m) => {
        // Inclusion criteria: Surgical Strike OR Ultra Confidence OR VVIP
        return (
          (m.verdict && m.verdict.includes('SURGICAL')) ||
      ...
      for (let size = strategy.minMatches; size <= strategy.maxMatches; size++) {
        ...
        combos.forEach((combo) => {
          ...
          const legs = combo.map((m) => {
            ...
            if ((m.away_win_probability || 0) > prob) {
              pick = '2'
              prob = m.away_win_probability
            }
            if ((m.draw_probability || 0) > prob && (m.draw_probability || 0) > 0.4) {
              pick = 'X'
              prob = m.draw_probability
            }

            const odds =
              m[`odds_${pick === '1' ? 'home' : pick === '2' ? 'away' : 'draw'}`] ||
              this._calculateFairOdds(prob)
            totalOdds *= odds
