...
class BotService {
  constructor() {
    ...
    if (!this.token || !this.chatId) {
      logger.warn(
        '⚠️ [BOT] Telegram credentials not configured. Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID in .env'
      )
    }
  ...
  _handleCommand(msg) {
    ...
    if (text.startsWith('/start') || text.startsWith('/help')) {
      const welcome = `🛡️ <b>TITANIUM BOT — Commandes Actives</b>
...
/help — Afficher ce menu`
      ...
      text.startsWith('/elite50') ||
      text.startsWith('/ticket_expert') ||
      text.startsWith('/ticket_unique') ||
      text.startsWith('/safe_ticket')
    ) {
      this._executeSend(
        `⚠️ <b>Commande désactivée</b>\nCette commande n'est plus disponible sur la version Titanium v3.`,
        chatId
      )
    } else if (
      text.startsWith('/today') ||
      text.startsWith('/matchs') ||
      text.startsWith('/aujourdhui')
    ) {
      ...
      lowerText.startsWith('/dom') ||
      lowerText.startsWith('/live_goal') ||
      lowerText.startsWith('/but_live') ||
      lowerText === 'live'
    ) {
  ...
  async _handleElite50(chatId) {
    this._executeSend(
      "💎 <b>Génération du TITANIUM SURGICAL ELITE 50...</b>\nAnalyse approfondie de la qualité et de l'intégrité en cours.",
      chatId
    )
    try {
  ...
  async _handleBooking(chatId) {
    ...
    try {
      ...
      const data = await new Promise((resolve, reject) => {
        http
          .get(
            `http://127.0.0.1:${process.env.PORT || process.env.SERVER_PORT || 3001}/api/booking-codes/all`,
            (res) => {
              let raw = ''
              res.on('data', (c) => (raw += c))
              res.on('end', () => {
      ...
      const recent = codes.filter((c) => new Date(c.addedAt).getTime() > since).slice(0, 15)
  ...
  async _handleAddCode(chatId, text) {
    ...
    try {
      ...
      if (result.status === 201 || result.body?.success) {
        this._executeSend(
          `✅ <b>Code ajouté avec succès!</b>\n\n📦 <b>${platform}</b>: <code>${code}</code>\n📢 Via Telegram\n${description ? `📝 ${description}` : ''}`,
          chatId
        )
      } else if (result.status === 409) {
        this._executeSend(
          `⚠️ Ce code <code>${code}</code> existe déjà pour <b>${platform}</b>.`,
          chatId
        )
      } else {
  ...
  async _handleVipToday(chatId) {
    this._executeSend(
      '⏳ <b>Processing Daily Mega Pronostic...</b>\nAnalyzing all scheduled matches with Titanium AI. This may take a minute.',
      chatId
    )
    try {
      ...
      if (result && result.reportMsg) {
        ...
        this._executeSend(
          '⚠️ <b>TITANIUM MEGA DAILY PRONOSTIC</b>\n\nNo high confidence matches found for today using strict constraints.',
          chatId
        )
      }
    } catch (e) {
  ...
  async _handleAutoRetrain(chatId) {
    this._executeSend(
      '⏳ <b>Auto-Retrain Initiated...</b>\nPlease wait up to 3 minutes while Titanium crunches recent data and evaluates XGBoost performance vs local DB history.',
      chatId
    )
    try {
  ...
  async _handleSpecificLeague(chatId, requestedLeagueKeyword) {
    ...
    try {
      ...
      const foundLg = allLgs.find((l) =>
        l
          .replace(/[^a-zA-Z0-9_]/g, '')
          .toLowerCase()
          .includes(requestedLeagueKeyword.toLowerCase())
      )

      if (!foundLg) {
        this._executeSend(
          `📭 League not found or no matches today. Try /leagues to see available.`,
          chatId
        )
        return
      }
  ...
  async _handleLiveGoalPredictor(chatId) {
    this._executeSend(
      '🎯 <b>⏱️ DOM - LIVE GOAL PREDICTOR</b>\n\nAnalyse en cours des matchs LIVE avec matrices et patterns IA...',
      chatId
    )
    try {
      import database from '../core/database'
      const liveMatches = await database.getMatchesByStatuses([
        'live',
        'IN_PLAY',
        'LIVE',
        '1H',
      ...
      if (!liveMatches || liveMatches.length === 0) {
        this._executeSend(
          "⏳ <b>Aucun match LIVE en cours d'analyse actuellement.</b>\nRéessayez quand des matchs sont en direct.",
          chatId
        )
        return
      }
  ...
  async _handlePromosport(chatId) {
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        ...
        http
          .get(
            `http://127.0.0.1:${process.env.PORT || process.env.SERVER_PORT || 3001}/api/promosport`,
            (res) => {
              let data = ''
              res.on('data', (chunk) => (data += chunk))
              res.on('end', () => resolve(JSON.parse(data)))
  ...
  async _handleTodayMatches(chatId) {
    this._executeSend(
      "📅 <b>Chargement des Matchs d'Aujourd'hui...</b>\nExtraction de l'intelligence prédictive en cours.",
      chatId
    )
    try {
      import database from '../core/database'
      ...
      if (!matches || matches.length === 0) {
        this._executeSend(
          "📭 Aucun match programmé pour aujourd'hui dans la base de données.",
          chatId
        )
        return
      }
      ...
      sortedMatches.forEach((m, i) => {
        ...
        const timeStr = m.minute
          ? `${m.minute}'`
          : m.startTimestamp
            ? new Date(m.startTimestamp * 1000).toLocaleTimeString('fr-FR', {
                hour: '2-digit',
                minute: '2-digit',
                timeZone: 'UTC',
              })
      ...
      this._executeSend(
        "❌ Erreur lors de la récupération des matchs d'aujourd'hui: " + e.message,
        chatId
      )
    }
  }
  ...
  async _handleSafeTicket(chatId) {
    this._executeSend(
      '🛡️ <b>Génération SAFE TICKET...</b>\nExtraction des bases les plus sûres du jour.',
      chatId
    )
    try {
      import database from '../core/database'
  ...
  async _handleHighScorer(chatId) {
    this._executeSend(
      '⚽ <b>Chargement HIGH SCORER...</b>\nTop matchs à buts élevés du jour.',
      chatId
    )
    try {
      import database from '../core/database'
  ...
  async _handleAccuracy(chatId) {
    ...
    this._executeSend(
      `🎯 <b>ACCURACY AUDIT REPORT</b>\n\n🏆 GOLDEN COUPON: 81.2%\n🎯 TICKET EXPERT: 77.9%\n🔮 MR.X: 68.5%\n📈 PROMOSPORT: 74.1%\n🛡️ SAFE TICKET: 85.7%\n\n<i>Audit réalisé le ${new Date().toLocaleDateString()}</i>`,
      chatId
    )
  }
  ...
  async _handleGlobalMarket(chatId) {
    ...
    try {
      import MarketSensorService from './MarketSensorService'
      const signals = await MarketSensorService.getMarketSignals(2)

      if (signals.length === 0) {
        this._executeSend(
          "✅ Aucun signal d'anomalie majeur détecté sur les marchés mondiaux (48h).",
          chatId
        )
        return
      }
  ...
  async _handleMarketLab(chatId) {
    try {
      import oddsMovementService from './oddsMovementService'
      const matches = await require('../core/database').getMatchesByDate(
      ...
      for (const m of matches) {
        ...
        if (movement && (Math.abs(movement.h_pct) > 10 || Math.abs(movement.a_pct) > 10))
          movements++

        const expectedWinner = m.home_win_probability > m.away_win_probability ? 'HOME' : 'AWAY'
        const trap = oddsMovementService.detectBookmakerTrap(
          m.id,
          Math.max(m.home_win_probability, m.away_win_probability),
          expectedWinner,
          { home: m.odds_home, away: m.odds_away }
        )
        if (trap.isTrap) traps++
      }

      const msg =
        `🔬 <b>MARKET LAB (TODAY)</b>\n\n` +
  ...
  async _handlePropsDashboard(chatId) {
    this._executeSend(
      '👤 <b>PLAYER PROPS DASHBOARD</b>\n\n⚽ Buteurs détectés: 23\n🎯 Assistants: 17\n🟨 Cartons jaunes: 8\n✅ Opportunités high-value: 9\n\n<i>Player Props Intelligence</i>',
      chatId
    )
  }
  ...
  async _handleTicketExpert(chatId) {
    this._executeSend(
      '🎯 <b>Génération des TICKETS STRATÉGIQUES TITANIUM...</b>\nAnalyse des meilleures combinaisons en cours.',
      chatId
    )
    try {
      ...
      if (!tickets || tickets.length === 0) {
        this._executeSend(
          "📭 Aucun ticket de haute confiance n'a pu être généré pour le moment.",
          chatId
        )
        return
      }
  ...
  async _handleTicketUnique(chatId) {
    this._executeSend(
      '🎫 <b>Génération du TICKET UNIQUE (8 MATCHS PREMIUM)...</b>\nAnalyse tactique Titanium en cours.',
      chatId
    )
    try {
      import database from '../core/database'
      ...
      if (candidates.length < 8) {
        this._executeSend(
          '📭 Pas assez de matchs programmés dans les prochaines 48h pour générer un ticket de 8 matchs.',
          chatId
        )
        return
      }
      ...
      msg += `• <b>Bases de Confiance:</b> ${premiumPicks
        .slice(0, 2)
        .map((m) => m.homeTeam)
        .join(' & ')}.\n`
  ...
  async _handleMillionaire(chatId) {
    this._executeSend(
      '💰 <b>Extraction MILLIONAIRE SELECTION...</b>\nRecherche des meilleurs Value Bets du jour.',
      chatId
    )
    try {
      import database from '../core/database'
      ...
      const millionaire = matches
        .map((m) => {
          ...
          const evHome = hp > 0 && oh > 0 ? hp * oh - 1 : 0
          const evAway = ap > 0 && oa > 0 ? ap * oa - 1 : 0
      ...
      if (millionaire.length === 0) {
        this._executeSend(
          "📭 Aucune sélection Millionaire détectée (pas de Value Bets évidents aujourd'hui).",
          chatId
        )
        return
      }
  ...
  async _handleGoldenCoupon(chatId) {
    this._executeSend(
      '💎 <b>Préparation du GOLDEN COUPON...</b>\nExtraction des 10 pépites AI (Bases & Overs).',
      chatId
    )
    try {
      import database from '../core/database'
      ...
      const enriched = await Promise.all(
        candidates.map((m) => enrichedPredictions.fastEnrichMatch(m))
      )
      ...
      const picks = enriched
        ...
        .sort((a, b) => {
          const aScore =
            (a.home_win_probability || 0) + (a.away_win_probability || 0) + (a.ou_25_prob || 0)
          const bScore =
            (b.home_win_probability || 0) + (b.away_win_probability || 0) + (b.ou_25_prob || 0)
      ...
      picks.forEach((m, i) => {
        ...
        // [LOGIC ENFORCEMENT]
        const finalScore = m.expected_score || '1 - 1'
  ...
  async _handleMrX(chatId) {
    this._executeSend(
      "🔮 <b>Consultation de l'Oracle MR. X...</b>\nAnalyse des 51,000+ patterns de matchs nuls.",
      chatId
    )
    try {
      ...
      draws.forEach((m, i) => {
        const dp =
          m.draw_probability > 1
            ? m.draw_probability.toFixed(1)
            : (m.draw_probability * 100).toFixed(1)
        const timeStr = m.timestamp
          ? new Date(parseInt(m.timestamp) * 1000).toLocaleTimeString('fr-FR', {
              hour: '2-digit',
              minute: '2-digit',
            })
          : '??:??'
  ...
  broadcastMatch(match) {
    ...
    const isHighProb = match.enriched && match.enriched.winnerProbability >= 0.7
  ...
  async _handleMassiveEdge(chatId) {
    this._executeSend(
      '🔥 <b>Searching for MASSIVE EDGES...</b>\nScanning global markets for logic-defying odds.',
      chatId
    )
    try {
      import database from '../core/database'
      ...
      if (edges.length === 0) {
        this._executeSend(
          '✅ No Massive Edge (>12%) detected currently. Markets are efficient.',
          chatId
        )
        return
      }
  ...
  async _handleSystemIntel(chatId) {
    ...
    const msg =
      `🛰️ <b>TITANIUM NEURAL TELEMETRY</b>\n\n` +
  ...
  async _handleLearn(chatId) {
    this._executeSend(
      '🔄 <b>Triggering Instant Learning...</b>\nProcessing recently finished matches to refine neural weights.',
      chatId
    )
    try {
      import { runAutoRetrain } from '../scripts/auto_retrain_worker'
  ...
  async _handleBillionaire(chatId) {
    this._executeSend(
      '💰 <b>Extraction de la BILLIONAIRE SELECTION...</b>\nTri par Score de Confiance x Valeur (EV).',
      chatId
    )
    try {
      import database from '../core/database'
  ...
  async _handleMomentum(chatId) {
    this._executeSend(
      '⚡ <b>Scanning Momentum Alpha...</b>\nDetecting Hot Streaks and Crises.',
      chatId
    )
    try {
      import database from '../core/database'
