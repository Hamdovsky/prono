...
const AUTOPSY_COLORS = {
  XG_WASTE: '#f59e0b', GK_WALL: '#3b82f6', LATE_GOAL: '#8b5cf6',
  PERSONNEL_DEFICIT_DISRUPTION: '#ef4444', SYSTEMIC_DEFENSIVE_FAILURE: '#dc2626',
  BIG_CHANCE_WASTE: '#f97316', EARLY_TACTICAL_DISRUPTION: '#06b6d4',
  SET_PIECE_DECIDER: '#10b981', SHOT_DOMINANCE: '#a855f7',
  CORNER_DOMINANCE: '#ec4899', LOW_INTENSITY_OFFENSE: '#64748b',
  POSSESSION_FAIL: '#84cc16', UNKNOWN: '#374151',
};
  ...
  return (
    <div className="surgical-report animate-in">
      <div className="surgical-grid">
        <div className="surgical-radar"><RadarChartSVG data={chartData} height={220} color="#ef4444" /></div>
          ...
          <div className="surgical-timeline">
            {(data.criticalIncidents?.length > 0 ? data.criticalIncidents : []).map((inc, idx) => (
              <div key={idx} className="timeline-item">
                ...
                <span className="timeline-icon">{(inc.type || inc.incidentClass || '').toLowerCase().includes('goal') ? '⚽' : (inc.type || inc.incidentClass || '').toLowerCase().includes('card') ? '🟥' : '🎯'}</span>
      ...
      <div className="surgical-footer">
        ...
        <p className="surgical-tip">💡 Ce type d'échec est récurrent dans cette ligue. Envisage un pari Double Chance.</p>
      </div>
    </div>
  );
};
      ...
      <table className="acc-table">
        ...
        <tbody>
          {data.map((row, i) => (
            <tr key={row.id || i}>
              {columns.map(c => (
                <td key={c.key} style={{ textAlign: c.align || 'left', color: c.color ? (typeof c.color === 'function' ? c.color(row) : c.color) : undefined, fontWeight: c.bold ? 700 : undefined }}>
  ...
  return (
    <div className="acc-dashboard">
      ...
      <div className="acc-tabs">
        {tabs.map(t => (
          <button key={t.id} className={`acc-tab ${activeTab === t.id ? 'active' : ''}`} onClick={() => setActiveTab(t.id)}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'overview' && (
        <div>
          <div className="acc-kpis">
            ...
            <KpiCard label="Win Rate" value={`${perf?.win_rate || 0}%`} color={getAccColor(perf?.win_rate)} />
            <KpiCard label="ROI" value={`${perf?.roi_percent >= 0 ? '+' : ''}${perf?.roi_percent || 0}%`} color={perf?.roi_percent >= 0 ? '#10b981' : '#ef4444'} />
            <KpiCard label="Profit" value={`${perf?.profit_units >= 0 ? '+' : ''}${perf?.profit_units || 0}u`} color={perf?.profit_units >= 0 ? '#10b981' : '#ef4444'} />
            <KpiCard label="Précision globale" value={`${tracker?.winRate || 0}%`} color={getAccColor(tracker?.winRate)} />
            ...
            <div className="acc-chart-card">
              ...
              <div className="acc-breakdown-grid">
                {[
                  ...
                  const barColor = val < 0 ? '#fb7185' : val >= c.max * 0.8 ? '#10b981' : val >= c.max * 0.5 ? '#38bdf8' : '#64748b';
                  return (
                    <div key={c.key} className="acc-breakdown-row">
                      <div className="acc-breakdown-header">
                        <span>{c.icon} {c.label}</span>
                        <span style={{ color: barColor, fontWeight: 700 }}>{val > 0 ? '+' : ''}{val}/{c.max} pts</span>
                      </div>
                      <div className="acc-breakdown-bar-bg">
                        <div className="acc-breakdown-bar" style={{ width: `${Math.max(0, pct)}%`, background: barColor }} />
                      </div>
                    </div>
                  );
                })}
          ...
          {last30.length > 0 && (
            <div className="acc-chart-card">
              ...
              <AreaChartSVG data={last30} xKey="date" yKey="cumulativeRoi" height={280} color="#10b981" />
            </div>
          )}
        </div>
      )}
        ...
        <div className="acc-chart-card">
          ...
          <MiniTable
            ...
            columns={[
              ...
              { key: 'winRate', label: 'Win Rate', align: 'center', color: r => getAccColor(r.winRate), render: r => `${r.winRate}%` },
            ]}
          />
        </div>
      )}
      ...
      {activeTab === 'confidence' && (
        <div className="acc-chart-card">
          ...
          <MiniTable
            data={perf?.by_confidence ? Object.entries(perf.by_confidence).map(([k, v]) => ({ bracket: k, ...v })) : []}
            ...
            columns={[
              ...
              { key: 'win_rate', label: 'Win Rate', align: 'center', color: r => getAccColor(r.win_rate), render: r => `${r.win_rate}%` },
            ]}
          />
        </div>
      )}
      ...
      {activeTab === 'markets' && (
        <div className="acc-chart-card">
          ...
          <MiniTable
            data={perf?.by_market ? Object.entries(perf.by_market).map(([k, v]) => ({ market: k, ...v })) : []}
            emptyMsg="Aucune donnée."
            columns={[
              ...
              {(autopsyData?.report || []).slice(0, 30).map((m, i) => (
                <div key={i} className={`acc-miss-entry ${expandedMiss === m.id ? 'expanded' : ''}`} onClick={() => setExpandedMiss(expandedMiss === m.id ? null : m.id)}>
                  ...
                  <div className="acc-miss-tactical">
                    ...
                    <span className="acc-miss-desc">{m.autopsy?.reason || 'Analyse en cours...'}</span>
                  </div>
                  {expandedMiss === m.id && <SurgicalAutopsy data={m} />}
                  <div className="acc-miss-meta">Confiance: {m.confidence}% · {m.prediction} · Clique pour détails</div>
                </div>
              ))}
            </div>
          </div>
