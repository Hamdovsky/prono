...
class PlayerImpactService {
  ...
  async calculateImpact(matchId) {
    ...
    try {
      ...
      return {
        home_attack_mod: this.evaluateMissingPlayers(
          home?.missingPlayers,
          'attack',
          home?.substitutes
        ),
        home_defense_mod: this.evaluateMissingPlayers(
          home?.missingPlayers,
          'defense',
          home?.substitutes
        ),
        away_attack_mod: this.evaluateMissingPlayers(
          away?.missingPlayers,
          'attack',
          away?.substitutes
        ),
        away_defense_mod: this.evaluateMissingPlayers(
          away?.missingPlayers,
          'defense',
          away?.substitutes
        ),
  ...
  evaluateMissingPlayers(missing, mode, substitutes = []) {
    ...
    for (const m of missing) {
      ...
      if (substitutes && substitutes.length > 0 && marketValue > 0) {
        ...
        const bestSub = samePosSubs.reduce(
          (prev, current) => {
      ...
      if (mode === 'attack') {
        if (pos === 'F' || pos === 'M') {
          let penalty = 1.0
          if (isElite)
            penalty = 0.92 // User Request: -8% for Forward (Elite default)
