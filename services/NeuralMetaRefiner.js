...
class NeuralMetaRefiner {
    ...
    async refreshBiasMatrix() {
        ...
        try {
            ...
            logger.info(`[META-REFINER V3] Matrix updated: ${this.biasCache.size} active keys (from ${rows.length} records, cutoff: ${this.MAX_HISTORY_DAYS}d)`);
        } catch (e) {
    ...
    async refine(match) {
        ...
        if (this.biasCache.has(hKey)) {
            ...
            refined.home_win_probability = Math.min(99, Math.max(1, (match.home_win_probability || 0) * adjustedFactor));
        ...
        if (this.biasCache.has(dKey)) {
            ...
            const adjustedFactor = 1.0 + (factor - 1.0) * confidence;
            refined.draw_probability = Math.min(99, Math.max(1, (match.draw_probability || match.draw_win_probability || 0) * adjustedFactor));
        ...
        if (this.biasCache.has(aKey)) {
            ...
            const adjustedFactor = 1.0 + (factor - 1.0) * confidence;
            refined.away_win_probability = Math.min(99, Math.max(1, (match.away_win_probability || 0) * adjustedFactor));
            refined.meta_correction_a = +adjustedFactor.toFixed(4);
        }

        // 4. Renormalize probabilities to sum to 100%
        const total = (refined.home_win_probability || 0) + (refined.draw_probability || 0) + (refined.away_win_probability || 0);
