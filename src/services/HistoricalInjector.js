...
try {
  archiveDb = new Database(path.join(__dirname, '..', '..', 'data', 'historical_archive.sqlite'), {
    readonly: true,
  })
...
async function injectHistoricalData(matchId) {
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      rows.forEach((r) => {
        const isHomeInArchive = r.homeTeam
          .toLowerCase()
          .includes(home.substring(0, 5).toLowerCase())
    ...
    console.log(
      `📚 [ARCHIVE] Historical Context & Line Movement injected for Match ${matchId} (${home} vs ${away})`
    )
