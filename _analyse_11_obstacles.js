...
async function analyseConcours879() {
  ...
  function analyseObstacles(m, idx) {
    ...
    const bmScore = Math.abs(mlH - 0.33) < 0.05 && Math.abs(mlX - 0.33) < 0.05
      ? 3 : (Math.max(mlH, mlX, mlA) > 0.45 ? 2 : 4);
    
    // 2. TERRAIN — domicile/extérieur (logique: équipe1 = domicile)
    const terrainScore = m.home.includes('CRICI') || m.home.includes('GOIAS') || m.home.includes('NAUTI') 
      ? 2 : (m.away.includes('FRANCE') || m.away.includes('SPAIN') || m.away.includes('ENGLAND') || m.away.includes('BELGIUM')
        ? 3 : 3);

    // 3. STATISTIQUES — basé sur l'entropie du match
    const H = -(mlH * Math.log2(Math.max(0.01, mlH)) + mlX * Math.log2(Math.max(0.01, mlX)) + mlA * Math.log2(Math.max(0.01, mlA)));
    const statsScore = H > 1.5 ? 4 : (H > 1.4 ? 3 : 2);

    // 4. PSYCHOLOGIE — contexte du match (Coupe du Monde = pression)
    const isWC = ['CANADA','MAROC','MOROCCO','BRAZIL','NORVÈGE','NORWAY','MEXICO','ENGLAND','PORTUGAL','SPAIN','PARAGUAY','FRANCE','USA','BELGIUM'];
    ...
    try {
      const homeHist = historicalData.filter(h => 
        h.matches?.some(m => m.home?.toUpperCase().includes(m.home) || m.away?.toUpperCase().includes(m.away))
      );
  ...
  if (lowRisk.length > 0) {
    console.log(`🟢 MATCHS SÛRS (moy < 2.5) : ${lowRisk.map(r => `M${r.id} (${r.avgScore})`).join(', ')}`);
  }
