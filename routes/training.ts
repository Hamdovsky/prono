...
const MODELS_CONFIG = [
  {
    id: 'v24',
    name: 'XGBoost V24',
    file: 'stitch_v24_hybrid.json',
    script: 'core/train_v24_top_analyst.py',
    cron: '1er du mois 04:00',
    worker: 'runAutoRetrain',
  },
  {
    id: 'v56',
    name: 'XGBoost V56 Auto',
    file: 'stitch_v553_premium.json',
    script: 'scripts/auto_retrain.py',
    cron: 'Samedi 23:30',
    worker: 'runV56Retrain',
  },
  {
    id: 'live',
    name: 'Live Goal',
    file: 'live_goal_xgb.json',
    script: 'core/train_live_model.py',
    cron: 'Dimanche 05:00',
    worker: 'runLiveModelRetrain',
  },
  {
    id: 'promosport',
    name: 'Promosport',
    file: 'promosport_xgb.json',
    script: 'scripts/train_promosport_xgboost.py',
    cron: 'Samedi 04:00',
    worker: 'promosport',
  },
  {
  ...
  },
  {
    id: 'corners',
    name: 'Corners',
    file: 'stitch_corners_v1.json',
    script: 'core/train_secondary_markets.py',
    cron: '—',
    worker: null,
  },
  {
...
async function runStep(name, cmd, args, timeout) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (d) => {
      const s = d.toString()
      out += s
      trainState.log.push(`[${name}] ${s.trim()}`)
      if (trainState.log.length > 200) trainState.log.splice(0, 50)
    })
    ...
    child.on('close', (code) =>
      code === 0 ? resolve(out) : reject(new Error(`${name} exited ${code}: ${out.slice(-300)}`))
    )
...
router.get('/status', (req, res) => {
  res.json({
    ...
    type: trainState.type,
    startedAt: trainState.startedAt,
    lastResult: trainState.lastResult,
    log: trainState.log.slice(-50),
  })
})
  ...
  try {
    ...
    if (type === 'promosport') {
      ...
      const importOut = await runStep(
        'Import',
        pythonCmd,
        [path.join(scriptsDir, 'import_promosport_archive.py')],
        120000
      )
      const trainOut = await runStep(
        'Train',
        pythonCmd,
        [path.join(scriptsDir, 'train_promosport_xgboost.py')],
        600000
      )
