...
class AliasResolver {
  constructor(db) {
    ...
    if (this.db && typeof this.db.exec === 'function') {
      ...
      console.warn(
        '⚠️ [ALIAS RESOLVER] Initialized without a valid database handle. Some features will be disabled.'
      )
    }
  }
  ...
  resolve(teamId, scrapedName) {
    ...
    try {
      const row = this.db
        .prepare('SELECT master_name FROM id_master_normalization WHERE sofascore_id = ?')
        .get(teamId)
      if (row) {
  ...
  fuzzyResolve(name) {
    ...
    try {
      ...
      for (const m of masters) {
        ...
        if (
          normalizedScraped.includes(normalizedMaster) ||
          normalizedMaster.includes(normalizedScraped)
        ) {
