...
function getOddsForPick(match, pick) {
    ...
    if (p === '1X' || p === 'X2' || p === '12') {
        ...
        if (p === '1X') { const prob = (1/h + 1/d); return prob > 0 ? 1 / prob : 0; }
        if (p === 'X2') { const prob = (1/a + 1/d); return prob > 0 ? 1 / prob : 0; }
        if (p === '12') { const prob = (1/h + 1/a); return prob > 0 ? 1 / prob : 0; }
    }
    return 0;
}
...
function computeStats(rows) {
  ...
  const totalReturned = rows.reduce((s, r) => r.result === 'won' ? s + (r.stake || 0) * (r.odds || 1) : s, 0);
  ...
  return { total, won, lost, pending, voided, totalStaked: Math.round(totalStaked * 100) / 100, totalReturned: Math.round(totalReturned * 100) / 100, netProfit: Math.round(netProfit * 100) / 100, roi, winRate };
}
...
router.post('/', (req, res) => {
  try {
    ...
    if (!match_label || !pick || !odds || stake === undefined) {
      return res.status(400).json({ success: false, error: 'match_label, pick, odds, stake requis' });
    }
    const db = database.db;
    const profit = result === 'won' ? Math.round(((stake || 0) * (odds || 1) - (stake || 0)) * 100) / 100 : (result === 'lost' ? -(stake || 0) : 0);
    const info = db.prepare(
      'INSERT INTO bets (match_label, league, pick, odds, stake, result, profit, note, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(match_label, league || '', pick, odds, stake, result, profit, note || '', date || new Date().toISOString().split('T')[0]);
...
router.put('/:id', (req, res) => {
  try {
    ...
    const profit = finalResult === 'won' ? Math.round((finalStake * finalOdds - finalStake) * 100) / 100 : (finalResult === 'lost' ? -finalStake : 0);
    db.prepare(
...
router.post('/import', (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (!pick) continue;
      const existing = db.prepare('SELECT id FROM bets WHERE match_label = ? AND pick = ?').get(`${row.homeTeam} vs ${row.awayTeam}`, pick);
      if (existing) continue;
      const sh = row.scoreHome, sa = row.scoreAway;
      ...
      ).run(`${row.homeTeam} vs ${row.awayTeam}`, row.league || '', pick, Math.round(odds * 100) / 100, stake, won ? 'won' : 'lost', profit, row.timestamp ? row.timestamp.split('T')[0] : new Date().toISOString().split('T')[0]);
      imported++;
    }
