...
function buildScoreMatrix(xgH, xgA, maxG = 8) {
  ...
  for (let h = 0; h <= maxG; h++) {
    m[h] = []
    const ph = poissonProb(h, xgH)
    for (let a = 0; a <= maxG; a++) m[h][a] = ph * poissonProb(a, xgA)
  }
  return m
}
...
function determinePick(pH, pD, pA) {
  const p = [
    { l: '1', p: pH },
    { l: 'X', p: pD },
    { l: '2', p: pA },
  ...
  return {
    pick: b.l,
    prob: Math.round(b.p * 10) / 10,
    ev: Math.round(((b.p / 100) * 2 - 1) * 100) / 100,
  }
}
...
async function tryXgb(match) {
  try {
    ...
    let resp
    try {
      resp = await axios.post(`${FASTAPI_URL}/predict`, payload, {
        timeout: XGB_TIMEOUT,
    ...
    const pHome = parseFloat(py.home_win_probability) || 0,
      pDraw = parseFloat(py.draw_probability) || 0,
      pAway = parseFloat(py.away_win_probability) || 0
    if (pHome + pDraw + pAway <= 0.01) return null
    const xgbConf = parseFloat(py.xgboost_confidence || py.confidence || 0)
    const pHPct = +(pHome * 100).toFixed(1),
      pDPct = +(pDraw * 100).toFixed(1),
      pAPct = +(pAway * 100).toFixed(1)
    if (xgbConf < 0.4 || pDPct > 50) return null
    ...
    return {
      home_win_probability: pHPct,
      draw_probability: pDPct,
      away_win_probability: pAPct,
      home_xg: Math.round(xgH * 100) / 100,
      away_xg: Math.round(xgA * 100) / 100,
...
function tryJSEngine(match) {
  try {
    ...
    const pH = Math.round(m.home * 1000) / 10,
      pD = Math.round(m.draw * 1000) / 10,
      pA = Math.round(m.away * 1000) / 10
...
;(async () => {
  ...
  let matches
  try {
    matches = await database.getInsufficientDataMatches()
  } catch (e) {
    console.error('DB query failed:', e.message)
    process.exit(1)
  }
  if (!matches || matches.length === 0) {
    console.log('No insufficient-data matches found.')
    return
  }
  ...
  for (let i = 0; i < matches.length; i += batchSize) {
    ...
    for (const item of results) {
      ...
      try {
        ...
        if (r.prediction) {
          dbPayload.verdict = r.verdict || r.prediction
          dbPayload.prediction_probability = r.prediction_probability
          dbPayload.ev_score = r.ev_score
        }
