...
class IntegrityService {
  ...
  static async analyzeMatch(match, modelPredictions, intelligence) {
    ...
    const rawProb = modelPredictions.home_win_probability || modelPredictions.home || 0
    const normalizedProb = rawProb > 1 ? rawProb / 100 : rawProb

    const fairPrice = normalizedProb > 0 ? (1 / normalizedProb).toFixed(2) : null
    const marketOdds =
      typeof match.market_odds === 'string'
        ? JSON.parse(match.market_odds)
        : match.market_odds || {}
    ...
    if (integrityScore < 15 && normalizedProb > 0.9 && squadHealth > 85) {
    ...
    if ((parseFloat(marketOdds.home) >= 2.0 && parseFloat(edge) > 0.2) || hasInsidersEdge) {
  ...
  static _checkSmartMoneyPulse(match) {
    ...
    const odds =
      typeof match.market_odds === 'string' ? JSON.parse(match.market_odds) : match.market_odds
  ...
  static async _checkRefereeRisk(match) {
    ...
    const isStrict = yellowAvg > 4.5 || redAvg > 0.25 || penAvg > 0.4
  ...
  static _checkVolatility(match) {
    if (match.odds_drop_alert) {
      return {
        ...
        details:
          'رصد هبوط حاد في الأسعار قبل صافرة البداية: احتمال تسريب معلومات (Information Leak).',
        weight: 30,
  ...
  static _checkSentimentRisk(intel) {
    ...
    const matched = intel.headlines.filter((h) =>
      crisisKws.some((kw) => h.toLowerCase().includes(kw))
    )
    if (matched.length > 0) {
      return {
  ...
  static async _checkGhostPattern(match) {
    ...
    const scoreCounts = scores.reduce((acc, s) => {
      acc[s] = (acc[s] || 0) + 1
      return acc
    }, {})
  ...
  static _getRecommendation(score, anomaly, insidersEdge) {
    ...
    if (anomaly && (anomaly.tag === 'BOOKIE_ERROR' || anomaly.tag === 'INSIDERS_EDGE'))
      return 'استغلال (EXPLOIT)'
  ...
  static _checkAHDivergence(match, model) {
    ...
    if (match.odds_drop_alert && odds.home > 2.5) {
      return {
        ...
        details:
          'البوكميكر يرفع سعر فوز صاحب الأرض، لكنه يحمي الهانديكاب. هذا يشير إلى توقع "تعادل موجه" أو حماية من فوز الضيف بصعوبة.',
        weight: 25,
        tag: 'AH_TRAP',
      }
    }
