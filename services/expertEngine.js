...
async function getMatchIntelligence(m) {
  try {
    ...
    const daHome =
      statsSource.dangerousAttacks?.home ||
      getStatFromArray('Dangerous Attacks', true) ||
      parseStat(m.dangerous_attacks_home) ||
      0
    const daAway =
      statsSource.dangerousAttacks?.away ||
      getStatFromArray('Dangerous Attacks', false) ||
      parseStat(m.dangerous_attacks_away) ||
      0
    const totalDA = daHome + daAway

    const possHome =
      statsSource.possession?.home ||
      getStatFromArray('Possession', true) ||
      parseStat(m.possession_home) ||
      50
    const possAway =
      statsSource.possession?.away ||
      getStatFromArray('Possession', false) ||
      parseStat(m.possession_away) ||
      50

    const shotsHome =
      statsSource.totalShots?.home ||
      getStatFromArray('Shots on Target', true) + getStatFromArray('Shots off Target', true) ||
      parseStat(m.shots_on_target_home) ||
      0
    const shotsAway =
      statsSource.totalShots?.away ||
      getStatFromArray('Shots on Target', false) + getStatFromArray('Shots off Target', false) ||
      parseStat(m.shots_on_target_away) ||
      0

    const cornersHome =
      statsSource.corners?.home ||
      getStatFromArray('Corner Kicks', true) ||
      parseStat(m.corners_home) ||
      0
    const cornersAway =
      statsSource.corners?.away ||
      getStatFromArray('Corner Kicks', false) ||
      parseStat(m.corners_away) ||
      0
    ...
    let visionBoost = 0
    try {
      const vision = database.get(
        'SELECT description FROM vision_log ORDER BY timestamp DESC LIMIT 1'
      )
    ...
    const fatiguePenalty =
      travelFatigue.impact === 'HIGH' ? 8 : travelFatigue.impact === 'MEDIUM' ? 4 : 0
...
async function simulate(m, lineupImpact = {}) {
  try {
    ...
    const baseLambda = (ou25Prob / 100) * 1.6 + 1.6
    ...
    const homeShare = (homeWinP + drawP * 0.25) / (totalWinP || 1)
    const awayShare = (awayWinP + drawP * 0.25) / (totalWinP || 1)
