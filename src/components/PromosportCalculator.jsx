...
export default function PromosportCalculator({ matches, fetcher }) {
  ...
  const calcLocal = (c, d) => {
    ...
    if (matches && matches.length > 0) {
      expectedCorrect = matches.reduce((sum, m) => {
        const best = Math.max(
          m.mlProbs?.h || m.probs?.h || 33,
          m.mlProbs?.x || m.probs?.x || 33,
          m.mlProbs?.a || m.probs?.a || 34
        )
        return sum + best / 100
      }, 0)
  ...
  const generateColumns = () => {
    ...
    const analyzed = matches.map((m, i) => {
      ...
      const ph = hp / 100,
        ...
        pa = ap / 100
      const entropy = -(
        ph * Math.log2(ph + 0.001) +
        px * Math.log2(px + 0.001) +
        pa * Math.log2(pa + 0.001)
      )
    ...
    const uncertainSorted = analyzed
      .filter((a) => a.isUncertain)
  ...
  return (
    <div className="promosport-calculator" style={{ padding: '20px' }}>
      ...
      >
        <h3
          style={{ color: '#818cf8', fontSize: '1.6rem', marginBottom: '5px', fontWeight: '900' }}
        >
              ...
              {COL_QUICK.map((v) => (
                <button
                  ...
                  style={{
                    ...
                    padding: '8px 4px',
                    borderRadius: '8px',
                    border: cols === v ? '2px solid #818cf8' : '1px solid rgba(99, 102, 241, 0.25)',
                    ...
                    flex: '0 0 auto',
                    textAlign: 'center',
                    boxShadow: cols === v ? '0 0 12px rgba(99, 102, 241, 0.3)' : 'none',
                  }}
                >
                  {v}
                </button>
          ...
          <div style={{ flex: '1', minWidth: '240px' }}>
            ...
            >
              {DOUBLE_VALS.map((v) => (
                <button
                  key={v}
                  onClick={() => setDoubles(v)}
                  style={{
                    padding: '8px 0',
                    borderRadius: '8px',
                    border:
                      doubles === v ? '2px solid #fbbf24' : '1px solid rgba(251, 191, 36, 0.2)',
                    background:
                      doubles === v ? 'rgba(251, 191, 36, 0.25)' : 'rgba(251, 191, 36, 0.04)',
        ...
        {result && (
          ...
          >
            ...
            >
              ...
              >
                <div style={{ color: '#818cf8', fontSize: '1.8rem', fontWeight: '900' }}>
                  {result.coverage?.systemCoverage?.toFixed(1) || result.coverage}%
                </div>
                <div style={{ color: '#94a3b8', fontSize: '0.75rem' }}>COUVERTURE SYSTÈME</div>
                <div style={{ color: '#64748b', fontSize: '0.65rem', marginTop: '4px' }}>
                  {result.input?.cols || cols} / {result.combinations?.fullSystem || result.full}{' '}
                  colonnes
                </div>
              </div>
              ...
              >
                <div style={{ color: '#34d399', fontSize: '1.8rem', fontWeight: '900' }}>
                  {result.combinations?.fullSystem || result.full}
                </div>
                <div style={{ color: '#94a3b8', fontSize: '0.75rem' }}>COMBINAISONS TOTALES</div>
                <div style={{ color: '#64748b', fontSize: '0.65rem', marginTop: '4px' }}>
                  2<sup>{result.input?.doubles || doubles}</sup> ={' '}
                  {(result.combinations?.fullSystem || result.full).toLocaleString()}
                </div>
              </div>
              ...
              {result.expectedCorrect && (
                ...
                >
                  <div style={{ color: '#a78bfa', fontSize: '1.8rem', fontWeight: '900' }}>
                    {result.expectedCorrect}/13
                  </div>
                  <div style={{ color: '#94a3b8', fontSize: '0.75rem' }}>
                    BONNES RÉPONSES ESTIMÉES
                  </div>
                  <div style={{ color: '#64748b', fontSize: '0.65rem', marginTop: '4px' }}>
                    Moyenne des meilleures probas
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom: '15px' }}>
              ...
              >
                ...
                <span style={{ color: '#818cf8', fontWeight: 'bold' }}>
                  {(result.coverage?.systemCoverage ?? result.coverage).toFixed(1)}%
                </span>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  {(result.coverage?.systemCoverage ?? result.coverage) >= 5 &&
                    `${(result.coverage?.systemCoverage ?? result.coverage).toFixed(0)}%`}
                </div>
              </div>
            </div>
            ...
            >
              <div>
                ...
                <span style={{ color: '#94a3b8' }}>
                  {result.systemType || result.calculations?.systemType}
                </span>
              </div>
              <div>
                <span style={{ color: '#64748b' }}>Prix/colonne :</span>{' '}
                <span style={{ color: '#94a3b8' }}>{COLUMN_PRICE} DT</span>
              </div>
              <div>
                <span style={{ color: '#64748b' }}>Taxe (6%) :</span>{' '}
                <span style={{ color: '#94a3b8' }}>
                  {result.pricing?.tax || `${(result.costBefore * TAX_RATE).toFixed(3)} DT`}
                </span>
              </div>
              <div>
                <span style={{ color: '#64748b' }}>Budget max :</span>{' '}
                <span style={{ color: '#94a3b8' }}>
                  {result.pricing?.total || `${result.costAfter.toFixed(3)} DT`}
                </span>
              </div>
            ...
            {(result.coverage?.systemCoverage ?? result.coverage) < 10 && (
              ...
              >
                ⚠️ Réduction sévère (
                {(result.coverage?.systemCoverage ?? result.coverage).toFixed(1)}%). Envisagez plus
                de colonnes ou moins de doubles.
              </div>
            )}
            {(result.coverage?.systemCoverage ?? result.coverage) >= 50 && (
              ...
              >
                ✅ Bonne couverture (
                {(result.coverage?.systemCoverage ?? result.coverage).toFixed(1)}%). Risque modéré.
              </div>
            )}
          </div>
        )}
            ...
            {displayCols && stats && (
              ...
              >
                ...
                >
                  ...
                  >
                    <div style={{ color: '#818cf8', fontSize: '1.4rem', fontWeight: '900' }}>
                      {stats.avg.toFixed(1)}
                    </div>
                    <div
                      style={{
                  ...
                  >
                    <div style={{ color: '#34d399', fontSize: '1.4rem', fontWeight: '900' }}>
                  ...
                  >
                    <div style={{ color: '#f87171', fontSize: '1.4rem', fontWeight: '900' }}>
                ...
                >
                  <span style={{ color: '#94a3b8', fontSize: '0.7rem', fontWeight: '700' }}>
                    {displayCols.cols.length} colonnes affichées · {generatedCols.count} générées
                    sur {generatedCols.total} possibles
                  </span>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button
                      onClick={() => setSortBy(sortBy === 'expected' ? 'index' : 'expected')}
                      style={{
                        background:
                          sortBy === 'expected' ? 'rgba(99, 102, 241, 0.2)' : 'transparent',
