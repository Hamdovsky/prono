...
function getDb() {
  ...
  const columns = db
    .prepare('PRAGMA table_info(archive_matches)')
    .all()
    .map((c) => c.name)
  const required = [
...
async function fetchFinishedMatchesForDate(dateStr) {
  ...
  try {
    ...
    logger.info(
      `📦 [AutoArchiver] ${dateStr}: ${finished.length} finished matches found out of ${events.length}`
    )
    return finished
  } catch (e) {
...
function parseStats(statisticsData) {
  ...
  result.home_shots_on_target =
    allStats['shots_on_target_home'] || allStats['shots_on_goal_home'] || 0
  result.away_shots_on_target =
    allStats['shots_on_target_away'] || allStats['shots_on_goal_away'] || 0
    ...
    for (const event of matches) {
      try {
        ...
        if (saved) {
          totalArchived++
          logger.info(
            `  ✅ Archived: ${event.homeTeam?.name} ${event.homeScore?.current}-${event.awayScore?.current} ${event.awayTeam?.name}`
          )
        } else {
          totalSkipped++
        }
