...
assert(
  typeof s1.confidence === 'number' && s1.confidence >= 15,
  `Confidence ≥ 15% (got ${s1.confidence}%)`
)
assert(s1.match_type === 'Early-Phase Match', 'Early-Phase Match type triggered')
const hasAH =
  s1.adjusted_prediction.includes('Asian Handicap') || s1.adjusted_prediction.includes('Next Goal')
assert(hasAH, 'Asian Handicap or Next Goal pivot applied for early deficit')
assert(
  Array.isArray(s1.alternative_angle) && s1.alternative_angle.length === 3,
  '3-tier alternative angles returned'
)
assert(
  s1.alternative_angle.some((a) => a.tier === 'Aggressive'),
  'Aggressive tier present'
)
assert(
  s1.alternative_angle.some((a) => a.tier === 'Moderate'),
  'Moderate tier present'
)
assert(
  s1.alternative_angle.some((a) => a.tier === 'Conservative'),
  'Conservative tier present'
)
...
const s2 = e.processLive({
  ...
  away_win_probability: 33.4, // flat
  odds_away: 2.0,
  odds_away_open: 2.5, // away odds shortened by 0.5 — strong drift
  odds_home: 3.5,
  odds_home_open: 3.4,
})
...
const s3 = e.processLive({
  ...
  odds_home: 2.8,
  odds_home_open: 2.6, // odds drifting out for home (under-priced by model)
  _raw_odds_home: 2.8,
})
...
assert(
  Array.isArray(s3.alternative_angle) && s3.alternative_angle.length === 3,
  '3-tier angles present in normal live mode'
)
...
assert(
  typeof s4.confidence === 'number' && s4.confidence >= 15,
  `Confidence ≥ 15% even on null (got ${s4.confidence}%)`
)
assert(
  s4.trap_alert && typeof s4.trap_alert.flagged === 'boolean',
  'trap_alert.flagged always a boolean'
)
