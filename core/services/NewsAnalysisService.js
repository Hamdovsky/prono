...
class NewsAnalysisService {
  calculateNewsScore(headlines, confirmedInjuries = [], teamAvgRating = null) {
    if (
      ...
      (!confirmedInjuries || confirmedInjuries.length === 0)
    ) {
    ...
    for (const injury of confirmedInjuries || []) {
      ...
      if (
        ['injury', 'illness', 'suspended', 'red card', 'unavailable', 'missing'].some((k) =>
          reason.includes(k)
        ) ||
        !reason
      ) {
        ...
        criticalNews.push(
          `${name || 'Player'} OUT (${source.includes('sofa') ? 'Official' : 'TM'})`
        )
      }
    }
    ...
    for (const headline of headlines || []) {
      ...
      if (
        ...
        h.includes('key ') ||
        h.includes('main ') ||
        h.includes('first-choice')
      )
        importanceModifier = 1.5
      ...
      if (
        h.includes('late fitness test') ||
        h.includes('doubtful') ||
        h.includes('decision close to kick-off')
      ) {
        chaosBoost += 5
