...
import marketCorrelation from './MarketCorrelationEngine'
import logger from '../core/logger'
import lineup from './LineupService'
import squadRotation from './SquadRotationAnalyst'
import oracle from './expertEngine'
...
function earlyMatchHeuristics(m) {
  ...
  if (scoreA > scoreH && scoreA - scoreH === 1) {
    ...
    const prediction = motivationBoost ? `${homeName} +1 Asian Handicap` : `Next Goal: ${homeName}`
    const note =
      `Early Deficit Recovery [min=${minute}]: ${homeName} is 0-1 down. ` +
    ...
    return {
      ...
      alternative_angles: [
        {
          tier: 'Aggressive',
          angle: `Next Goal: ${homeName}`,
          rationale: 'Comeback pressure — first reactor advantage',
        },
        {
          tier: 'Moderate',
          angle: `${homeName} +1 Asian Handicap`,
          rationale: 'Absorbs early deficit — safer recovery play',
        },
        {
          tier: 'Conservative',
          angle: 'Over 1.5 Goals',
          rationale: 'Both teams incentivised to score early',
        },
      ],
    }
  }
  ...
  if (scoreH > scoreA && scoreH - scoreA === 1) {
    return {
      ...
      alternative_angles: [
        {
          tier: 'Aggressive',
          angle: `Next Goal: ${awayName}`,
          rationale: 'Away pressure after early concession',
        },
        {
          tier: 'Moderate',
          angle: `${awayName} +1 Asian Handicap`,
          rationale: 'Safe buffer for away recovery',
        },
        {
          tier: 'Conservative',
          angle: 'Draw No Bet',
          rationale: 'Level scoreline still very probable at min ' + minute,
        },
      ],
    }
  }
  ...
  if (scoreH === 0 && scoreA === 0 && minute >= 5) {
    ...
    return {
      ...
      alternative_angles: [
        {
          tier: 'Aggressive',
          angle: 'Over 0.5 Goals (1st Half)',
          rationale: `Both teams active — xG total ${(xgH + xgA).toFixed(2)}`,
        },
        {
          tier: 'Moderate',
          angle: dom ? `${dom} Win` : 'Draw No Bet',
          rationale: 'Early-stage momentum signal',
        },
...
function bayesianRedistribution(pHome, pDraw, pAway, driftH, driftA, minute, livePressure = 0) {
  ...
  if (driftH > 0.05) {
    ...
    // home odds lengthening — shift away
...
function buildMultiAngleOutput(m, identity, scores, trap, decision) {
  ...
  if (scoreH === 0 && scoreA === 0 && scores.tempo_score > 70) {
    ...
    aggressive = {
      angle: 'Over 2.5 Goals',
      rationale: `High xG total (${xgSum.toFixed(2)}) — both teams creating`,
    }
  } else if (xgH > 0.8 && xgA > 0.6) {
    aggressive = {
      angle: 'BTTS Yes',
      rationale: `Both teams active xG: H${xgH.toFixed(2)}/A${xgA.toFixed(2)}`,
    }
  } else if (minute < 30) {
    aggressive = { angle: 'Over 1.5 Goals', rationale: 'Early phase — goals still expected' }
  } else {
    aggressive = {
      angle: `${domSide} to Score Next`,
      rationale: `Live xG dominance (${domP.toFixed(0)}% win prob)`,
    }
  }
  ...
  if (trap.flagged) {
    moderate = {
      angle: trap.safe_angle || 'Draw No Bet',
      rationale: `Trap detected — ${trap.reason.slice(0, 60)}`,
    }
  } else if (domP >= 55) {
    const handicap = domP >= 65 ? '-1' : '+0 (Draw No Bet)'
    moderate = {
      angle: `${domSide} ${handicap} Asian Handicap`,
      rationale: `${domP.toFixed(0)}% win probability — buffer bet`,
    }
  } else {
  ...
  if (xgSum < 2.0 && minute > 30) {
    conservative = {
      angle: 'Under 2.5 Goals',
      rationale: `Low xG (${xgSum.toFixed(2)}) — match pacing under budget`,
    }
  } else {
    conservative = {
      angle: 'Under 4.5 Goals',
      rationale: 'Wide safety net — high-probability outcome',
    }
  }
...
function marketLiquidityTrust(m) {
  ...
  const aligned =
    (modelFavoursHome && marketFavoursHome) ||
  ...
  const liquidity_level =
    m.liquidity_index > 0.7 ? 'Elite' : driftMag > 0.2 ? 'High' : driftMag > 0.08 ? 'Medium' : 'Low'
...
function classifyMatchIdentity(m, forceType = null) {
  if (forceType) {
    return {
      type: forceType,
      justification: 'Trend Alignment forced match typography due to major score deficit',
    }
  }
  ...
  const isTrapSuspect =
    ...
    (sharpScore < SHARP_MIN_SCORE &&
      Math.max(pHome, pAway) > TRAP_CONSENSUS_THRESHOLD * 100 &&
      Math.max(oddsDropH, oddsDropA) < 0.1)

  if (isTrapSuspect) {
    return {
      type: 'Trap Match',
      justification: `Odds contradiction or over-consensus (H:${pHome}%/A:${pAway}%, sharp_score=${sharpScore})`,
    }
  }
  ...
  if (chaos > 15 || Math.abs(newsScore) > 20 || _scoreFromCritical(m) >= 3) {
    return {
      type: 'High Volatility Match',
      justification: `News chaos=${chaos}, news_score=${newsScore}, critical=${_scoreFromCritical(m)}`,
    }
  }
  ...
  if (xgDiff >= 0.9 && pSpread >= 30) {
    ...
    return {
      type: 'One-sided Domination',
      justification: `${dom} dominates xG (Δ${xgDiff.toFixed(2)}) with ${pSpread.toFixed(0)}% probability spread`,
    }
  }
  ...
  if (xgSum >= 2.8 && xgH >= 1.1 && xgA >= 1.0) {
    return {
      type: 'High Tempo Attacking Match',
      justification: `xG sum=${xgSum.toFixed(2)}, both teams threatening`,
    }
  }
  ...
  if (xgSum <= 1.6) {
    return {
      type: 'Low Tempo Defensive Match',
      justification: `xG sum=${xgSum.toFixed(2)}, defensive structure expected`,
    }
  }

  // — Balanced Tactical (default)
  return {
    type: 'Balanced Tactical Match',
    justification: `No dominant signal — xG=${xgSum.toFixed(2)}, spread=${pSpread.toFixed(0)}%`,
  }
}
...
function buildIntelligenceScores(m) {
  ...
  const tempoScore = Math.min(
    100,
    Math.round((xgH + xgA) * 20 + (bcH + bcA) * 5 + Math.abs(posH - 50) * 0.5)
  )
  ...
  const volatilityIndex = Math.min(
    100,
    Math.round(oddsSpeedH * 3 + oddsSpeedA * 3 + chaos * 2 + _scoreFromCritical(m) * 6)
  )
...
function checkModelTrust(m, identityType) {
  ...
  if (m._is_live) {
    const modelFavoursHome = pHome > pAway && pHome > pDraw
    const is_misaligned =
      (modelFavoursHome && oddsDropH < -0.1) || (!modelFavoursHome && oddsDropH > 0.1)
    const reason = is_misaligned
  ...
  const is_misaligned =
    (modelFavoursHome && oddsMovingAgainstHome) || (!modelFavoursHome && oddsDropH > 5)

  const trust = !(is_obvious && !hasSharpMoney) && !is_misaligned
  const reasons = []
  if (is_obvious && !hasSharpMoney)
    reasons.push(
      `⚠️ Prediction is too obvious (${topProb.toFixed(1)}%) with no sharp money corroboration`
    )
...
function detectTrap(m, scores, trust) {
  ...
  if (topProb > TRAP_CONSENSUS_THRESHOLD * 100 && sharpScore < SHARP_MIN_SCORE) {
    reasons.push(
      `Over-consensus (${topProb.toFixed(1)}%) without sharp money signal (score=${sharpScore})`
    )
  }
  // Rule 3: Volatility/Risk mismatch
  if (volatility_index > VOLATILITY_HIGH && risk_score < VOLATILITY_LOW) {
    reasons.push(
      `High volatility index (${volatility_index}) with low risk score (${risk_score}) — possible manipulation`
    )
  }
  ...
  if (flagged) {
    ...
    const xgA = parseFloat(m.away_xg || 0)
    safe_angle =
      xgH + xgA > 2.4 ? 'Over 2.5 Goals' : xgH > 1.0 && xgA > 0.8 ? 'BTTS Yes' : 'Under 2.5 Goals'
  }
...
function buildFPISOutput(m, identity, scores, trust, trap, antiRep, decision, liveExtras = null) {
  ...
  const eliteBoost =
    identity.type === 'One-sided Domination' || liveExtras?.value_signal?.signal !== 'NEUTRAL'
      ? 15
      : 0
  ...
  const liquidBoost =
    liveExtras?.liquidity_level === 'High' ? 10 : liveExtras?.liquidity_level === 'Medium' ? 5 : 0
...
function _buildHiddenInsight(m, identity, scores, trust, trap) {
  ...
  if (xgH > 0.3 && xgA > 0.3) {
    ...
    if (Math.abs(xgDiff) > 0.6) {
      insights.push(
        `xG gap of ${Math.abs(xgDiff).toFixed(2)} favours ${xgDiff > 0 ? m.homeTeam || 'Home' : m.awayTeam || 'Away'} more than odds suggest`
      )
    }
  }
  ...
  if (sharpScore >= 40) {
    insights.push(
      `Strong institutional money signal (sharp_score=${sharpScore}) — professionals disagree with public`
    )
  }
  ...
  if (m.master_v20?.is_pattern) {
    insights.push(
      `V20 Master Pattern matched (${m.master_v20.master_verdict}) — historical profile aligned`
    )
  }
  ...
  if (scores.motivation_score > 70) {
    insights.push(
      `High motivation index (${scores.motivation_score}) — expect high-intensity pressing`
    )
  }

  return insights.length > 0
    ? insights[0]
    : 'No non-obvious insight detected — standard model output is reliable'
}
...
function _safeFallback(isLive = false, errMsg = '') {
  return {
    ...
    alternative_angle: isLive
      ...
      : 'Under 1.5 Goals',
    hidden_insight: isLive
      ? 'Live data not yet available — predictions pending'
      : 'Engine error — fallback applied',
    risk_level: 'High',
...
async function process(m) {
  try {
    ...
    const lineupImpact = { home: 0, away: 0, missing: [], rotationH: null, rotationA: null }
    if (m.id) {
      ...
      if (deficit && deficit.isFetched) {
        ...
        const adjustedProbs = squadRotation.adjustProbability(
          {
            pHome: m.home_win_probability,
            pDraw: m.draw_probability,
            pAway: m.away_win_probability,
          },
          rotH,
          rotA
        )
...
async function processLive(rawLiveMatch) {
  try {
    ...
    if (earlyHeuristic?.override) {
      ...
      const earlyIdentity = {
        type: 'Early-Phase Match',
        justification: earlyHeuristic.adjustment_note,
      }
      const earlyScores = buildIntelligenceScores(m)
      const earlyTrust = marketLiquidityTrust(m)
      const earlyTrap = {
        flagged: false,
        reason: 'Early phase — trap detection bypassed',
        safe_angle: null,
      }
      const earlyAntiRep = { forced_alternative: false, alternative_type: null }
      const earlyDecision = {
        finalPrediction: earlyHeuristic.adjusted_prediction,
        adjustmentNote: earlyHeuristic.adjustment_note,
      }
      const liveExtras = {
...
function recordResult(matchId, actualResult) {
  try {
    ...
    const matched =
      (actual.includes('home') && predicted.includes('home')) ||
...
export = { process, processLive, recordResult }
