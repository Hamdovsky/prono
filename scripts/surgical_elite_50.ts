...
import IntegrityService from '../services/integrity_service'
...
if (!BOT_TOKEN || !CHAT_ID) {
  console.warn(
    '[ELITE50] Telegram credentials not configured. Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID in .env'
  )
}
...
function analyzeMotivation(m) {
  ...
  try {
    const hc = m.historical_context
      ? typeof m.historical_context === 'string'
        ? JSON.parse(m.historical_context)
  ...
  if (
    homeAssess.risk !== 'SAFE' &&
    awayAssess.risk !== 'SAFE' &&
    homeAssess.risk !== 'UNKNOWN' &&
    awayAssess.risk !== 'UNKNOWN'
  ) {
...
function selectSurgicalMarket(
  ...
  if (pHO25 >= 52) {
    markets.push({
      type: '1_O25',
      prob: pHO25,
      label: `🛡️ فوز ${homeTeam} & +2.5 أهداف (1 & Over 2.5)`,
      arLabel: `1 & +2.5`,
    })
  }
  if (pAO25 >= 52) {
    markets.push({
      type: '2_O25',
      prob: pAO25,
      label: `🛡️ فوز ${awayTeam} & +2.5 أهداف (2 & Over 2.5)`,
      arLabel: `2 & +2.5`,
    })
  }
  if (pGGO25 >= 56) {
    markets.push({
      type: 'GG_O25',
      prob: pGGO25,
      label: `🔥 كلا الفريقين يسجل & +2.5 أهداف (GG & Over 2.5)`,
      arLabel: `GG & +2.5`,
    })
  }
  ...
  if (pH >= 65) {
    markets.push({
      type: 'H_Handicap',
      prob: pH - 15,
      label: `🛡️ هانديكاب ${homeTeam} (-1)`,
      arLabel: `Handicap ${homeTeam} (-1)`,
    })
  }
  if (pA >= 65) {
    markets.push({
      type: 'A_Handicap',
      prob: pA - 15,
      label: `🛡️ هانديكاب ${awayTeam} (-1)`,
      arLabel: `Handicap ${awayTeam} (-1)`,
    })
  }
  ...
  if (pH > pA && pH >= 52 && pOU25 >= 52) {
    markets.push({
      type: '1X_O15',
      prob: Math.min(85, pH + 5),
      label: `🛡️ فوز ${homeTeam} أو تعادل (1X) & أكثر من 1.5 هدف`,
      arLabel: `1X & +1.5`,
    })
  }
  if (pA > pH && pA >= 52 && pOU25 >= 52) {
    markets.push({
      type: 'X2_O15',
      prob: Math.min(85, pA + 5),
      label: `🛡️ فوز ${awayTeam} أو تعادل (X2) & أكثر من 1.5 هدف`,
      arLabel: `X2 & +1.5`,
    })
  }
  ...
  if (maxProb >= 52) {
    markets.push({
      type: '1X2',
      prob: maxProb,
      label: pH > pA ? `🏠 فوز ${homeTeam}` : `✈️ فوز ${awayTeam}`,
      arLabel: pH > pA ? `فوز ${homeTeam}` : `فوز ${awayTeam}`,
    })
  }
  if (pBTTS >= 55) {
    markets.push({
  ...
  if (pOU25 >= 60) {
    markets.push({
      type: 'Over2.5',
      prob: pOU25,
      label: `🔥 أكثر من 2.5 هدف (Over 2.5)`,
      arLabel: `Over 2.5`,
    })
  }
  ...
  markets.forEach((m) => {
    m.prob = Math.min(99, Math.max(0, m.prob))
  })
  markets.sort((a, b) => b.prob - a.prob)

  const best = markets[0]
  const backup =
    markets.find((m) => m.type !== best.type && !m.type.includes(best.type.split('_')[0])) ||
    markets[1]
...
Le scraper collecte les données - relancez dans 10 min.`,
    ...
    for (const m of rows) {
      ...
      let homeStats = {},
        awayStats = {}
      try {
        const ts = typeof m.teamStats === 'string' ? JSON.parse(m.teamStats) : m.teamStats || {}
        homeStats = ts.home || {}
        awayStats = ts.away || {}
      } catch (_) {}
      ...
      let defXGH = 1.3,
        defXGA = 1.0
      if (league.includes('iceland') || league.includes('reykjavik')) {
        defXGH = 1.9
        defXGA = 1.6
      } else if (league.includes('bundesliga') || league.includes('netherlands')) {
        defXGH = 1.8
        defXGA = 1.4
      } else if (league.includes('norway') || league.includes('sweden')) {
        defXGH = 1.6
        defXGA = 1.3
      }
      ...
      const { best, backup } = selectSurgicalMarket(
        pH,
        pD,
        pA,
        pOU,
      ...
      // Integrity check
      const modelPreds = {
        home_win_probability: pH / 100,
        draw_probability: pD / 100,
        away_win_probability: pA / 100,
      }
      ...
      if (pH + pD + pA >= 30) {
        ...
        if (divergence > 0.25)
          confluenceFactor = 0.55 // désaccord critique
        else if (divergence > 0.15)
          confluenceFactor = 0.78 // désaccord modéré
      ...
      if (hXG_stored === 0 && aXG_stored === 0) {
        dataQualityFactor = 0.6 // Pas de données xG — incertitude haute
      }
      ...
      const motivFactor =
        motiv.verdict === 'SKIP'
          ? 0.0
          : motiv.verdict === 'HIGH_RISK'
            ? 0.35
            : motiv.verdict === 'CAUTION'
              ? 0.7
              : 1.0
      const qualityScore =
        best.prob * integrityFactor * motivFactor * confluenceFactor * dataQualityFactor
    ...
    console.log(
      `🧠 [MOTIVATION] SKIP: ${skipCount} | DANGER: ${dangerCount} | CAUTION: ${cautionCount} | SAFE: ${scored.length - skipCount - dangerCount - cautionCount}`
    )
    ...
    console.log(
      `✅ ${filtered.length} matches passed quality filters (motivation + integrity + confluence)`
    )
    ...
    top50.forEach((s, i) => {
      const date = s.m.startTimestamp
        ? new Date(s.m.startTimestamp * 1000).toLocaleString('fr-FR', {
            day: '2-digit',
            month: '2-digit',
            hour: '2-digit',
            minute: '2-digit',
      ...
      console.log(
        `#${String(i + 1).padStart(2, '0')} ${icon} ${s.m.homeTeam} vs ${s.m.awayTeam} [${date}]`
      )
      console.log(
        `    ✂️  ${s.best.label} (${s.confidence}%) | 🔄 ${s.backup.label} (${Math.round(s.backup.prob)}%)`
      )
...
if (require.main === module) {
  runSurgicalElite50(true)
    .then(() => process.exit(0))
    .catch(() => process.exit(1))
}

export = { runSurgicalElite50 }
