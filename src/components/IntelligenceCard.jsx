...
const IntelligenceCard = ({ match, onOpenUltimate }) => {
  ...
  try {
    integrityAudit =
      typeof match.integrity_audit === 'string'
        ? JSON.parse(match.integrity_audit)
        : match.integrity_audit || {}
  } catch (e) {}
  ...
  const oddsH =
    match.display_odds_home ||
    match.best_odds_home ||
    match.odds_home ||
    match.market_odds ||
    match.odds?.home
  const oddsHOpen = match.odds_home_open || match.odds?.home_open
  const dropPct =
    oddsHOpen && oddsH && oddsH < oddsHOpen
      ? Math.floor(((oddsHOpen - oddsH) / oddsHOpen) * 100)
      : 0

  const displayTime = () => {
    if (match.status === 'FT' || match.status === 'FINISHED') return 'FINISHED'
    if (match.startTimestamp) {
      return new Date(match.startTimestamp * 1000).toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit',
      })
    }
    ...
    <div className={`titanium-card ${isGolden ? 'golden-pick' : ''}`}>
            ...
            <div className="val-row">
              <span className="val">
                {match.v22_success_rate ||
                  enriched.v22_success_rate ||
                  (match.power_score ? Math.round(match.power_score / 1.2 + 20) : '--')}
                %
              </span>
              {(match.smart_money_active || enriched.smart_money_active) && (
                <span className="smart-money-icon">💰</span>
              )}
            </div>
          </div>
          {match.power_score && (
      ...
      <div className="titanium-duel">
        <div className="team home">
          <span className="name">
            {match.momentum?.home > 1.05 && (
              <span title="Hot Streak" style={{ marginRight: '4px' }}>
                🔥
              </span>
            )}
            {match.momentum?.home < 0.95 && (
              <span title="Crisis" style={{ marginRight: '4px' }}>
                ❄️
              </span>
            )}
            {match.homeTeam}
        ...
        <div className="score-display">
          ...
          {(() => {
            ...
            if (isLive || isFt) {
              return (
                <>
                  <span className="val">{match.homeScore ?? '-'}</span>
                  <span className="sep">:</span>
                  <span className="val">{match.awayScore ?? '-'}</span>
                </>
              )
            }
            ...
            return (
              <>
                <span className="val-analyzing" style={{ fontSize: '0.8rem', color: '#6366f1' }}>
                  ANALYZING...
                </span>
              </>
            )
        ...
        <div className="team away">
          <span className="name">
            {match.awayTeam}
            {match.momentum?.away > 1.05 && (
              <span title="Hot Streak" style={{ marginLeft: '4px' }}>
                🔥
              </span>
            )}
            {match.momentum?.away < 0.95 && (
              <span title="Crisis" style={{ marginLeft: '4px' }}>
                ❄️
              </span>
            )}
          </span>
      ...
      <div className="titanium-metadata">
        ...
        {enriched.bankroll_advice ? (
          <div
            className="recommended-stake bankroll-v90"
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}
          >
            <span className="label" style={{ color: '#fbbf24' }}>
              BANKROLL V90 (Kelly)
            </span>
            <div
              className="val"
              style={{
                fontSize: '1.2rem',
                fontWeight: 'bold',
                color: enriched.bankroll_advice.recommendedPercentage > 2.5 ? '#10b981' : '#f59e0b',
              }}
            >
          ...
          <div className="recommended-stake">
            ...
            <div className="stake-dots">
              {[...Array(10)].map((_, i) => (
                <span
                  key={i}
                  className={`stake-dot ${i < recommendedStake ? 'active' : ''}`}
                ></span>
              ))}
      ...
      <div className="titanium-stats-grid v22-grid">
        {mainPredictions
          .filter((p) => p.label !== 'Both Teams to Score')
          .map((p, i) => (
            <div key={i} className="stat-slot">
              <label>{p.label === 'Goals Prediction' ? 'TOTAL GOALS PREDICTED' : p.label}</label>
              <span className="value">
                {p.label === 'Goals Prediction'
                  ? match.total_goals_label || enriched.total_goals_label || p.val
                  : p.val}
              </span>
            </div>
          ))}
          ...
          <div className="v70-metrics">
            ...
            {v70.odds_velocity && Math.abs(v70.odds_velocity.diffPct) >= 5 && (
              <div className="v70-badge velocity">
                ...
                <span>
                  {v70.odds_velocity.trend} ({v70.odds_velocity.diffPct > 0 ? '+' : ''}
                  {v70.odds_velocity.diffPct}%)
                </span>
              </div>
            )}
          </div>
          ...
          {v70.xg_regression && (
            <div className="v70-xg-matrix">
              <div className="xg-row">
                ...
                <span className="xg-badge">
                  {v70.xg_regression.home?.badge} {v70.xg_regression.home?.status}
                </span>
              </div>
              <div className="xg-row">
                ...
                <span className="xg-badge">
                  {v70.xg_regression.away?.badge} {v70.xg_regression.away?.status}
                </span>
              </div>
            </div>
          )}
