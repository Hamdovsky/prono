...
import DeepFormService from './DeepFormService'
import LogisticsService from './LogisticsService'
import MotivationEnrichService from './MotivationEnrichService'
import SharpIntelligenceService from './SharpIntelligenceService'
import bTeamDetector from './bTeamDetector'
import promosportSurpriseService from './promosportSurpriseService'
import tacticalContextEngine from './tacticalContextEngine'
import probabilityCalibrator from './probabilityCalibrator'
import competitionAnalyzer from './competitionAnalyzer'
import crowdHackerService from './crowdHackerService'
...
class PromosportIntelligence {
  ...
  async optimizeGrid(matches, strategy = 'balanced') {
    ...
    const enrichedMatches = await Promise.all(
      matches.map(async (m) => {
        ...
        const logistics = LogisticsService.calculateFatigue(
          m.awayCity,
          m.homeCity,
          m.daysRestA || 4
        )
        ...
        const H = -(
          p1 * Math.log2(p1 || 0.01) +
          px * Math.log2(px || 0.01) +
          p2 * Math.log2(p2 || 0.01)
        )
  ...
  async generateSecretWeapons(matches) {
    ...
    const weapons = matches.map((m, idx) => {
      ...
      const realFav = p1Cal > 0.45 ? '1' : p2Cal > 0.4 ? '2' : 'X'
      ...
      const competitionIntel = competitionAnalyzer.getMatchIntel(
        homeName,
        awayName,
        idx + 1,
        m.leagueName
      ...
      if (crowdSignal) {
        crowdAnalysis.push(
          `Promosport: ${crowdSignal.promosportPick} (${crowdSignal.promosportAccuracy}%)`
        )
        ...
        if (crowdSignal.historicalEdge) {
          crowdAnalysis.push(
            `📊 Contrariant: ${crowdSignal.modelVersusPromosport.disagreeAccuracy}% de réussite`
          )
        }
      ...
      if (contrarianStrength.isContrarian) {
        weaponParts.push(
          `🎯 Contrarian: foule→${crowdFav}, nous→${realFav} (${contrarianStrength.label})`
        )
      }
  ...
  getGridOptimizationHints(matches, weapons) {
    ...
    const sortedByContrarian = [...weapons].sort(
      (a, b) => b.contrarianStrength.score - a.contrarianStrength.score
    )
    const bestContrarian = sortedByContrarian
      .filter((w) => w.contrarianStrength.isContrarian)
      .slice(0, 5)
      .map((w) => w.id)
    ...
    const doubleCandidates = sortedByEntropy.slice(0, 5).map((w) => ({
      ...
      reason:
        w.bTeamHome?.isBTeam || w.bTeamAway?.isBTeam
          ? 'B-Team incertaine'
          : w.isDeadRubber
            ? 'Dead rubber'
            : 'Entropie élevée (incertain)',
    }))

    const safePicks = weapons
      .filter(
        (w) => !w.isContrarian && !w.bTeamHome?.isBTeam && !w.bTeamAway?.isBTeam && !w.isDeadRubber
      )
    ...
    return {
      doubleCandidates,
      bestContrarian: bestContrarian.map((id) => ({
        id,
        match: weapons.find((w) => w.id === id)
          ? `${weapons.find((w) => w.id === id).home} vs ${weapons.find((w) => w.id === id).away}`
          : '',
      })),
      bestEdge: bestEdge.map((id) => ({
        id,
        match: weapons.find((w) => w.id === id)
          ? `${weapons.find((w) => w.id === id).home} vs ${weapons.find((w) => w.id === id).away}`
          : '',
  ...
  async generateLLMSecretWeapons(matches) {
    ...
    const matchData = matches.map((m, idx) => {
      ...
      return {
        ...
        bTeam:
          rotation.home.isBTeam || rotation.away.isBTeam
            ? `B-Team: ${rotation.home.reason || ''} ${rotation.away.reason || ''}`.trim()
            : null,
        homeHisto: homeStats.team
          ? `${homeStats.team.homeWinRate}%V ${homeStats.team.homeDrawRate}%N ${homeStats.team.homeLossRate}%D`
          : null,
        awayHisto: awayStats.team
          ? `${awayStats.team.awayWinRate}%V ${awayStats.team.awayDrawRate}%N ${awayStats.team.awayLossRate}%D`
          : null,
      }
    })
...
export = new PromosportIntelligence()
