...
function ROIBadge({ roi }) {
  ...
  return (
    <span className="ct-roi" style={{ color }}>
      {roi > 0 ? '+' : ''}
      {roi?.toFixed(1)}%
    </span>
  )
  ...
  return (
    <div className="ct-dashboard">
      ...
      <div className="ct-header">
        ...
        <button className="ct-refresh-btn" onClick={fetchAll}>
          🔄 Rafraîchir
        </button>
      </div>
      ...
      <div className="ct-week-stats">
        <div className="ct-stat-card">
          <div className="ct-stat-val" style={{ color: '#6c63ff' }}>
            {totalEntry}
          </div>
          <div className="ct-stat-label">Combos (7j)</div>
        </div>
        <div className="ct-stat-card">
          <div className="ct-stat-val" style={{ color: '#2ecc71' }}>
            {wonCombos}
          </div>
      ...
      <div className="ct-tabs">
        <button
          className={`ct-tab ${tab === 'today' ? 'active' : ''}`}
          onClick={() => setTab('today')}
        >
          📅 Aujourd'hui ({todayCombos.length})
        </button>
        <button
          className={`ct-tab ${tab === 'history' ? 'active' : ''}`}
          ...
          {todayCombos.length === 0 ? (
            <div className="ct-empty">
              ...
              <p className="ct-empty-sub">
                Les combos apparaissent automatiquement après le scraping.
              </p>
            </div>
          ) : (
            todayCombos.map((combo, i) => <ComboCard key={i} combo={combo} />)
          )}
        </div>
      )}
      ...
      {tab === 'history' && (
        <div className="ct-combos-list">
          {histCombos.length === 0 ? (
            ...
            histCombos.map((combo, i) => <ComboCard key={i} combo={combo} showDate />)
          )}
        </div>
      )}
    </div>
    ...
    <div className="ct-combo-card" style={{ borderTopColor: color }}>
      <div className="ct-combo-top" onClick={() => setExpanded((e) => !e)}>
        <div className="ct-combo-left">
          ...
          <span className="ct-odds">
            📊 Cote: <strong>{combo.totalOdds?.toFixed(2) || '–'}</strong>
          </span>
        </div>
