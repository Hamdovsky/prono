  ...
  static removeMarginShin(odds) {
    ...
    for (let i = 1; i < 100; i++) {
      ...
      for (const p of impliedProbs) {
        const term1 = Math.pow(currentZ, 2)
        const term2 = 4 * (1 - currentZ) * (Math.pow(p, 2) / (margin + 1))

        if (term1 + term2 >= 0) {
          const calcP = (Math.sqrt(term1 + term2) - currentZ) / (2 * (1 - currentZ))
          sumP += calcP
