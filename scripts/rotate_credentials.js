...
async function main() {
  ...
  for (const [name, key] of Object.entries({
    ...
    try {
      const res = await axios.post(
        'https://api.render.com/v1/user/api-keys/rotate',
        {},
        {
  ...
  try {
    const res = await axios.post(
      `https://console.neon.tech/api/v2/projects/${NEON_PROJECT_ID}/actions/reset_password`,
      {},
      {
  ...
  console.log(
    '  2. Render Account 2: https://dashboard.render.com/u/api-keys (déconnecter/reconnecter)'
  )
  console.log('  3. Neon: https://console.neon.tech → Reset password')
  console.log('  4. Upstash: https://console.upstash.com → Reset password')
  console.log(
    '  5. Mettre à jour API_SECRET_KEY sur TOUS les services Render (Dashboard → Environment)'
  )
