...
function analyzeMotivation(m) {
  ...
  if (sigUpper.includes('ZONE MORTE')) {
  ...
  if (sigUpper.includes('RELEGUE')) {
    result.signatures.push('RELEGUE_DB')
    result.riskScore = Math.max(result.riskScore, 70)
  }
  if (sigUpper.includes('ENJEU CRITIQUE')) {
  ...
  try {
    ...
    const reports = matches.map((m) => {
      ...
      const markets = [
        ...
        {
          type: 'AH',
          prob: Math.max(pH, pA) * 0.9,
          label: pH > pA ? `AH -0.5 ${m.homeTeam}` : `AH -0.5 ${m.awayTeam}`,
        },
        {
          type: 'DNB',
          prob: Math.max(dnbH, dnbA),
          label: dnbH > dnbA ? `DNB ${m.homeTeam}` : `DNB ${m.awayTeam}`,
        },
    ...
    const filteredReports = reports.filter(
      (r) => r !== null && (r.motiv.verdict !== 'SKIP' || r.isPromosport)
    )
    console.log(
      `🧠 Motivation Filter: Supprimé ${reports.length - filteredReports.length} matchs sans enjeu (Promosport préservé).`
    )

    // 3. Separate Millionaire Selection (Surgical Strikes)
    const millionairePicks = filteredReports
      .filter((r) => (r.verdict && r.verdict.includes('SURGICAL')) || r.confidence >= 88)
      .slice(0, 3)
    ...
    for (let i = 0; i < filteredReports.length; i += chunkSize) {
      ...
      chunk.forEach((r) => {
        if (r.isPromosport) {
          ...
          const tunisiaTime = matchDate.toLocaleTimeString('fr-TN', {
            hour: '2-digit',
            minute: '2-digit',
