    ...
    <div className="performance-audit-container" dir="rtl">
      <header className="audit-header">
        ...
        <div className="header-text">
          <h1>
            تدقيق أداء الذكاء الاصطناعي <span>Alpha V40</span>
          </h1>
          ...
          <div className="stat-bars">
            <div className="stat-bar-item">
              ...
              <div className="bar-bg">
                <div className="bar-fill" style={{ width: `${global.accuracy}%` }}></div>
              </div>
            </div>
          ...
          <div className="timing-list">
            {Object.values(timing).map((t, i) => (
              <div className="timing-item" key={i}>
                ...
                <span className="t-acc" style={{ color: t.accuracy > 70 ? '#10b981' : '#f59e0b' }}>
                  {t.accuracy}%
                </span>
              </div>
            ))}
        ...
        <section className="audit-card league-dna">
          ...
          <div className="dna-split">
            <div className="dna-col">
              <h4>🏆 الأفضل أداءً</h4>
              {leagues.best.map((l, i) => (
                <div key={i} className="dna-rank">
                  ⭐ {l.league} ({Math.round((l.correct / l.total) * 100)}%)
                </div>
              ))}
            </div>
            <div className="dna-col">
              <h4>⚠️ الأضعف أداءً</h4>
              {leagues.worst.map((l, i) => (
                <div key={i} className="dna-rank red">
                  🚩 {l.league} ({Math.round((l.correct / l.total) * 100)}%)
                </div>
              ))}
            </div>
          ...
          <div className="pattern-list">
            {errors.map((e, i) => (
              <div className="pattern-item" key={i}>
                ...
                <span className={`p-freq ${e.frequency === 'مرتفع' ? 'high' : ''}`}>
                  {e.frequency}
                </span>
              </div>
            ))}
