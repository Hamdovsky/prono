      ...
      draws: candidates.map((m) => ({
        ...
        time: m.timestamp
          ? isNaN(m.timestamp)
            ...
            : new Date(
                parseInt(m.timestamp) > 1e11 ? parseInt(m.timestamp) : parseInt(m.timestamp) * 1000
              ).toISOString()
          : null,
...
router.get('/high-scoring', async (req, res) => {
  try {
    ...
    const sortedCandidates = candidates.sort((a, b) => {
      ...
      const tierA =
        a.league_tier || Object.values(LEAGUE_MAP).find((l) => l.name === a.league)?.tier || 'TIER3'
      const tierB =
        b.league_tier || Object.values(LEAGUE_MAP).find((l) => l.name === b.league)?.tier || 'TIER3'
    ...
    for (const m of subset) {
      try {
        const hasExistingPred =
          (m.ou_2_5_prob && m.ou_2_5_prob > 0) || (m.ou_25_prob && m.ou_25_prob > 0)
        if (!hasExistingPred) {
    ...
    const picks = enriched
      .filter((m) => {
        ...
        const hasGoodOdds = !bestOdds || (bestOdds >= 1.2 && bestOdds <= 3.5)
...
router.get('/golden-coupon', async (req, res) => {
  try {
    ...
    const rankedCandidates = candidates
      .map((m) => {
        const leagueConfig =
          LEAGUE_MAP[m.league_id] ||
          Object.values(LEAGUE_MAP).find((l) => l.name === m.league) ||
          {}
        const tierWeight = { ELITE: 120, TIER1: 80, TIER2: 40 }[leagueConfig.tier] || 10
        const baseQuality = m.xgboost_confidence
          ? m.xgboost_confidence * 100
          : m.v22_success_rate || 50
    ...
    const enriched = await Promise.all(
      rankedCandidates.map((m) => enrichedPredictions.fastEnrichMatch(m))
    )
