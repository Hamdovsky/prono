  ...
  async calibrate(match, baseConfidence) {
    ...
    try {
      ...
      if (riskFactor > 1.0) {
        ...
        penalties.push({
          factor: 'evolution_risk',
          value: -((riskFactor - 1) * 100).toFixed(1) + '%',
        })
      }
    } catch (e) {
    ...
    if (leagueStability < 1.0) {
      penalties.push({
        factor: 'league_volatility',
        value: -((1 - leagueStability) * 100).toFixed(1) + '%',
      })
    }
    ...
    if (match.data_age_hours && match.data_age_hours > 48) {
      ...
      penalties.push({
        factor: 'stale_data',
        value: '-' + ((1 - freshnessPenalty) * 100).toFixed(1) + '%',
      })
    }
...
export = new ConfidenceCalibrationEngine()
