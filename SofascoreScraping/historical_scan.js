...
async function runHistorical() {
  ...
  while (currentDate <= todayStr) {
    ...
    console.log(
      `🎯 [FILTER] Found ${targetMatches.length} FINISHED matches to process for ${currentDate}.`
    )
    ...
    const processMatch = async (match) => {
      try {
        ...
        const [homeStats, awayStats, eventDetails, h2h, homeForm, awayForm, standings, matchStats] =
          await Promise.all([
        ...
        if (done % 10 === 0) {
          process.stdout.write(
            `\r  ⚙️  Progress for ${currentDate}: ${done}/${total} (${Math.round((done / total) * 100)}%) `
          )
        }
      }
    }
