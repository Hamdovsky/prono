  ...
  const matchLog = [
    {
      ...
      message: `DANGEROUS ATTACK DETECTED - ${(stats?.pressure?.home || 0) > (stats?.pressure?.away || 0) ? 'HOME' : 'AWAY'}`,
      type: 'danger',
    },
    {
      time: new Date().toLocaleTimeString(),
      message: `ANALYST CLASSIFICATION: ${analysis?.tagLabel || 'SYNCING...'}`,
      type: 'elite',
    },
    {
      time: new Date().toLocaleTimeString(),
      message: 'RECALCULATING MOMENTUM (0.42ms)',
      type: 'info',
    },
  ]
    ...
    <div className="ai-analysis-container" id="ai-analysis-capture">
      ...
      <section className="match-card-compact">
        ...
        <div className="score-area">
          <div className="time">
            {minute}' <span className="material-symbols-outlined">timer</span>
          </div>
          <div className="score">
            {score?.home ? Math.round(parseFloat(score.home)) : 0} -{' '}
            {score?.away ? Math.round(parseFloat(score.away)) : 0}
          </div>
        </div>
      ...
      <div className="war-rings-grid">
        <RingStat
          label="GOALS WAR"
          val={stats?.pressure?.home || 0}
          icon="local_fire_department"
          color="#22c55e"
        />
        <RingStat
          label="CORNERS WAR"
          val={(stats?.corners?.home || 0) * 10}
          icon="flag"
          color="#3b82f6"
        />
        <RingStat
          label="CARDS WAR"
          val={(stats?.cards?.home || 0) * 20}
          icon="style"
          color="#ef4444"
        />
      ...
      <div className="analyst-report-box">
        ...
        <div className="report-content">
          <div className="report-item">
            ...
            <span
              className={`report-value tag-badge ${analysis?.tagLabel?.includes('BANKER') ? 'banker' : analysis?.tagLabel?.includes('TREND') ? 'trend' : 'value'}`}
            >
              {analysis?.tagLabel}
            </span>
          </div>
