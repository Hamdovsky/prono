...
import liveLabService from './liveLabService'

class BacktestService {
  async runHistoricalBacktest(limit = 500, period = 'all') {
    try {
      console.log(
        `🧪 [BACKTEST] Analyzing archived matches (Limit: ${limit}, Period: ${period})...`
      )
      ...
      for (const m of matches) {
        ...
        try {
          ...
          pronos.forEach((p) => {
            ...
            const is1X2 =
              label.includes('Winner') || label.includes('1X2') || label.includes('Choix')
            const isGoals =
              label.includes('Goals') || label.includes('الأهداف') || label.includes('O/U')

            if (is1X2) {
              const isHomePred =
                val.includes(m.homeTeam) || val.includes('🏠') || val.includes('domicile')
              const isAwayPred =
                val.includes(m.awayTeam) || val.includes('✈️') || val.includes('extérieur')
            ...
            const isElite =
              (p.confidence && p.confidence >= 80) ||
              val.includes('🔥') ||
              fullData.verdict === 'SAFE BET'
            if (isElite) {
              eliteSignals++
            ...
            if (isCorrect) simulatedROI += 0.85
            else simulatedROI -= 1
          })
        } catch (e) {
