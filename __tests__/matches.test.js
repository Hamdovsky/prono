...
jest.mock('../core/enriched_predictions', () => ({
  ...
  fastEnrichMatch: jest.fn((match) => Promise.resolve({
    ...match,
...
describe('Matches API Routes', () => {
  describe('GET /api/matches/upcoming', () => {
    it('should return upcoming matches array', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        { id: 'match-1', homeTeam: 'Barcelona', awayTeam: 'Real Madrid', league: 'La Liga', startTimestamp: Math.floor(Date.now() / 1000) + 86400, odds_home: 1.95, odds_draw: 3.40, odds_away: 3.80 }
      ])
    ...
    it('should filter out reserve/youth teams', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        { id: '1', homeTeam: 'Barcelona II', awayTeam: 'Real Madrid', league: 'Test', startTimestamp: Math.floor(Date.now() / 1000) + 86400, odds_home: 2.0, odds_away: 3.0 },
        { id: '2', homeTeam: 'Bayern Munich', awayTeam: 'Dortmund II', league: 'Test', startTimestamp: Math.floor(Date.now() / 1000) + 86400, odds_home: 2.0, odds_draw: 3.0, odds_away: 3.0 },
        { id: '3', homeTeam: 'Liverpool', awayTeam: 'Chelsea', league: 'Test', startTimestamp: Math.floor(Date.now() / 1000) + 86400, odds_home: 2.0, odds_draw: 3.0, odds_away: 3.0 }
      ])
    ...
    it('should filter out matches with very low odds (< 1.10)', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        { id: 'm1', homeTeam: 'Alpha FC', awayTeam: 'Beta United', odds_home: 1.05, odds_draw: 8.0, odds_away: 10.0, startTimestamp: Math.floor(Date.now() / 1000) + 86400 },
        { id: 'm2', homeTeam: 'Gamma City', awayTeam: 'Delta Rovers', odds_home: 1.85, odds_draw: 3.40, odds_away: 3.60, startTimestamp: Math.floor(Date.now() / 1000) + 86400 }
      ])
    ...
    it('should deduplicate matches by team pair', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        { id: 'dup-a', homeTeam: 'Alpha FC', awayTeam: 'Beta United', league: 'Test', startTimestamp: Math.floor(Date.now() / 1000) + 86400, odds_home: 2.0, odds_away: 3.0 },
        { id: 'dup-b', homeTeam: 'Alpha FC', awayTeam: 'Beta United', league: 'Test', startTimestamp: Math.floor(Date.now() / 1000) + 86400, odds_home: 2.0, odds_away: 3.0 }
      ])
    ...
    it('should apply date window filter', async () => {
      ...
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        { id: 'old', startTimestamp: nowSec - 86400, homeTeam: 'Old', awayTeam: 'Match', league: 'Test', odds_home: 2.0, odds_away: 3.0 },
        { id: 'future', startTimestamp: nowSec + 7 * 86400, homeTeam: 'Far Future', awayTeam: 'Match', league: 'Test', odds_home: 2.0, odds_away: 3.0 },
        { id: 'valid', startTimestamp: nowSec + 3600, homeTeam: 'Current', awayTeam: 'Match', league: 'Test', odds_home: 2.0, odds_away: 3.0 }
      ])
    ...
    it('should enrich matches without predictions via fastEnrichMatch', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        { id: 'unenriched', homeTeam: 'Team X', awayTeam: 'Team Y', league: 'Test', odds_home: 1.85, odds_draw: 3.40, odds_away: 4.20, startTimestamp: Math.floor(Date.now() / 1000) + 86400 }
      ])
  ...
  describe('GET /api/matches/odds/steam/:matchId', () => {
    it('should return steam odds for a match', async () => {
      const mockSteam = { homeOdds: 1.90, drawOdds: 3.50, awayOdds: 3.90, steam_detected: true }
  ...
  describe('GET /api/matches/market/edge', () => {
    it('should return value bets with edge opportunities', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        {
          id: 'value-1', homeTeam: 'Alpha FC', awayTeam: 'Beta United', league: 'Test League',
          home_win_probability: 60, draw_probability: 22, away_win_probability: 18,
      ...
      IntegrityService.analyzeMatch.mockResolvedValue({
        score: 85, trafficLight: 'GREEN', recommendation: 'Clear', strategicTags: ['high-value']
      })
    ...
    it('should return empty array when no value bets found', async () => {
      jest.spyOn(database, 'getMatchesByStatuses').mockResolvedValue([
        {
          id: 'no-value', homeTeam: 'Alpha FC', awayTeam: 'Beta United', league: 'Test',
          home_win_probability: 50, odds_home: 2.00,
          source: 'africanobet'
        }
      ])
  ...
  describe('POST /api/matches/refresh-lineups/:id', () => {
    it('should refresh lineups for a match', async () => {
      ...
      jest.spyOn(database, 'getMatchById').mockResolvedValue({ id: matchId, id_sofa: '12345', homeTeam: 'Alpha FC', awayTeam: 'Beta United', startTimestamp: Date.now() })
