...
const statCols = ['teamStats', 'form_context', 'h2h_data', 'stats_blob', 'news_data', 'statistics', 'form', 'h2h', 'stats'];
...
for (const sc of statCols) {
  if (existingCols.includes(sc)) {
    ...
    console.log(`${sc}: ${nonNull.cnt} non-null rows, sample: ${sample ? String(sample[sc]).substring(0, 300) : 'NULL'}`);
  }
}
