...
export function analyzeValue({ modelHome, modelDraw, modelAway, homeOdds, drawOdds, awayOdds }) {
  ...
  const build = (label, mProb, odds, fairP) => {
    if (!odds || odds < 1.3 || !mProb) return null
...
export const determineIdealPick = (openMatch, homeSquadH, awaySquadH, winProb, homeXG, awayXG) => {
  ...
  if (openMatch.isOpen && homeXG > 1.1 && awayXG > 1.1) {
    picks.push({
      pick: 'Over 2.5 + BTTS',
      logic: 'High combined xG and defensive gaps detected on both sides.',
    })
  } else if (!openMatch.isOpen && homeXG + awayXG < 1.9) {
    picks.push({
      pick: 'Under 2.5 Goals',
      logic: 'Low xG totals and tight defensive structures expected.',
    })
  } else if (winProb > 55) {
    picks.push({
      pick: 'Home Draw No Bet',
      logic: 'Defensive safety while backing the favorable home trend.',
    })
  } else if (winProb < 45) {
    picks.push({
      pick: 'Away Draw No Bet',
      logic: 'Securing the away advantage with draw protection.',
    })
  } else {
    picks.push({
      pick: 'Under 3.5 Goals',
      logic: 'Strategic coverage for a tight tactical stalemate.',
    })
  }
  ...
  if (homeSquadH === 100 && awaySquadH < 85 && winProb > 60) {
    picks.push({
      ...
      logic: 'Elite fitness gap favoring the 100% fit home side.',
    })
  } else if (awaySquadH === 100 && homeSquadH < 85 && winProb < 40) {
    picks.push({
      pick: 'Handicap (-1) Away',
      logic: 'Away side at full strength against a depleted home rotation.',
    })
  } else if (winProb > 55) {
    picks.push({
      pick: 'Home win',
      logic: 'Statistical win probability favors a direct home victory.',
    })
  } else if (winProb < 45) {
    picks.push({
      pick: 'Away win',
      logic: 'Model signals high confidence in away performance quality.',
    })
  } else {
    picks.push({
      pick: 'Double Chance 12',
      logic: 'High volatility suggests a decisive outcome for either side.',
    })
  }
  ...
  if (homeXG > 1.5 || awayXG > 1.5) {
    picks.push({
      pick: 'Over 1.5 Team Goals',
      logic: 'High individual team xG suggests consistent scoring potential.',
    })
  } else if (homeSquadH < 90 && awaySquadH < 90) {
    picks.push({
      pick: 'Both Teams to Score',
      logic: 'Mutual defensive vulnerabilities from squad rotation.',
    })
  } else {
    picks.push({
      pick: 'Under 2.5 Goals',
      logic: 'Conservative start expected due to mutual tactical respect.',
    })
  }
