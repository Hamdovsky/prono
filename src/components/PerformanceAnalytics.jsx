...
const shorten = (name) => {
  ...
  return parts.length <= 2 ? name : `${parts[0]} ${parts.slice(1).map(w => w[0]+'.').join(' ')}`;
};
...
const ConfBadge = ({ value }) => {
  ...
  const bg = v >= 75 ? 'rgba(0,255,170,0.12)' : v >= 60 ? 'rgba(56,189,248,0.12)' : 'rgba(248,113,113,0.12)';
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: '5px',
      background: bg, color, borderRadius: '20px', padding: '2px 10px 2px 8px',
      fontWeight: '800', fontSize: '11px', fontFamily: "'JetBrains Mono', monospace",
      letterSpacing: '-0.3px',
    }}>
  ...
  const rows = useMemo(() => {
    return matches
      ...
      .map(m => {
        ...
        if (!pick || ['risky bet', 'skip', 'no bet', 'null', 'undefined', ''].includes(pick)) return null;
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: '#475569', letterSpacing: '0.8px' }}>📊 ANALYTICS</span>
          {stats.total > 0 && (
            <span style={{ fontSize: 11, fontWeight: 800, color: '#94a3b8' }}>
              {stats.wins}W / {stats.losses}L
              <span style={{ color: stats.winRate >= 60 ? '#00ffaa' : stats.winRate >= 50 ? '#fbbf24' : '#f87171', marginLeft: 6 }}>
                ({stats.winRate}%)
              </span>
            </span>
          )}
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {onTrackRecord && (
            <button onClick={onTrackRecord} style={{
              padding: '4px 12px', borderRadius: 6, border: '1px solid rgba(139,92,246,0.25)',
              background: 'rgba(139,92,246,0.08)', color: '#a78bfa',
              fontWeight: 700, fontSize: 9, cursor: 'pointer', letterSpacing: '0.5px',
            }}>📋 HISTORIQUE</button>
          )}
          <button onClick={() => { setForceOpen(true); fetchForce(); }} style={{
            padding: '4px 12px', borderRadius: 6, border: '1px solid rgba(249,115,22,0.25)',
      ...
      {rows.length === 0 && (
        <div style={{
          padding: '20px 0', textAlign: 'center', color: '#334155', fontSize: 11,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {rows.map(r => (
            <div key={r.id} style={{
              ...
              <ResultDot won={r.won} />
              <span style={{
                flex: 1, fontSize: 11, fontWeight: 600, color: '#cbd5e1',
                overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
              }}>
                <span style={{ color: '#64748b', fontWeight: 500 }}>[{r.league}]</span>{' '}
                {r.home} vs {r.away}
              </span>
              <span style={{
                fontWeight: 900, fontSize: 11, color: '#f1f5f9',
      ...
      {forceOpen && (
        <div onClick={() => setForceOpen(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ fontSize: 10, fontWeight: 800, color: '#2d3f56', letterSpacing: '1.5px', marginBottom: 14 }}>
              🔬 FORCE ANALYSIS — RÉSULTATS SERVEUR
            </div>

            {forceLoading && <div style={{ textAlign: 'center', padding: 30, color: '#64748b', fontSize: 12 }}>Chargement…</div>}
            {forceError && <div style={{ textAlign: 'center', padding: 30, color: '#f87171', fontSize: 12 }}>❌ {forceError}</div>}
              ...
              <>
                ...
                <div style={{
                  ...
                  {[
                    ...
                    { label: 'ROI', value: `${forceData.roi_percent >= 0 ? '+' : ''}${forceData.roi_percent}%`, color: forceData.roi_percent >= 0 ? '#00ffaa' : '#f87171' },
                    { label: 'Profit', value: `${forceData.profit_units >= 0 ? '+' : ''}${forceData.profit_units}u`, color: forceData.profit_units >= 0 ? '#00ffaa' : '#f87171' },
                  ].map(k => (
                    <div key={k.label} style={{
                      background: 'rgba(0,0,0,0.4)', borderRadius: 10, padding: '10px 8px', textAlign: 'center',
                    }}>
                      <div style={{ fontSize: 8, fontWeight: 700, color: '#475569', marginBottom: 3, letterSpacing: '0.5px' }}>{k.label}</div>
                ...
                {forceData.by_confidence && (
                  <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 10, padding: 12, marginBottom: 10 }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: '#a855f7', letterSpacing: '0.8px', marginBottom: 6 }}>🎯 PAR CONFIANCE</div>
                    <table style={{ width: '100%', fontSize: 10, borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ color: '#475569', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                          {['Bracket', 'W', 'L', 'Total', 'Win Rate'].map(h => (
                            <th key={h} style={{ textAlign: h === 'Bracket' ? 'left' : 'center', padding: '3px 6px', fontWeight: 700 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(forceData.by_confidence).map(([b, d]) => d.total > 0 && (
                          <tr key={b} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                            <td style={{ padding: '3px 6px', color: '#e2e8f0' }}>{b}</td>
                            <td style={{ textAlign: 'center', padding: '3px 6px', color: '#00ffaa' }}>{d.won}</td>
                            <td style={{ textAlign: 'center', padding: '3px 6px', color: '#f87171' }}>{d.lost}</td>
                            <td style={{ textAlign: 'center', padding: '3px 6px', color: '#94a3b8' }}>{d.total}</td>
                            <td style={{ textAlign: 'center', padding: '3px 6px', fontWeight: 700, color: d.win_rate >= 60 ? '#00ffaa' : d.win_rate >= 50 ? '#fbbf24' : '#f87171' }}>{d.win_rate}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {forceData.by_market && (
                  <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 10, padding: 12, marginBottom: 10 }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: '#38bdf8', letterSpacing: '0.8px', marginBottom: 6 }}>📊 PAR MARCHÉ</div>
                    <table style={{ width: '100%', fontSize: 10, borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ color: '#475569', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                          {['Marché', 'W', 'L', 'Total', 'Win Rate'].map(h => (
                            <th key={h} style={{ textAlign: h === 'Marché' ? 'left' : 'center', padding: '3px 6px', fontWeight: 700 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(forceData.by_market).map(([m, d]) => d.total > 0 && (
                ...
                {forceData.trend?.length > 0 && (
                  <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 10, padding: 12 }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: '#fbbf24', letterSpacing: '0.8px', marginBottom: 8 }}>📈 TENDANCE 7 JOURS</div>
                    <ResponsiveContainer width="100%" height={120}>
                      <LineChart data={forceData.trend.map(d => ({ ...d, date: d.date.slice(5) }))}>
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                        <XAxis dataKey="date" tick={{ fontSize: 9, fill: '#475569' }} axisLine={false} tickLine={false} />
                        <YAxis domain={[0, 100]} tick={{ fontSize: 9, fill: '#475569' }} axisLine={false} tickLine={false} width={28} />
                        <Tooltip
                          contentStyle={{ background: '#0f172a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, fontSize: 11 }}
                          labelStyle={{ color: '#94a3b8' }}
                          formatter={(v) => `${v}%`}
                        />
                        <Line type="monotone" dataKey="win_rate" stroke="#00ffaa" strokeWidth={2} dot={{ r: 3, fill: '#00ffaa' }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
