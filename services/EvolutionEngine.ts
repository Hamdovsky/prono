...
class EvolutionEngine {
  ...
  _estimateRoiImpact(type) {
    const map = {
      ...
      GK_WALL: 0.25,
      GK_MASTERCLASS: 0.25,
      SET_PIECE_DECIDER: 0.2,
      LATE_GOAL: 0.2,
      LATE_GOAL_VARIANCE: 0.15,
      PENALTY_ANOMALY: 0.15,
      ODDS_TRAP_PATTERN: 0.3,
      XG_WASTE: 0.2,
      BIG_CHANCE_WASTE: 0.15,
      LOW_XG_CONVERSION: 0.2,
      MOTIVATION_MISREAD: 0.25,
      SYSTEMIC_DEFENSIVE_FAILURE: 0.3,
      PERSONNEL_DEFICIT_DISRUPTION: 0.2,
  ...
  async getMatchRiskFactor(league, home, away, refereeId) {
    try {
      ...
      for (const p of patterns) {
        ...
        if (p.frequency > 15) riskMultiplier += 0.1
      }
