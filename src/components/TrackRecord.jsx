...
const classifyMarket = (pick) => {
  ...
  if (
    ...
    return 'DC'
  if (pick.includes('home') || pick.includes('dom') || pick === '1' || pick.includes(' 1 '))
    return '1X2'
  if (pick.includes('away') || pick.includes('ext') || pick === '2' || pick.includes(' 2 '))
    return '1X2'
  if (pick.includes('draw') || pick.includes('nul') || pick === 'x' || pick.includes(' x '))
    return '1X2'
...
const evaluatePick = (pick, score) => {
  ...
  if (pick.includes('btts') || pick.includes('marquent') || pick.includes('oui'))
    return h > 0 && a > 0
  return false
}
...
const isNonStandard = (p) =>
  p.includes('risky bet') ||
  p.includes('skip') ||
  p.includes('no bet') ||
  p.includes('البطاقات') ||
  p.includes('cards') ||
...
const TrackRecord = ({ matches, onClose }) => {
  ...
  const sortedRecords = useMemo(() => {
    ...
    if (resultFilter !== 'ALL')
      filtered = filtered.filter((r) => (resultFilter === 'WON' ? r.isWin : !r.isWin))
        ...
        >
          <div>
            ...
            <div style={{ fontSize: '10px', color: '#64748b', marginTop: '4px' }}>
              {stats.total} pronos · {stats.winRate}% réusite · {stats.profit > 0 ? '+' : ''}
              {stats.profit}u net
            </div>
          </div>
          <button
