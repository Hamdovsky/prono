...
function validateProbabilities(match) {
  ...
  if (h > 0 && h < MIN_VALID_PROB)
    return { valid: false, reason: 'PROB_TOO_LOW', field: 'home', value: h }
  if (d > 0 && d < MIN_VALID_PROB)
    return { valid: false, reason: 'PROB_TOO_LOW', field: 'draw', value: d }
  if (a > 0 && a < MIN_VALID_PROB)
    return { valid: false, reason: 'PROB_TOO_LOW', field: 'away', value: a }
...
function validateMatchFreshness(match) {
  ...
  const age = now - tsMs
  if (age > MAX_MATCH_AGE_MS)
    return { valid: false, reason: 'MATCH_TOO_STALE', ageHours: Math.round(age / 3600000) }
...
function sanitizeMatches(matches, opts = {}) {
  ...
  for (const m of matches) {
    ...
    if (rejectReason) {
      ...
      if (logRejections) {
        logger.warn(
          `🚫 [SANITIZER] Rejected: ${m.homeTeam || '?'} vs ${m.awayTeam || '?'} → ${rejectReason}`
        )
      }
    } else {
      stats.kept++
  ...
  if (logRejections && stats.rejected > 0) {
    logger.info(
      `🧹 [SANITIZER] ${stats.rejected}/${stats.total} matches rejected. Reasons:`,
      stats.reasons
    )
  }
