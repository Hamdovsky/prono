...
class TeamRegistry {
  ...
  async register(teamName, league) {
    ...
    try {
      const regId = Math.abs(
        teamName.split('').reduce((h, c) => ((h << 5) - h + c.charCodeAt(0)) | 0, 0)
      )
      await this.db
        .prepare(
  ...
  validateMatch(homeTeam, awayTeam) {
    ...
    const integrity =
      homeCheck.confidence >= 0.7 && awayCheck.confidence >= 0.7
        ? 'HIGH'
        : homeCheck.confidence >= 0.5 && awayCheck.confidence >= 0.5
          ? 'MEDIUM'
          : 'LOW'
    ...
    return {
      ...
      integrity,
      warnings: [
        ...(homeCheck.isSuspicious
          ? [
              `Home team "${homeCheck.original}" → "${homeCheck.matched}" (${(homeCheck.confidence * 100).toFixed(0)}%)`,
            ]
          : []),
        ...(awayCheck.isSuspicious
          ? [
              `Away team "${awayCheck.original}" → "${awayCheck.matched}" (${(awayCheck.confidence * 100).toFixed(0)}%)`,
            ]
          : []),
      ],
    }
