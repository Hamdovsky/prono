      ...
      <section className="config-section">
        ...
        <div className="sync-master-bar">
          ...
          <div className="master-actions">
            ...
            >
              <span
                className={`material-symbols-outlined ${syncStatus === 'scanning' ? 'spin' : ''}`}
              >
                search
              </span>
              {syncStatus === 'scanning'
        ...
        <div className="priority-banner">
          ...
          SYNC PRIORITY: <strong>HIGH</strong> — All 12 leagues synchronized to local database &
          XGBoost Hub
        </div>
          ...
          grouped.map(({ tier, leagues: tierLeagues }) => (
            <div key={tier} className="tier-group">
              <div className="tier-label" style={{ color: TIER_LABELS[tier]?.color }}>
                {tier === 'ELITE' ? '🏆 EUROPEAN BIG 5' : '🌙 MENA & ARAB LEAGUES + SOUTH AMERICA'}
              </div>
      ...
      <section className="config-section">
        <div className="toggle-list">
          <ToggleItem
            ...
            desc="Removes finished match data"
            checked={autoPurge}
            onChange={() => setAutoPurge(!autoPurge)}
            color="#0d93f2"
          />
          <ToggleItem
            icon="content_copy"
            title="Duplicate Detection"
            desc="Heuristic redundancy check"
            checked={duplicateDetection}
            onChange={() => setDuplicateDetection(!duplicateDetection)}
            color="#ffd700"
          />
        </div>
      </section>
          ...
          <div className="input-field">
            ...
            >
              {['FLASHSCORE_LOCAL', 'EXTERNAL_API'].map((mode) => (
                ...
                >
                  {mode === 'FLASHSCORE_LOCAL' ? 'LOCAL SCRAPER' : 'SCRAPER API'}
                </button>
              ))}
            </div>
          </div>
          <InputField
            label="Live-Soccer-Scraper Endpoint"
            icon="link"
            type="text"
            value={scraperUrl}
            onChange={setScraperUrl}
            placeholder="https://api..."
          />
          <InputField
            label="Apify API Token (Radar)"
            icon="key"
            type="password"
            value={apifyToken}
  ...
  return (
    <div className="league-sync-card">
      ...
      <div className="lcard-status-row">
        <span className={`lcard-badge ${scanOn ? 'badge-scan' : 'badge-off'}`}>⚡ SMART SCAN</span>
        <span className={`lcard-badge ${isLive ? 'badge-webhook' : 'badge-off'}`}>🔗 WEBHOOK</span>
        {league.arabicNewsEnabled && <span className="lcard-badge badge-arabic">📡 AR-NEWS</span>}
      </div>
    </div>
  )
}
