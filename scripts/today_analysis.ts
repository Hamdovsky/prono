...
function getConfBand(conf) {
  ...
  if (c >= 0.6 || c >= 60) return 'MED' // 60–85%
  return 'LOW' // < 60%
}
...
async function runAnalysis(dateStr) {
  ...
  // Build league table
  const leagueTable = Object.entries(byLeague)
    .map(([league, d]) => ({
      league,
      hits: d.hits,
      misses: d.misses,
      ...
      accuracy:
        d.hits + d.misses > 0
          ? parseFloat(((d.hits / (d.hits + d.misses)) * 100).toFixed(1))
          : null,
    }))
  ...
  const confidenceBands = {
    HIGH: {
      ...
      accuracy:
        confBands.HIGH.hits + confBands.HIGH.misses > 0
          ? parseFloat(
              ((confBands.HIGH.hits / (confBands.HIGH.hits + confBands.HIGH.misses)) * 100).toFixed(
                1
              )
            )
          : null,
    ...
    MED: {
      ...
      accuracy:
        confBands.MED.hits + confBands.MED.misses > 0
          ? parseFloat(
              ((confBands.MED.hits / (confBands.MED.hits + confBands.MED.misses)) * 100).toFixed(1)
            )
          : null,
    },
    LOW: {
      ...
      accuracy:
        confBands.LOW.hits + confBands.LOW.misses > 0
          ? parseFloat(
              ((confBands.LOW.hits / (confBands.LOW.hits + confBands.LOW.misses)) * 100).toFixed(1)
            )
          : null,
    },
  }
  ...
  console.log(`╠══════════════════════════════════════════════════`)
  console.log(
    `║  ✅ 1X2 Accuracy : ${accuracy !== null ? accuracy + '%' : 'N/A'} (${hits}/${total})`
  )
  console.log(
    `║  ⚽ O/U 2.5     : ${result.markets.ou25.accuracy !== null ? result.markets.ou25.accuracy + '%' : 'N/A'} (${ou_hits}/${ou_total})`
  )
  console.log(
    `║  🤝 BTTS        : ${result.markets.btts.accuracy !== null ? result.markets.btts.accuracy + '%' : 'N/A'} (${btts_hits}/${btts_total})`
  )
...
export = { runAnalysis, loadAccuracyLog }
