...
const TicketDuJour = ({ matches }) => {
  const handleDownload = () => {
    saveAsJpeg(
      'ticket-du-jour-capture',
      `TicketDuJour_${new Date().toISOString().split('T')[0]}.jpg`
    )
  }
    ...
    const extractPick = (match) => {
      ...
      if (cs !== 'N/A' && cs.includes('-')) {
        ...
        if (!isNaN(h) && !isNaN(a)) {
          if (h > a) {
            pred1X2 = '1'
            pickLabel = 'Victoire Domicile (1)'
          } else if (a > h) {
            pred1X2 = '2'
            pickLabel = 'Victoire Extérieur (2)'
          } else {
            pred1X2 = 'X'
            pickLabel = 'Match Nul (X)'
          }
        }
      }
      if (pred1X2 === 'N/A') {
      ...
      if (validPicks.length === 0) {
        validPicks = picks.filter((p) => p.odd >= 1.2 && p.odd <= 2.1)
      }
    ...
    return (
      <div className="ticket-jour-container ticket-jour-empty">
        ...
        <div style={{ padding: '20px', textAlign: 'center', fontSize: '12px', color: '#64748b' }}>
          Aucun match avec une confiance ≥ 75% pour aujourd'hui.
          <br />
          <span style={{ fontSize: '11px', color: '#2d3f56' }}>
            Réessayez avec un filtre de date différent ou revenez plus tard.
          </span>
        </div>
      </div>
    )
    ...
    <div className="ticket-jour-container" id="ticket-du-jour-capture">
      <div className="ticket-jour-header">
        ...
        <div className="ticket-jour-badge">
          ✓ Confirmé par l'expert (Min {ticket.selections[0].confidence >= 80 ? '95%' : '85%'}{' '}
          fiabilité)
        </div>
      </div>
      ...
      <div className="ticket-jour-matches">
        {ticket.selections.map((sel, idx) => (
          <div className="ticket-match-row" key={`${sel.id}-${idx}`}>
            ...
            <div className="ticket-match-odd">{sel.odd.toFixed(2)}</div>
          </div>
        ))}
      </div>
