...
async function callPythonSentiment(headlines, retryCount = 0) {
  ...
  try {
    const response = await axios.post(
      'http://127.0.0.1:3001/api/sentiment',
      { headlines },
      {
        ...pooledConfig,
        timeout: 15000,
...
async function getEnglishNews(teamName, maxHours = 48) {
  const langs = [{ q: teamName, hl: 'en', gl: 'US', ceid: 'US:en' }]
        ...
        async () => {
          ...
          return [...raw.matchAll(/<item>([\s\S]*?)<\/item>/g)].map((m) => {
            ...
            const title = (block.match(/<title>([\s\S]*?)<\/title>/)?.[1] || '')
              ...
              .trim()
            const pubDate =
              block.match(/<pubDate>([\s\S]*?)<\/pubDate>/)?.[1] || new Date().toUTCString()
...
async function getSofaMissingPlayers(teamId) {
  ...
  try {
    ...
    const res = await fetchWithRetry(url, {
      headers: getSofaHeaders(`https://www.sofascore.com/team/football/team/${teamId}`),
    })
...
async function getSoccerwayInjuries(teamName) {
  try {
    ...
    const injuryRows = [
      ...html.matchAll(/class="(?:injury|suspension|absent)[^"]*"[^>]*>([\s\S]*?)<\/tr>/gi),
    ]
...
const TM_TEAM_MAP = {
  ...
  // ---- MENA : Maghreb ----
  'Wydad Casablanca': { slug: 'wydad-casablanca', id: 4451 },
  'Raja Casablanca': { slug: 'raja-casablanca', id: 4453 },
  'Esperance Tunis': { slug: 'esperance-sportive-de-tunis', id: 4439 },
  'Etoile du Sahel': { slug: 'etoile-sportive-du-sahel', id: 4443 },
  'Club Africain': { slug: 'club-africain', id: 4012 },
  'CS Sfaxien': { slug: 'cs-sfaxien', id: 4047 },
  'USM Alger': { slug: 'usm-alger', id: 4560 },
  'MC Alger': { slug: 'mc-alger', id: 4574 },
  'JS Kabylie': { slug: 'js-kabylie', id: 4324 },
  'ES Setif': { slug: 'es-setif', id: 4615 },
  'CR Belouizdad': { slug: 'cr-belouizdad', id: 4586 },
  'Hilal Sudanese': { slug: 'al-hilal-ombdurman', id: 12003 },
  'Al-Merrikh': { slug: 'al-merrikh-sc', id: 12004 },
  // ---- MENA : Golfe ----
  'Al-Sadd': { slug: 'al-sadd-sports-club', id: 6555 },
  'Al Duhail': { slug: 'al-duhail-sc', id: 8457 },
  'Al-Rayyan': { slug: 'al-rayyan-sc', id: 4041 },
  'Al-Shabab': { slug: 'al-shabab-riyad', id: 2926 },
  'Al-Taawoun': { slug: 'al-taawoun-fc', id: 22332 },
  'Al-Fateh': { slug: 'al-fateh-sc', id: 8367 },
  'Al-Wehda': { slug: 'al-wehda-club-mecca', id: 3186 },
  'Al-Ettifaq': { slug: 'al-ettifaq-club-dammam', id: 2659 },
  'Al-Adalah': { slug: 'al-adalah-club', id: 30753 },
  'Al-Ahli Jeddah': { slug: 'al-ahli-sfc', id: 1848 },
  'Al-Ain': { slug: 'al-ain-fc', id: 3620 },
  'Shabab Al-Ahli': { slug: 'shabab-al-ahli-dubai', id: 8427 },
  'Al-Wahda UAE': { slug: 'al-wahda-fc', id: 4078 },
  'Al-Jazira': { slug: 'al-jazira-club', id: 7589 },
  'Umm Salal': { slug: 'umm-salal-sc', id: 8508 },
  // ---- MENA : Egypte ----
  'Pyramids FC': { slug: 'pyramids-fc', id: 42620 },
  'Al Masry': { slug: 'al-masry-sc', id: 2580 },
  'Ismaily': { slug: 'ismaily-sc', id: 2391 },
  'ENPPI': { slug: 'enppi-club', id: 2977 },
  'Smouha': { slug: 'smouha-sc', id: 32952 },
  // ---- Ailleurs : ligues européennes majeures ----
  ...
  Sevilla: { slug: 'fc-sevilla', id: 368 },
  Valencia: { slug: 'fc-valencia', id: 1049 },
  Villarreal: { slug: 'villarreal-cf', id: 1050 },
  'Real Betis': { slug: 'real-betis-sevilla', id: 150 },
  Atalanta: { slug: 'atalanta-bergamo', id: 800 },
  Fiorentina: { slug: 'ac-florenz', id: 398 },
  Monaco: { slug: 'as-monaco', id: 162 },
  Lille: { slug: 'losc-lille', id: 1082 },
  Lyon: { slug: 'olympique-lyon', id: 1041 },
  'Sporting Lisbon': { slug: 'sporting-cp', id: 336 },
  Rangers: { slug: 'rangers-fc', id: 599 },
  Celtic: { slug: 'celtic-fc', id: 371 },
  Ajax: { slug: 'ajax-amsterdam', id: 610 },
  'PSV': { slug: 'psv-eindhoven', id: 383 },
}
  ...
  try {
    return await retry(
      async () => {
        ...
        const injuries = []
        const rowMatches = [
          ...res.data.matchAll(/<tr[^>]*class="[^"]*zebra[^"]*"[^>]*>([\s\S]*?)<\/tr>/g),
        ]
        for (const row of rowMatches.slice(0, 8)) {
          ...
          if (cells.length >= 3) {
            injuries.push({
              name: cells[0],
              reason: cells[2],
              returnDate: cells[cells.length - 1],
...
async function getSofaLineups(sofaMatchId) {
  ...
  try {
    ...
    const res = await fetchWithRetry(url, {
      headers: getSofaHeaders(`https://www.sofascore.com/event/${sofaMatchId}`),
    })
    const data = res ? await res.json() : null
...
function analyzeIntelligenceImpact(headlines, injuries) {
  ...
  const allText = (
    headlines.join(' ') +
    ' ' +
    injuries.map((i) => i.name + ' ' + i.reason).join(' ')
  ).toLowerCase()
  ...
  return {
    ...
    summary:
      severity === 'CRITICAL'
        ? '⚠️ غيابات جوهرية مؤثرة على التوقع'
        : severity === 'MODERATE'
          ? '⚖️ غيابات متوسطة القوة'
          : '✅ لا مؤشرات سلبية قوية',
...
async function getPlayerFormRating(playerId) {
  ...
  try {
    ...
    const res = await fetchWithRetry(url, {
      headers: getSofaHeaders(`https://www.sofascore.com/player/player/${playerId}`),
    })
    const data = res ? await res.json() : null
...
async function getNewsForTeam(teamName, maxHours = 48, options = {}) {
  ...
  const gNews =
    goalNews.status === 'fulfilled' && goalNews.value
      ? [
          {
            title: goalNews.value.latestTitle,
            pubDate: goalNews.value.timestamp,
            source: 'goal_direct',
          },
        ]
      : []
  ...
  const intelligence = analyzeIntelligenceImpact(
    allNews.map((n) => n.title),
    injuries
  )
...
async function getMatchIntelligence(
  sofaMatchId,
  homeTeam,
  awayTeam,
  matchTimestamp,
  { forceRefresh = false, countryHint = '', homeTeamId = null, awayTeamId = null } = {}
) {
  ...
  const home =
    homeData.status === 'fulfilled'
      ...
      : {
          headlines: [],
          injuries: [],
          intelligence: { severity: 'MINOR', impactScore: 0, features: {} },
        }
  const away =
    awayData.status === 'fulfilled'
      ? awayData.value
      : {
          headlines: [],
          injuries: [],
  ...
  if (sofaLineups?.missingKey?.length) {
    sofaLineups.missingKey.forEach((mp) => {
      ...
      if (!target.injuries.find((i) => i.name === mp.name))
  ...
  if (sofaLineups?.confirmed) {
    ;[hForm, aForm] = await Promise.all([
      computeTeamFormRating(sofaLineups.home),
      computeTeamFormRating(sofaLineups.away),
    ])
  }
  ...
  const result = {
    ...
    home,
    away,
    confirmed: sofaLineups?.confirmed ?? false,
    home_form_rating: hForm,
    away_form_rating: aForm,
    intelligence_verdict:
      home.intelligence.severity === 'CRITICAL' || away.intelligence.severity === 'CRITICAL'
        ? 'HIGH_INTEL'
        : 'NORMAL',
    fetched_at: Date.now(),
  }
...
async function getSofaH2H(eventId) {
  ...
  try {
    ...
    const res = await fetchWithRetry(url, {
      headers: getSofaHeaders(`https://www.sofascore.com/event/${eventId}`),
    })
