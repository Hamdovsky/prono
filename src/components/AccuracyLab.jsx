    ...
    <div className="lab-tooltip">
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color }}>
          {p.name === 'Accuracy' ? 'الدقة' : 'الإصابات'}:{' '}
          <strong>
            {p.value}
            {p.name === 'Accuracy' ? '%' : ''}
          </strong>
...
export default function AccuracyLab() {
  ...
  const overallAccuracy = data?.history?.length
    ? Math.round(
        data.history.reduce((acc, curr) => acc + (curr.accuracy || 0), 0) / data.history.length
      )
    : 0
  ...
  return (
    <div className="accuracy-lab">
      ...
      <div className="lab-tabs">
        ...
        <button
          className={activeTab === 'misses' ? 'active' : ''}
      ...
      <div className="lab-content">
            ...
            <div className="summary-card main">
              ...
              <div className="score-ring">
                <svg viewBox="0 0 36 36" className="circular-chart">
                  ...
                  <path
                    className="circle"
                    strokeDasharray={`${current.accuracy || 0}, 100`}
                    d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                  />
              ...
              <div className="summary-details">
                <div className="det">
                  <span>✅ إصابات:</span> <strong>{current.hits}</strong>
                </div>
                <div className="det">
                  <span>❌ إخفاقات:</span> <strong>{current.misses}</strong>
                </div>
                <div className="det">
                  <span>⏳ قيد الانتظار:</span> <strong>{current.pending}</strong>
                </div>
              </div>
            </div>
              ...
              <div className="league-mini-list">
                {(current.leagueTable || []).slice(0, 5).map((l, i) => (
                  <div key={i} className="league-mini-item">
                    ...
                    <div className="b">
                      <div style={{ width: `${l.accuracy}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'misses' && (
          <div className="lab-misses-container">
            ...
            <p className="hint">
              هذه المباريات تم إرسالها لمحرك التدريب بوزن (5.0x) لتصحيح الخطأ في المرة القادمة.
            </p>
              ...
              {(current.missedPredictions || []).map((m, i) => (
                <div key={i} className="miss-card-v19">
                  ...
                  <div className="m-verdict">
                    <div className="v">
                      <span>وقعنا:</span> {m.predicted}
                    </div>
                    <div className="v">
                      <span>الواقع:</span> {m.actual}
                    </div>
                  </div>
                  <div className="m-footer">ثقة XGBoost: {Math.round((m.xgbConf || 0) * 100)}%</div>
                </div>
              ))}
            </div>
          </div>
