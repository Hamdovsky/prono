...
class AuditService {
      ...
      const total =
        (
          await database.db.query(
            `SELECT COUNT(*) as count FROM prediction_history WHERE status != 'PENDING'`
          )
        ).rows[0]?.count || 0
      const correct =
        (
          await database.db.query(
            `SELECT COUNT(*) as count FROM prediction_history WHERE status = 'CORRECT'`
          )
        ).rows[0]?.count || 0
      const wrong =
        (
          await database.db.query(
            `SELECT COUNT(*) as count FROM prediction_history WHERE status = 'WRONG'`
          )
        ).rows[0]?.count || 0
  ...
  _generateEvaluation(stats) {
    ...
    if (stats.global.accuracy < 75)
      issues.push('انخفاض معدل التحويل العالمي لأقل من المستهدف (80%)')

    return {
      rating: rating.toFixed(1),
      strengths: ['قوة كبيرة في توقع الفائز (1X2)', 'دقة عالية في الدوري الإنجليزي وهولندا'],
      weaknesses: [
        'تأثر كبير بالهجمات القاتلة (Late Goals)',
        'ضعف في أسواق الأهداف (O/U 2.5) في الدوريات الدفاعية',
      ],
      recommendations: [
