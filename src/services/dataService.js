...
import {
  ...
  isReserveTeam,
  deduplicateMatches,
} from '../utils/teamNameNormalizer.js'
const logger = {
...
class DataService {
  ...
  _handleMatchesUpdate(raw) {
    if (Array.isArray(raw)) {
      const validMatches = raw.map((m) => this._normalizeMatch(m, 'live')).filter((m) => m !== null)

      const uniqueMap = new Map()
      validMatches.forEach((m) => uniqueMap.set(m.id, m))
      this.matches = Array.from(uniqueMap.values())

      this._hasLiveMatches = this.matches.some(
        (m) => m.isLive || m.status === 'live' || (m.minute && m.minute.includes("'"))
      )
  ...
  async _get(url) {
    ...
    const fetchPromise = (async () => {
      try {
        ...
        logger.info(
          `✅ [FETCHER] Parse complete for ${url} (${Date.now() - (startTime + fetchTime)}ms)`
        )
        return data
      } finally {
  ...
  _handleRateLimit() {
    if (!this.isRateLimited) {
      ...
      this.subscribers.forEach((cb) =>
        cb({ error: 'RATE_LIMIT', message: 'Rate Limit Active. Retrying...' })
      )
    }
  }
  ...
  _normalizeMatch(match, mode = 'live') {
    ...
    if (tsMs && (tsMs < now - pastWindow || tsMs > now + futureWindow)) {
      logger.warn(
        `🚫 [SANITY] Rejecting match with invalid date range: ${match.homeTeam} vs ${match.awayTeam} (${new Date(tsMs).toLocaleDateString()})`
      )
      return null
    }
    ...
    return {
      ...
      league: this._normalizeName(
        match.league ||
          match.league_name ||
          match.leagueName ||
          (typeof match.league === 'object' ? match.league.name : 'Unknown')
      ),
      ...
      prediction: (() => {
        ...
        if (Array.isArray(p))
          return p.length > 0 ? p[0]?.label || p[0]?.name || JSON.stringify(p[0]) : null
  ...
  _normalizeName(name) {
    ...
    const sponsorRegex =
      /(Barclays|McDonald's|McDonald’s|Enilive|Betclic|EA Sports|Bwin|Sky Bet)\s+/gi
    ...
    for (const [key, val] of Object.entries(NORMALIZATION_MAP)) {
      if (
        lowerN === key.toLowerCase() ||
        (lowerN.startsWith(key.toLowerCase()) && n.length < key.length + 5)
      ) {
        return val
      }
    }
    ...
    this._upcomingFetchPromise = (async () => {
      try {
        const endpoint = force
          ? `${getApiUrl('/api/upcoming')}?days=14&force=true`
          : this.upcomingApiEndpoint
        logger.info('📡 [DATA] Fetching upcoming matches from:', endpoint)
        const raw = await this._get(endpoint)

        logger.info(
          `📊 [DATA] Received ${Array.isArray(raw) ? raw.length : 'non-array'} raw matches.`
        )

        const rawMatches = Array.isArray(raw)
          ? raw
          : raw?.matches || [
  ...
  async fetchPromosportFeedbackStats() {
    try {
      return await this._get(getApiUrl('/api/promosport/feedback/stats'))
    } catch (error) {
      return null
    }
  }
  ...
  async fetchPromosportCalculator(cols, doubles, triples = 0) {
    try {
      return await this._get(
        getApiUrl(`/api/promosport/calculator?cols=${cols}&doubles=${doubles}&triples=${triples}`)
      )
    } catch (error) {
  ...
  _checkEliteNotifications() {
    const eliteTargets = this.matches.filter(
      (m) => (m.winProb || 0) >= 90 && !this._notifiedMatches.has(m.id)
    )
