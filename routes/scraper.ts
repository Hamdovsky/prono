...
/**
 * GET /api/news-watch
 * Returns news-watch precision data for the PrecisionTracker component.
 */
...
router.post('/scraper/betx2-ticket', async (req, res) => {
  ...
  try {
    ...
    await mainPage.goto('https://betx2.com/fr/sport', {
    ...
    const inputTyped = await sportPage.evaluate((ticketId) => {
      ...
      const known = [
        'input.dg_bs_book_input',
        'input[class*="book_input"]',
        'input[placeholder*="nregistrement"]',
        'input[placeholder*="oupon"]',
        'input[placeholder*="icket"]',
    ...
    const confirmClicked = await sportPage.evaluate(() => {
      ...
      for (const b of btns) {
        ...
        if (
          (t.includes('confirm') || t.includes('vérif') || t.includes('check') || t === 'ok') &&
    ...
    if (lower.includes('en cours') || lower.includes('ouvert') || lower.includes('pending'))
      status = 'Pending'
    ...
    if (browser) {
      try {
        ...
        logger.error('[SCRAPER] Error closing pages:', e.message)
      }
    }
  }
})
...
router.post('/scraper/bibeet-ticket', async (req, res) => {
  ...
  try {
    ...
    const inputResult = await target.evaluate(
      (tid, isBooking) => {
        ...
        if (!inputEl) {
          inputEl = document.querySelector(
            'input[class*="booking"], input[class*="fast"], input[class*="search-bets"]'
          )
        }
        ...
        if (!inputEl && isBooking) {
          ...
          const bookingLabel = labels.find(
            (l) => l.innerText && l.innerText.toLowerCase().includes('booking')
          )
          if (bookingLabel) {
            inputEl =
              bookingLabel.parentElement.querySelector('input') ||
              bookingLabel.querySelector('input')
          }
        }
    ...
    if (!inputResult.ok) {
      ...
      await target.evaluate(() => {
        const icons = Array.from(
          document.querySelectorAll('[aria-label], [title], [class*="icon"]')
        )
    ...
    if (oddsMatches) {
      ...
      const candidates = oddsMatches
        .map((m) => parseFloat(m.replace(',', '.')))
        .filter((v) => v > 1.05 && v < 10000)
    ...
    if (fs.existsSync(bibeetFile)) {
      try {
        ...
        if (bibeet.bookingCode && bibeet.bookingCode !== 'NOT_FOUND') {
          const alreadyIn = codes.some(
            (c) => c.code === bibeet.bookingCode && c.platform === 'Bibeet'
          )
          if (!alreadyIn) {
            codes.unshift({
              id: 'bibeet_auto',
...
router.post('/booking-codes/add', localOrAuth, (req, res) => {
  try {
    ...
    // Avoid duplicates
    const exists = db.codes.some(
      (c) => c.code === newEntry.code && c.platform === newEntry.platform
    )
