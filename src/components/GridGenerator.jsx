...
const GridGenerator = ({ eliteMatches }) => {
      ...
      if (dvb) {
        if (dcUsed < 6) {
          ...
          golden = {
            pick: standardPick,
            type: '⚠️ RISQUE',
            prob: Math.round(hProb > aProb ? hProb : aProb),
          }
        }
  ...
  return (
    <div className="grid-generator-container">
      <button className="grid-generator-btn" onClick={() => setShowGrid((s) => !s)}>
      ...
      {showGrid && (
        <div className="promosport-container">
          <div className="promosport-header">
            <span className="promosport-title">
              🎰 GRILLE PROMOSPORT TUNISIE — ⚡ TITANIUM SELECTION
            </span>
            <span className="golden-badge">
              🏆 LA COLONNE D'OR ACTIVÉE ({dcCount} DOUBLES CHANCES INJECTÉS)
            </span>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="text-center golden-cell">
                      <span
                        className={`golden-badge-pick ${isDouble ? 'double-gold' : 'single-gold'}`}
                      >
                        {g.golden.pick}
                        {g.golden.prob > 0 && (
                          <span className="golden-prob"> {g.golden.prob}%</span>
                        )}
                      </span>
                    </td>
                    <td className="text-center">
                      <span
                        className={`goals-tag ${g.ouResult.label.includes('OVER') ? 'tag-over' : g.ouResult.label.includes('UNDER') ? 'tag-under' : 'tag-neutral'}`}
                      >
                        {g.ouResult.label}
                    ...
                    <td className="text-center text-nature">
                      <span
                        className={`nature-tag ${isTrapp ? 'tag-piege' : isSurprise ? 'tag-surprise' : isCover ? 'tag-cover' : 'tag-base'}`}
                      >
                        {g.golden.type}
                        {g.golden.prob > 0 && (
                          <span className="nature-prob"> — {g.golden.prob}%</span>
                        )}
                      </span>
                    </td>
                  </tr>
          ...
          <div className="promosport-footer">
            <span>
              📊 Système Combinatoire: <b>{dcCount} Doubles</b> = <b>{Math.pow(2, dcCount)}</b>{' '}
              Colonnes Équivalentes Simple.
            </span>
            <button className="btn-print" onClick={() => window.print()}>
              🖨️ IMPRIMER LA COLONNE D'OR
            </button>
          </div>
        </div>
      )}
