...
class SmartOddsAnalyzer {
  ...
  async detectPattern(movement, history) {
    const profile = {
      direction:
        movement.home_delta_total < 0
          ? 'HOME_DOWN'
          : movement.away_delta_total < 0
            ? 'AWAY_DOWN'
            : 'STABLE',
      steepness:
        Math.abs(movement.home_delta_total) > 15
          ? 'SHARP'
          : Math.abs(movement.home_delta_total) > 5
            ? 'MODERATE'
            : 'FLAT',
  ...
  async getPatternPrediction(pattern) {
    ...
    const maxWin = Math.max(existing.win_rate_home, existing.win_rate_draw, existing.win_rate_away)
  ...
  async adjustPrediction(basePrediction, matchId) {
    ...
    oddsAnalysis.signals.list.forEach((signal) => {
      if (signal.type === 'STEAM_HOME') {
        adjusted.home_win_probability = Math.min(
          95,
          adjusted.home_win_probability + signal.strength * 0.8
        )
        adjusted.confidence = Math.min(95, adjusted.confidence + 5)
      }
      if (signal.type === 'STEAM_AWAY') {
        adjusted.away_win_probability = Math.min(
          95,
          adjusted.away_win_probability + signal.strength * 0.8
        )
        adjusted.confidence = Math.min(95, adjusted.confidence + 5)
      }
    ...
    const total =
      adjusted.home_win_probability + adjusted.draw_probability + adjusted.away_win_probability
