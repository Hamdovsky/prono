...
import EnrichedPredictionService from './enriched_predictions'
...
async function reEnrich() {
  console.log(
    `🚀 [RE-ENRICH] Starting mass prediction update (batch: ${BATCH_LIMIT}, timeout: ${TIMEOUT_MS}ms)...`
  )
  ...
  try {
    ...
    const matches = allMatches
      .filter((m) => riskLabels.includes(m.prediction) || !m.prediction)
      .slice(0, BATCH_LIMIT)
