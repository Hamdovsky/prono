...
async function populatePredictions() {
  ...
  try {
    ...
    for (const row of matches) {
      console.log(
        `⏳ Processing: ${row.homeTeam || row.hometeam} vs ${row.awayTeam || row.awayteam}...`
      )
      ...
      try {
        ...
        if (result.success && result.predictions) {
          ...
          console.log(
            `✅ Updated ${row.homeTeam}: ${winRaw.home}% / ${winRaw.draw}% / ${winRaw.away}% [Conf: ${Math.round((result.xgboost_confidence || 0.5) * 100)}%]`
          )
        } else {
          console.warn(
            `⚠️ Failed to predict ${row.homeTeam} vs ${row.awayTeam}: ${result.error || 'Unknown error'}`
          )
        }
      } catch (err) {
...
export = { populatePredictions, runPythonPrediction }
