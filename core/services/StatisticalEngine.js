...
class StatisticalEngine {
  ...
  predictCorners(match, winnerProbability) {
    ...
    if (leagueName.includes('premier league') || leagueName.includes('champions')) {
      ...
      leagueName.includes('national league')
    ) {
      baseCorners = 9
    }
    ...
    const variation =
      Math.floor(
        this._deterministicHash(
          match.homeTeam || '',
          match.awayTeam ? match.awayTeam.charCodeAt(0) : 0
        ) * 4
      ) - 2
  ...
  predictCards(match) {
    ...
    const variation =
      Math.floor(
        this._deterministicHash(
          match.awayTeam || '',
          match.homeTeam ? match.homeTeam.charCodeAt(0) : 1
        ) * 3
      ) - 1
  ...
  predictGoals(match, winnerProbability) {
    ...
    if (
      ...
      leagueName.includes('iceland')
    ) {
      baseGoals = 3.3
    } else if (
      leagueName.includes('premier league') ||
      leagueName.includes('norway') ||
      leagueName.includes('sweden')
    ) {
      baseGoals = 2.9
    } else if (
    ...
    const variation =
      (this._deterministicHash(match.homeTeam || '' + match.awayTeam || '', 42) - 0.5) * 1.5
  ...
  _getThetaForLeague(league) {
    ...
    if (key.includes('bundesliga') || key.includes('netherlands') || key.includes('eredivisie'))
      return 3.5
    if (
  ...
  getGoalModelParams(league) {
    ...
    if (key.includes('iceland') || key.includes('reykjavik') || key.includes('women'))
      return { rho: -0.05, gamma: 0.03 }
    if (
      key.includes('bundesliga') ||
      key.includes('netherlands') ||
      key.includes('eredivisie') ||
      key.includes('austria')
    )
      return { rho: -0.08, gamma: 0.02 }
    if (
      key.includes('premier league') ||
      key.includes('championship') ||
      key.includes('norway') ||
      ...
      return { rho: -0.1, gamma: 0.01 }
    if (key.includes('ligue 1') || key.includes('france') || key.includes('ligue 2'))
      return { rho: -0.14, gamma: 0.0 }
    ...
    if (key.includes('club friendly') || key.includes('friendly'))
      return { rho: -0.06, gamma: 0.02 }
        ...
        if (hStr.matchCount >= 1 || aStr.matchCount >= 1) {
          ...
          if (!xgH) {
            ...
            if (
              ...
              league.includes('austria') ||
              league.includes('swiss') ||
              league.includes('belgium') ||
              league.includes('portugal')
            ) {
              baseXgH = 1.8
              baseXgA = 1.4
            } else if (
              ...
              league.includes('superliga') ||
              league.includes('super lig')
            ) {
              baseXgH = 1.35
              baseXgA = 1.1
            } else if (
              league.includes('misli') ||
              league.includes('azerbaijan') ||
              league.includes('ekstraklasa') ||
              league.includes('usl') ||
              league.includes('japan') ||
              league.includes('korea') ||
  ...
  _getLeagueBaseXG(league) {
    ...
    if (
      key.includes('club friendly') ||
      key.includes('friendly') ||
      key.includes('world cup') ||
      key.includes('worldcup')
    )
      return { h: 1.55, a: 1.25 }
    if (key.includes('iceland') || key.includes('reykjavik') || key.includes('women'))
      return { h: 2.0, a: 1.6 }
    if (
      key.includes('bundesliga') ||
      ...
      key.includes('austria') ||
      key.includes('swiss') ||
      key.includes('belgium') ||
      key.includes('portugal')
    )
      return { h: 1.85, a: 1.55 }
    if (
      key.includes('premier league') ||
      key.includes('championship') ||
      key.includes('norway') ||
      ...
      return { h: 1.45, a: 1.25 }
    if (
      key.includes('misli') ||
      key.includes('azerbaijan') ||
      key.includes('ekstraklasa') ||
      key.includes('usl') ||
      key.includes('japan') ||
      key.includes('korea') ||
      key.includes('china')
    ...
    if (
      ...
      key.includes('argentina') ||
      key.includes('brazil') ||
      key.includes('superliga') ||
      key.includes('super lig')
    )
      return { h: 1.3, a: 1.1 }
  ...
  getTeamAttackDefense(teamName, lambda = 0.008) {
    ...
    try {
      ...
      return { attack: 0, defense: 0, matchCount: 0 }
    }
  }
  ...
  calculateMarketProbs(xgH, xgA, opts = {}) {
    ...
    let pBTTS = 0,
      pBTTS_NO = 0
    const pOU = { 0.5: 0, 1.5: 0, 2.5: 0, 3.5: 0, 4.5: 0 }
    const pU = { 0.5: 0, 1.5: 0, 2.5: 0, 3.5: 0, 4.5: 0 }

    // Advanced Markets
    let pCleanSheetH = 0,
      pCleanSheetA = 0
    const pScoreFirstH = xgHadj / (xgHadj + xgAadj || 1)
    const pAH = { 'H-1.5': 0, 'H-1': 0, 'A-1.5': 0, 'A-1': 0, 'H+1': 0, 'A+1': 0 }
    const pEH = { 'H-1': 0, 'A-1': 0 }
    ...
    let bttsFactor = 1.0
    if (totalXG > 2.8)
      bttsFactor = 1.25 // +25% boost for high xG
  ...
  calculateFirstHalfProbs(xgH, xgA, m = {}) {
    ...
    else if (league.includes('brazil')) htRatio = 0.4
    ...
    return {
      ...
      dc: {
        '1X': (pH_ht + pD_ht) / totalHT,
        X2: (pA_ht + pD_ht) / totalHT,
        12: (pH_ht + pA_ht) / totalHT,
      },
      ou05: pOU05_ht,
      ou15: pOU15_ht,
      btts: pBTTS_ht,
