...
async function getBrowser() {
  ...
  if (globalBrowser && needsRestart) {
    console.log(
      `🔄 [REUSE-MANAGER] Threshold reached (Reqs: ${requestsInCurrentBrowser}, RAM: ${memUsage.toFixed(0)}MB). Restarting Browser...`
    )
    await closeBrowser()
  }
...
async function fetchWithPuppeteer(url) {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `⚠️ [Puppeteer] goto() failed for ${url}: ${gotoErr.message}. Trying evaluate() fallback...`
      )
    }
    ...
    if (gotoFailed) {
      const jsonResponse = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Evaluate execution timed out (35s)')), 35000)
        ),
      ])

      if (jsonResponse === null) return null
      if (jsonResponse && jsonResponse.errorStatus)
        throw new Error(`HTTP ${jsonResponse.errorStatus}`)
    ...
    if (isFatal && globalBrowser) {
      ...
      console.warn(
        `🔄 [Puppeteer] Page timeout — recycling page slot ${pagePool.indexOf(entry) + 1}...`
      )
      try {
      ...
      try {
        ...
        if (status === 403 || status === 429) {
          ...
          if (url.includes('/player/') && url.includes('/statistics')) {
            console.warn(
              `🛡️ [apiClient] 403 on non-critical Player Stats. Skipping Puppeteer bypass to save time.`
            )
            return null
          }
...
const SofaAPI = {
  ...
  async getTeamStats(teamId, tournamentId, seasonId) {
    const res = await fetchWithRetry(
      `${this.BASE}/team/${teamId}/unique-tournament/${tournamentId}/season/${seasonId}/statistics/overall`
    )
    return res ? res.json() : null
  },
  ...
  async getTeamForm(teamId, tournamentId, seasonId) {
    const res = await fetchWithRetry(
      `${this.BASE}/team/${teamId}/unique-tournament/${tournamentId}/season/${seasonId}/events/last/5`
    )
    return res ? res.json() : null
  },
  ...
  async getStandings(tournamentId, seasonId) {
    const res = await fetchWithRetry(
      `${this.BASE}/unique-tournament/${tournamentId}/season/${seasonId}/standings/total`
    )
    return res ? res.json() : null
  },
  ...
  async getPlayerStats(playerId, tournamentId, seasonId) {
    const res = await fetchWithRetry(
      `${this.BASE}/player/${playerId}/unique-tournament/${tournamentId}/season/${seasonId}/statistics/overall`
    )
    return res ? res.json() : null
  },
  ...
  async getTournamentEvents(tournamentId, seasonId, type = 'next') {
    ...
    const res = await fetchWithRetry(
      `${this.BASE}/unique-tournament/${tournamentId}/season/${seasonId}/events/${type}/0`
    )
    return res ? res.json() : { events: [] }
  },
  ...
  async getPlayerSeasonHeatmap(playerId, tournamentId, seasonId) {
    ...
    const res = await fetchWithRetry(
      `${this.BASE}/player/${playerId}/unique-tournament/${tournamentId}/season/${seasonId}/heatmap`
    )
    return res ? res.json() : null
  },
}
