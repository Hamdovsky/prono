...
class ShieldEngine {
  ...
  async checkProxyHealth() {
    ...
    for (const proxy of this.proxies) {
      ...
      try {
        ...
        logger.warn(
          `⚠️ [SHIELD] Proxy ${proxy} failed health check. Removing from active rotation.`
        )
      }
    }
    ...
    logger.info(
      `🛡️ [SHIELD] Health check complete. ${this.healthyProxies.length}/${this.proxies.length} proxies available.`
    )
  }
  ...
  rotateProxy(reason = 'Routine') {
    ...
    logger.info(
      `🔄 [SHIELD] Rotating Proxy (${reason}): Now using ${this.systemHealth.activeProxy}`
    )
  }
