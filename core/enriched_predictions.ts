...
import MatchAuditor from '../services/MatchAuditor'
...
import goalNewsService from '../services/goalNewsService'
import sharpService from '../services/SharpIntelligenceService'
import correlationEngine from '../services/MarketCorrelationEngine'
import fpisEngine from '../services/FPISEngine'
import motivationService from '../services/MotivationEnrichService'
import EnvironmentalIntelligence from '../services/EnvironmentalIntelligence'
import bankrollService from '../services/bankrollService' // V90
import NewsAnalysisService from './services/NewsAnalysisService'
import MarketIntelligenceService from './services/MarketIntelligenceService'
import StatisticalEngine from './services/StatisticalEngine'
import adaptiveLearningEngine from '../services/adaptiveLearningEngine'
import patternService from '../services/patternService'
import SmartOddsAnalyzer from '../services/SmartOddsAnalyzer'
import DiagnosticTrace from './utils/DiagnosticTrace'
import Schemas from './utils/Schemas'
import QuantumQuantEngine from './QuantumQuantEngine'
  ...
  async enrichMatch(match, timeoutMs = null) {
    ...
    try {
      ...
      if (liveOdds) {
        ...
        try {
          import QuantRiskService from '../services/quantRiskService'
  ...
  async fastEnrichMatch(match) {
    try {
      ...
      if (!hasRealXg) {
        ...
        if (m.sofascore_id) {
          try {
            import sofaXgSvc from '../services/sofascoreXgService'
      ...
      const hasRealOdds =
        parseFloat(m.odds_home) > 0 && parseFloat(m.odds_away) > 0 && !oddsAreSynthetic
      ...
      const insufficient =
        (!hasRealOdds && (!hasXg || !hasForm || v553isDefault)) || oddsAreSynthetic ? 1 : 0
      ...
      const resultData = {
        ...
        draw_value_bet: oddsAreSynthetic
          ? false
          : (() => {
        ...
        alt_market_hunter: (() => {
          const xgTotal =
            parseFloat(m.home_xg || m.xg_home || 0) + parseFloat(m.away_xg || m.xg_away || 0) ||
            (quantResult.expected_score
              ? (() => {
                  const parts = quantResult.expected_score
                    .split('-')
          ...
          const crashed = xgTotal > 2.85 && (marketHidden || (ou25Odds > 0 && ou25Odds < 1.4))
          ...
          if (bttsOdds > 0 && bttsProb > 0) {
            ...
            if (bttsEV > 0.05)
              alternatives.push({
                market: 'BTTS Yes',
                prob: Math.round(bttsProb),
                odds: bttsOdds,
                ev: Math.round(bttsEV * 100) / 100,
                label: '🎯 BTTS YES',
              })
          } else if (bttsOdds <= 0 && bttsProb > 55) {
            const estOdds = Math.min(2.2, (1 / (bttsProb / 100)) * 0.92)
            const bttsEV = (bttsProb / 100) * estOdds - 1
            if (bttsEV > 0.08)
              alternatives.push({
                market: 'BTTS Yes',
                prob: Math.round(bttsProb),
                odds: Math.round(estOdds * 100) / 100,
                ev: Math.round(bttsEV * 100) / 100,
                label: '🎯 BTTS YES (estimé)',
              })
          }
          ...
          if (htO10Odds > 0 && htGoalProb > 0) {
            ...
            if (htEV > 0.05)
              alternatives.push({
                market: 'Over 1.0 HT',
                prob: Math.round(htGoalProb),
                odds: htO10Odds,
                ev: Math.round(htEV * 100) / 100,
                label: '⚡ OVER 1.0 HT',
              })
          } else if (htGoalProb > 65) {
            const estOdds = Math.min(2.1, (1 / (htGoalProb / 100)) * 0.95)
          ...
          if (asian225Odds > 0) {
            ...
            if (aEV > 0.03)
              alternatives.push({
                market: 'Asian O2.25',
                prob: Math.round(asian225Prob * 100) / 100,
                odds: asian225Odds,
                ev: Math.round(aEV * 100) / 100,
                label: '🛡️ ASIAN O2.25',
              })
          }
          ...
          if (asian275Odds > 0) {
            ...
            if (aEV > 0.03)
              alternatives.push({
                market: 'Asian O2.75',
                prob: Math.round(asian275Prob * 100) / 100,
                odds: asian275Odds,
                ev: Math.round(aEV * 100) / 100,
                label: '🛡️ ASIAN O2.75',
              })
          ...
          return {
            detected: crashed,
            reason: marketHidden
              ? '🔒 Marché Over 2.5 verrouillé par le bookmaker — xG élevé'
              : crashed
                ? '🎯 Over 2.5 crashé — xG élevé'
                : '💡 Alternative value détectée',
...
export = new EnrichedPredictionService()
