    ...
    for (const m of todaysMatches) {
      try {
        ...
        const winProb = Math.max(
          enriched.home_win_probability || 0,
          enriched.away_win_probability || 0
        )
        const isConfirmed =
          enriched.xgboost_confidence >= 0.7 ||
          winProb >= 65 ||
          (enriched.enriched && enriched.enriched.confidence >= 65)
        ...
        const winner =
          enriched.home_win_probability > enriched.away_win_probability ? m.homeTeam : m.awayTeam
