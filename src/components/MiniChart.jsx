    ...
    <svg viewBox={`0 0 ${w} ${height}`} style={{ width: '100%', height }}>
      {[0, 0.5, 1].map((t) => (
        <line
          ...
          x1={pad.left}
          x2={pad.left + iw}
          y1={pad.top + ih - ((t * maxY) / maxY) * ih}
          y2={pad.top + ih - ((t * maxY) / maxY) * ih}
          stroke="rgba(255,255,255,0.05)"
          strokeDasharray="3 3"
        />
      ))}
        ...
        return (
          <g key={i}>
            ...
            <text
              ...
              fill="#64748b"
              fontSize={9}
              transform={`rotate(45,${x + bw / 2},${pad.top + ih + 14})`}
            >
              {d[xKey]?.slice(0, 6) || ''}
            </text>
          </g>
      ...
      <text x={pad.left - 8} y={pad.top + 4} textAnchor="end" fill="#64748b" fontSize={10}>
        {maxY.toFixed(1)}
      </text>
      <text x={pad.left - 8} y={pad.top + ih + 4} textAnchor="end" fill="#64748b" fontSize={10}>
        0
      </text>
    </svg>
  )
...
export function RadarChartSVG({ data, height = 250, color = '#ef4444' }) {
  ...
  return (
    <svg viewBox={`0 0 ${cx * 2} ${cy * 2}`} style={{ width: '100%', height }}>
      ...
      {/* dataset B */}
      <polygon
        points={data.map((d, i) => point(d.B || 0, i)).join(' ')}
        fill={color}
        fillOpacity={0.5}
        stroke={color}
        strokeWidth={2}
      ...
      {/* dataset A */}
      <polygon
        points={data.map((d, i) => point(d.A || 0, i)).join(' ')}
        fill="#3b82f6"
        fillOpacity={0.2}
        stroke="#3b82f6"
        strokeWidth={2}
      ...
      {/* Legend */}
      <text x={cx - 20} y={cy * 2 - 10} fill={color} fontSize={10} fontWeight={700}>
        ● الواقع
      </text>
      <text x={cx + 30} y={cy * 2 - 10} fill="#3b82f6" fontSize={10} fontWeight={700}>
        ● الخطة
      </text>
    </svg>
  )
...
export function PieChartSVG({ data, height = 250, innerRadius = 0 }) {
  ...
  const arc = (sa, ea, outerR = r, innerR2 = innerRadius) => {
    ...
    if (innerR2 > 0) {
      ...
      const ix1 = cx + innerR2 * Math.cos(sa),
        iy1 = cy + innerR2 * Math.sin(sa)
      const ix2 = cx + innerR2 * Math.cos(ea),
        iy2 = cy + innerR2 * Math.sin(ea)
