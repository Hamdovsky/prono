...
const MegaCorrelation = ({ matches }) => {
  ...
  const generateTechReport = (m) => {
    ...
    if (m.weather_temp || m.enriched?.weather?.temp) {
      ...
      if (temp > 28)
        logistics.push(
          `حرارة مرتفعة (${temp}°C) قد تسبب هبوطاً بدنياً "Energy Drain" في الشوط الثاني مما يبطئ رتم اللعب أو يخلق أخطاء دفاعية متأخرة.`
        )
      else if (temp < 5)
        logistics.push(
          `درجات حرارة منخفضة (${temp}°C) تفرض تحديات على العضلات. قد نرى بداية بطيئة للمباراة.`
        )
    }
    if (m.travelFatigue) {
      logistics.push(
        `الضيف عانى من رحلة شاقة (${m.travelFatigue.distance} كم بدرجة تأثير ${m.travelFatigue.impact})، مما يزيد من احتمالية تراجع مخزونه اللياقي.`
      )
    }
    ...
    const yellowCards =
      m.referee_yellow_avg || m.enriched?.analysis_data?.referee?.strictness_index || 0
    ...
    if (m.master_v20?.is_pattern) {
      ...
      corr =
        pOu > 55
          ? 'محرك الترابط يرصد تقاطعاً إيجابياً يدعم غزراة تهديفية (Over 2.5)، مدعوماً بضعف المنظومة الدفاعية تحت الضغط.'
          : 'يدعم محرك الترابط الديناميكي فرضية الانضباط الدفاعي وتدني معدل التسجيل، مقلصاً التوجه التصاعدي للرهان الهجومي.'
    }
  ...
  const topMatches = useMemo(() => {
    ...
    return [...matches]
      ...
      .sort((a, b) => {
        const confA =
          a.confidence ||
          a.enriched?.confidence ||
          a.enriched?.main_predictions?.[0]?.probability ||
          0
        const confB =
          b.confidence ||
          b.enriched?.confidence ||
          b.enriched?.main_predictions?.[0]?.probability ||
          0
    ...
    <div className="mega-container">
      ...
      <div className="mega-list">
        {topMatches.map((match, idx) => {
          const confidence =
            match.confidence ||
            match.enriched?.confidence ||
            match.enriched?.main_predictions?.[0]?.probability ||
            50
          ...
          return (
            <div
              key={match.id}
              className={`mega-row ${isExpanded ? 'expanded' : ''} ${isElite ? 'elite-border' : ''}`}
            ...
            >
              <div className="row-main-content">
                ...
                {/* Elite Flash Badge */}
                {isElite && <div className="elite-mega-flash">⚡ ELITE</div>}
                ...
                <div className="advanced-badges">
                  ...
                  {match.enriched?.news_data?.impact?.critical?.includes(
                    '⚠️ SQUAD ROTATION DETECTED'
                  ) && (
                    <div className="sharp-pulse-alert" style={{ background: '#f59e0b' }}>
                      ⚠️ ROTATION
                    </div>
                  )}
                  {match.enriched?.news_data?.impact?.critical?.includes(
                    ...
                    <div className="sharp-pulse-alert" style={{ background: '#ef4444' }}>
                      ⚽ SCORER OUT
                    </div>
                  )}
                  ...
                  {match.enriched?.confirmed && (
                    <div className="lineup-confirmed-badge" title="Official Lineups Confirmed">
                      ✅ LINEUPS
                    </div>
                  )}
                </div>
                ...
                <div className="row-meta">
                  <span className="league-code">
                    {(match.league || '???').substring(0, 3).toUpperCase()}
                  </span>
                  <span
                ...
                <div className="row-teams">
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      {match.tacticalLabels?.includes('🛡️ DEFENSIVE PROTOCOL') && (
                        <span className="tactical-scenario-badge defensive">DEF</span>
                      )}
                      {match.tacticalLabels?.includes('🔥 REMONTADA MODE') && (
                        <span className="tactical-scenario-badge remontada">REMONTADA</span>
                      )}
                      <span className="team h">{match.homeTeam}</span>
                    </div>
                    {match.ta_ratings?.home && (
                      <div className="star-ratings">
                        {'★'.repeat(Math.floor(match.ta_ratings.home - 5))}
                        {'☆'.repeat(5 - Math.floor(match.ta_ratings.home - 5))}
                      </div>
                    )}
                  </div>
                  ...
                  >
                    ...
                    {match.travelFatigue && (
                      <div
                        className={`travel-log ${match.travelFatigue.impact.toLowerCase()}-impact`}
                      >
                ...
                <div className="row-prediction">
                  ...
                  <div className="prediction-extras">
                    {match.master_v20?.correlations?.map((c) => (
                      <span
                        key={c.type}
                        className={`node-tag ${c.type.toLowerCase()}`}
                        title={c.type}
                      >
                        {c.type === 'ELITE_RESULT'
                          ? '🎯'
                    ...
                    {(match.asian_handicap || match.enriched?.asian_handicap) && (
                      <span className="ah-line">
                        AH{' '}
                        {match.asian_handicap?.ah_suggested_line ||
                          match.enriched?.asian_handicap?.ah_suggested_line}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="row-confidence">
                  <div className="conf-bar-bg">
                    <div
                      ...
                      style={{ width: `${match.master_v20?.master_confidence || confidence}%` }}
                    ></div>
                  </div>
                  <span className="conf-val">
                    {match.master_v20?.master_confidence || confidence}%
                  </span>
                  {(match.referee_yellow_avg > 4.5 ||
                    match.enriched?.analysis_data?.referee?.strictness_index > 70) && (
                    <span
                      className="ref-warning"
                      title={`Strict Referee: ${match.referee_yellow_avg || ''} Yellows`}
                    >
                      ⚠️ REF
                    </span>
                  )}
                ...
                <div
                  className={`row-verdict ${(match.master_v20?.master_verdict || verdict || '').toLowerCase().replace(/ /g, '-')}`}
                >
                    ...
                    <div className="tech-box">
                      ...
                      <div className="tb-prob-row">
                        ...
                        <div className="tb-bar-bg">
                          <div
                            className="tb-bar-fill home"
                            style={{
                              width: `${parseProb(match.enriched?.home_win_probability || match.home_win_probability || 0)}%`,
                            }}
                          ></div>
                        </div>
                        <div className="tb-val">
                          {parseProb(
                            match.enriched?.home_win_probability || match.home_win_probability || 0
                          )}
                          %
                        </div>
                      </div>
                      <div className="tb-prob-row">
                        ...
                        <div className="tb-bar-bg">
                          <div
                            className="tb-bar-fill draw"
                            style={{
                              width: `${parseProb(match.enriched?.draw_probability || match.draw_probability || 0)}%`,
                            }}
                          ></div>
                        </div>
                        <div className="tb-val">
                          {parseProb(
                            match.enriched?.draw_probability || match.draw_probability || 0
                          )}
                          %
                        </div>
                      </div>
                      <div className="tb-prob-row">
                        ...
                        <div className="tb-bar-bg">
                          <div
                            className="tb-bar-fill away"
                            style={{
                              width: `${parseProb(match.enriched?.away_win_probability || match.away_win_probability || 0)}%`,
                            }}
                          ></div>
                        </div>
                        <div className="tb-val">
                          {parseProb(
                            match.enriched?.away_win_probability || match.away_win_probability || 0
                          )}
                          %
                        </div>
                      </div>
                    ...
                    <div className="tech-box">
                      ...
                      <div className="tb-xg-display">
                        <div className="xg-col">
                          <span className="xg-val">
                            {parseFloat(match.home_xg || match.enriched?.home_xg || 0).toFixed(2)}
                          </span>
                        ...
                        <div className="xg-col">
                          <span className="xg-val">
                            {parseFloat(match.away_xg || match.enriched?.away_xg || 0).toFixed(2)}
                          </span>
                          <span className="xg-lbl">Away xG</span>
                        </div>
                      </div>
                      <div className="tb-insight">
                        {parseFloat(match.home_xg || 0) > parseFloat(match.away_xg || 0) + 0.5
                          ? '🔥 تفوق هجومي ملحوظ للمضيف'
                          : parseFloat(match.away_xg || 0) > parseFloat(match.home_xg || 0) + 0.5
                            ? '🔥 تفوق هجومي ملحوظ للضيف'
                    ...
                    <div className="tech-box">
                      ...
                      <div className="tb-market-row">
                        ...
                        <strong
                          style={{
                            color:
                              parseProb(match.enriched?.ou_25_prob || match.ou_25_prob || 0) > 60
                                ? '#38bdf8'
                                : '#94a3b8',
                          }}
                        >
                          {parseProb(match.enriched?.ou_25_prob || match.ou_25_prob || 0)}%
                        </strong>
                      </div>
                      <div className="tb-market-row">
                        ...
                        <strong
                          style={{
                            color:
                              parseProb(match.enriched?.btts_prob || match.btts_prob || 0) > 60
                                ? '#10b981'
                                : '#ef4444',
                          }}
                        >
                          {parseProb(match.enriched?.btts_prob || match.btts_prob || 0)}%
                        </strong>
                      </div>
                      <div className="tb-market-row">
                        <span>مؤشر القوة (Power Score):</span>
                        <strong style={{ color: '#f59e0b' }}>
                          {Math.round(match.power_score || match.enriched?.power_score || 0)} / 100
                        </strong>
                      </div>
                      <div
                        className="tb-market-row"
                        style={{
                          marginTop: '8px',
                      ...
                      >
                        ...
                        <strong style={{ color: '#c084fc', fontSize: '0.8rem' }}>
                          {v20.is_pattern ? `✓ ELITE MEGA-CORRELATION` : '⚡ DYNAMIC AI LOGIC'}
                        </strong>
                      </div>
                    </div>
                  ...
                  {v20.v20_plus?.expected_match_scenario && (
                    <div className="scenario-box">
                      <div className="scenario-header">
                        🛡️ سيناريو المباراة المتوقع (Monte Carlo 10k):
                      </div>
                      <div className="scenario-body">
                        {v20.v20_plus.expected_match_scenario}
                        <span className="mode-score">
                          {' '}
                          (النتيجة الأكثر تكراراً: {v20.v20_plus.monte_carlo_mode_score})
                        </span>
                      </div>
                    </div>
                  )}
                        ...
                        .map((paragraph, index) => (
                          <p key={index} style={{ marginBottom: '8px' }}>
                            ...
                            {paragraph.split('**').map((part, i) =>
                              i % 2 !== 0 ? (
                                <strong key={i} style={{ color: '#0ea5e9' }}>
                                  {part}
                                </strong>
                              ) : (
                                part
      ...
      <div className="mega-footer">
        ...
        <div className="footer-info">
          TITANIUM V29 // CYBERDECK ULTIMATE // STRATEGIC REASONING ACTIVE
        </div>
      </div>
    </div>
  )
