...
class SquadRotationAnalyst {
  constructor() {
    this.ROTATION_THRESHOLD = 0.5 // If < 50% of Standard XI is present -> B Team
  ...
  async getStandardXISignature(teamId) {
    try {
      ...
      rows.forEach((row) => {
        ...
        if (row.home_lineup.includes(String(teamId))) {
          // This check is a bit naive but works for the logic
      ...
      const keys = database.db
        .prepare(`SELECT player_id FROM team_key_players WHERE team_id = ?`)
        .all(teamId)
      if (keys.length > 0) {
        return {
  ...
  _getDescriptionAr(status, count) {
    if (status === 'B_TEAM')
      return `الفريق يلعب بتشكيلة احتياطية (فقط ${count} أساسيين). مخاطرة عالية.`
    if (status === 'HEAVY_ROTATION')
      return `هناك تدوير كبير في التشكيلة (${count} أساسيين). الحذر مطلوب.`
...
export = new SquadRotationAnalyst()
