...
const EliteROITracker = () => {
    ...
    return (
      <div className="roi-tracker-container">
        <div className="roi-tracker-bar">
          ...
          <span className="roi-value" style={{ color: '#8b949e' }}>
            ⚠️ {error}
          </span>
        </div>
      </div>
  ...
  return (
    <div className="roi-tracker-container">
      <div
        className="roi-tracker-bar"
        onClick={() => setExpanded((e) => !e)}
        style={{ cursor: 'pointer' }}
      >
        <span className="roi-label">📊 TRACK RECORD</span>
        <span className="roi-stat">{total_bets} paris</span>
        <span className="roi-stat">
          {won}W/{lost}L
        </span>
        <span className="roi-stat" style={{ color: win_rate >= 50 ? '#3fb950' : '#d29922' }}>
          {win_rate}%
        </span>
          ...
          <div className="roi-signal-strip">
            {by_signal.solid.count > 0 && (
              <span className="roi-signal-item solid">
                ⚡ SOLID: {by_signal.solid.count} | ROI {by_signal.solid.roi >= 0 ? '+' : ''}
                {by_signal.solid.roi}%
              </span>
            )}
            {by_signal.value_bet.count > 0 && (
              <span className="roi-signal-item valuebet">
                🎯 VALUE BET: {by_signal.value_bet.count} | ROI{' '}
                {by_signal.value_bet.roi >= 0 ? '+' : ''}
                {by_signal.value_bet.roi}%
              </span>
            )}
            {by_signal.dynamic.count > 0 && (
              <span className="roi-signal-item dynamic">DYNAMIC: {by_signal.dynamic.count}</span>
            )}
          </div>
          ...
          <div className="roi-match-list">
            {matches.map((m) => (
              <div key={m.id} className={`roi-match-row ${m.result}`}>
                ...
                <span className="roi-match-teams">
                ...
                <span className={`roi-match-result ${m.result}`}>
                  {m.result === 'won' ? '✅ +' : '❌ '}
                  {m.profit > 0 ? m.profit.toFixed(2) : m.profit.toFixed(2)}
                </span>
              </div>
            ))}
          </div>
