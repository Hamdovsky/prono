      ...
      for (const match of upcomingMatches) {
        try {
          ...
          const closingH = parseFloat(
            homeChoice.fractionalValue
              ? this._fractionalToDecimal(homeChoice.fractionalValue)
              : homeChoice.value
          )
          const closingD = parseFloat(
            drawChoice.fractionalValue
              ? this._fractionalToDecimal(drawChoice.fractionalValue)
              : drawChoice.value
          )
          const closingA = parseFloat(
            awayChoice.fractionalValue
              ? this._fractionalToDecimal(awayChoice.fractionalValue)
              : awayChoice.value
          )
          ...
          logger.info(
            `✅ [CLV] Captured for ${match.homeTeam}: Open ${openingH.toFixed(2)} -> Close ${closingH.toFixed(2)} | CLV: ${(clvValue * 100).toFixed(2)}%`
          )
