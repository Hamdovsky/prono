...
class Persistence {
  ...
  getMatch(matchId) {
    const row = this.db
      .prepare('SELECT id, fullData, odds_home, odds_draw, odds_away FROM matches WHERE id = ?')
      .get(matchId)
    if (!row) return null
    try {
  ...
  getOpeningOdds(matchId) {
    const row = this.db
      .prepare('SELECT odds_home_open, odds_draw_open, odds_away_open FROM matches WHERE id = ?')
      .get(matchId)
    return row || null
  }
}
