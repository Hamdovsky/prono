...
function ErrorDistribBar({ distrib }) {
  ...
  return (
    <svg
      viewBox={`0 0 600 ${data.length * h + 20}`}
      style={{ width: '100%', height: data.length * h + 20 }}
    >
      {data.map((d, i) => {
        const bw = (d.count / maxCount) * 400
        return (
          <g key={i}>
            <text x={5} y={i * h + h * 0.7} fill="#94a3b8" fontSize={10}>
              {d.name}
            </text>
            <rect
              x={150}
              ...
              fill={d.color}
            />
            <text x={155 + bw} y={i * h + h * 0.7} fill={d.color} fontSize={10}>
              {d.count}
            </text>
          </g>
        )
...
const RULE_TL = {
  ...
  downgrade_form_weight_in_next_match_same_team:
    'Dégrader le poids de la forme pour le prochain match',
}
...
function RulesTable({ rules }) {
  if (!rules?.length)
    return <div className="ld-empty">Aucune règle décisionnelle détectée pour le moment.</div>
  return (
    <table className="ld-rules-table" dir="ltr">
      ...
      <tbody>
        {rules.map((r, i) => (
          <tr key={i}>
            <td>
              <span className="ld-rule-type">{tl(r.rule_type)}</span>
            </td>
            <td className="ld-rule-cond" dir="ltr" style={{ textAlign: 'left' }}>
              {tl(r.condition)}
            </td>
            <td className="ld-rule-action">{tl(r.action)}</td>
            <td>
              <span
                className="ld-conf-badge"
                style={{
                  color:
                    r.confidence >= 0.7 ? '#10b981' : r.confidence >= 0.55 ? '#f59e0b' : '#ef4444',
                }}
              >
            ...
            <td>
              <span className="ld-hit-badge">{r.hit_count || 1}</span>
            </td>
          </tr>
        ))}
      </tbody>
...
function CasesFeed({ cases, isHistory = false }) {
  if (!cases?.length)
    return <div className="ld-empty">No cases stored yet. Run the engine on finished matches.</div>
  return (
        ...
        return (
          <div key={i} className={`ld-case-card ${isCorrect ? 'correct' : 'wrong'}`}>
            <div className="ld-case-header">
              <span className="ld-case-teams">
                {c.home_team} <span className="ld-vs">vs</span> {c.away_team}
              </span>
            ...
            <div className="ld-case-meta">
              ...
              {c.match_date && (
                <span className="ld-case-date">📅 {c.match_date.split('T')[0]}</span>
              )}
            ...
            <div className="ld-case-analysis">
              ...
              {c.context && <div className="ld-case-context">{c.context}</div>}
            </div>
            {tags.length > 0 && (
              <div className="ld-case-tags">
                {tags.map((t, j) => (
                  <TagBadge key={j} tag={t} />
                ))}
              </div>
            )}
          </div>
    ...
    <div className="ld-root">
      ...
      <div className="ld-header">
        ...
        <div className="ld-header-actions">
          ...
          <button className="ld-btn ld-btn-secondary" onClick={() => fetchReport(selectedLeague)}>
            🔄 Refresh
          </button>
        </div>
      </div>

      {processMsg && <div className="ld-process-msg">{processMsg}</div>}
      ...
      <div className="ld-kpis">
        ...
        <div className="ld-kpi" style={{ '--kc': '#6366f1' }}>
          <div className="ld-kpi-value" style={{ color: '#a5b4fc' }}>
            {totalCases}
          </div>
                  ...
                  {Object.entries(weights)
                    ...
                    .map(([k, v]) => (
                      <div key={k} className="ld-weight-row">
                        <span className="ld-weight-label">{FEATURE_LABELS[k] || k}</span>
                        <div className="ld-weight-bar-wrap">
                          <div
                            className="ld-weight-bar"
            ...
            <div className="ld-panel">
              <div className="ld-panel-title">
                ...
                <span className="ld-panel-sub">
                  Générées automatiquement à partir des motifs d'erreur récurrents
                </span>
              </div>
      ...
      {!loadingReport && !report && !error && (
        <div className="ld-empty-state">
          ...
          <p>
            Click <strong>Auto-Learn Finished Matches</strong> to start the engine.
          </p>
        </div>
      )}
    </div>
