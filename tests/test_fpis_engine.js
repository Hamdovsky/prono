...
const isDoublePred =
  s2.adjusted_prediction.includes('or Draw') || s2.adjusted_prediction.includes('Double')
assert(
  isDoublePred || s2.adjusted_prediction.length > 3,
  'Prediction expanded safely for volatility'
)
...
assert(
  typeof s5.match_type === 'string' && s5.match_type.length > 0,
  'Match type returned even with null data'
)
