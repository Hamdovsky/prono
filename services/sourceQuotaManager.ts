...
class SourceQuotaManager {
  constructor(source) {
    this.source = source
    this.limit = parseInt(
      process.env[ENV_LIMITS[source]] || String(DEFAULT_LIMITS[source] || 20),
      10
    )
  ...
  canProcessMatch(matchId) {
    ...
    logger.warn(
      `[${this.source.toUpperCase()}] Daily limit of ${this.limit} matches reached. Skipping ${id}.`
    )
    return false
  }
  ...
  registerMatch(matchId) {
    try {
      ...
      this._save(usage)
      logger.info(
        `[${this.source.toUpperCase()}] Match ${id} registered. Quota: ${usage.count}/${this.limit}.`
      )
      return usage.count
    } catch (e) {
