...
const G = (p) => ({ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: p.bg || 'rgba(255,255,255,0.03)', border: p.border || '1px solid rgba(255,255,255,0.06)', borderRadius: '8px', padding: p.pad || '6px 4px', minHeight: p.h || '56px', gap: p.gap || '2px', ...(p.sx || {}) })

const L = ({ c, s, w }) => <span style={{ fontSize: s || '8px', fontWeight: w || '700', color: c || '#64748b', letterSpacing: '0.3px', textTransform: 'uppercase', lineHeight: 1.2 }}>{c}</span>
const V = ({ c, s, w }) => <span style={{ fontSize: s || '16px', fontWeight: w || '900', color: c || '#f8fafc', fontFamily: "'JetBrains Mono', monospace", lineHeight: 1.2 }}>{c}</span>
const Pct = ({ v, c }) => <span style={{ fontSize: '11px', fontWeight: '800', color: c || '#94a3b8', fontFamily: "'JetBrains Mono', monospace" }}>{v}%</span>
...
const toScore = (s) => {
    ...
    const [h, a] = String(s).split('-').map(v => parseInt(v.trim()));
    if (!Number.isFinite(h) || !Number.isFinite(a)) return null;
    return { home: h, away: a, total: h + a };
}

const ACC_COLORS = {
    high: '#00ffaa', med: '#fbbf24', low: '#f87171'
}
...
const MatchRow = ({ match, isElite, onClick, style, now }) => {
    ...
    const getCS = () => {
        ...
        if (match.v22_cs_prediction) { const p = match.v22_cs_prediction.split(' - ')[0]; if (p?.includes('-')) return p }
        ...
        if (es?.includes('-')) { const [h, a] = es.split('-').map(v => parseInt(v.trim())); if (!isNaN(h) && !isNaN(a) && (h + a) > 0) return es }
    ...
    const alignCS = (s, pick) => {
        ...
        const [h, a] = p; const hi = over25Pct > 60 || bttsPct > 62
        const wH = pick === '1' || pick.includes('HOME'); const wA = pick === '2' || pick.includes('AWAY'); const wD = pick === 'X' || pick.includes('DRAW') || pick === 'NUL'
        if (wH && h <= a) return hi ? `${a + 1} - ${a}` : `${Math.max(1, a)} - ${Math.max(0, a - 1)}`
        if (wA && a <= h) return hi ? `${h} - ${h + 1}` : `${Math.max(0, h - 1)} - ${Math.max(1, h)}`
        if (wD && h !== a) { const dg = Math.round((h + a) / 2); return `${dg} - ${dg}` }
        return s
    }
    ...
    const mainPickClean = (quantObj?.main_pick || '').replace(/🛡️|⚽|⚡|🔥|🏠|✈️|AH_|EH_|COMBOS: |SMART VALUE: /g, '').trim()
    ...
    if (isNoData) { ms = null; msColor = '#475569'; msDesc = '⏳' }
    else if (match.insufficient_data === 1) { ms = Math.max(1, 2 + Math.floor(dataBonus / 3)); msColor = '#f59e0b'; msDesc = '⚠️ Faible' }
    else if (isBalanced) { ms = Math.min(8, 4 + Math.floor(dataBonus / 2)); msColor = '#38bdf8'; msDesc = '🔵 Balance' }
    else if (domProb >= 70) { ms = Math.min(10, 7 + Math.floor(dataBonus / 2)); msColor = '#00ffaa'; msDesc = '🟢 Haute' }
    else if (domProb >= 55) { ms = Math.min(9, 5 + Math.floor(dataBonus / 2)); msColor = '#fbbf24'; msDesc = '🟡 Modéré' }
    else { ms = Math.min(7, 3 + Math.floor(dataBonus / 2)); msColor = '#f97316'; msDesc = '🟠 Spéculatif' }
    ...
    if ((status === 'finished' || status === 'ft' || status === 'ended') && match.scoreHome !== null && match.scoreAway !== null) {
        actualScoreDisplay = `${match.scoreHome} - ${match.scoreAway}`
    }

    return (
        <div style={{ ...style, display: 'flex', flexDirection: 'column', gap: '6px', background: isElite ? 'linear-gradient(135deg, rgba(0,255,170,0.06), #030712)' : '#080c14', border: `1px solid ${isElite ? 'rgba(0,255,170,0.2)' : 'rgba(255,255,255,0.06)'}`, borderRadius: '10px', padding: '8px 10px', cursor: 'pointer' }} onClick={() => onClick(match)}>
            {/* HEADER: Match + League + Time */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', minHeight: '20px' }}>
                <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: countdownStr.includes('EN COURS') ? '#ef4444' : '#22c55e', flexShrink: 0 }} />
                <span style={{ fontSize: '9px', fontWeight: '800', color: '#fbbf24', fontFamily: "'JetBrains Mono', monospace", background: 'rgba(251,191,36,0.1)', padding: '1px 4px', borderRadius: '3px' }}>{formattedTime}</span>
                {countdownStr && !countdownStr.includes('EN COURS') && <span style={{ fontSize: '8px', fontWeight: '700', color: '#38bdf8', fontFamily: "'JetBrains Mono', monospace" }}>-{countdownStr}</span>}
                {countdownStr.includes('EN COURS') && <span style={{ fontSize: '8px', fontWeight: '900', color: '#ef4444', animation: 'pulse 1.5s infinite' }}>🔴 LIVE</span>}
                <span style={{ fontSize: '9px', fontWeight: '700', color: '#94a3b8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, minWidth: 0 }}>{match.league || 'N/A'}</span>
                ...
                {hForm > 0 && <span style={{ fontSize: '8px', fontWeight: '900', color: hForm >= 70 ? '#00ffaa' : '#fbbf24' }}>{Math.round(hForm)}</span>}
                {aForm > 0 && <span style={{ fontSize: '8px', fontWeight: '900', color: aForm >= 70 ? '#00ffaa' : '#fbbf24' }}>{Math.round(aForm)}</span>}
                {actualScoreDisplay && <span style={{ fontSize: '12px', fontWeight: '900', color: '#f8fafc', fontFamily: "'JetBrains Mono', monospace", marginLeft: 'auto' }}>{actualScoreDisplay}</span>}
            </div>

            {/* GRID: 6 independent boxes (3 cols desktop, 2 cols mobile) */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px' }}>
                {/* BOX 1: BASE 1X2 + Confidence */}
                <div style={G({ bg: 'rgba(0,255,170,0.05)', border: '1px solid rgba(0,255,170,0.12)', gap: '2px' })}>
                    <L c="#00ffaa" s="7px" w="900">BASE 1X2</L>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                        <V c="#f8fafc" s="18px">{mainPickClean}</V>
                        <V c={accColor(acc)} s="14px">{acc}%</V>
                    </div>
                    <div style={{ width: '100%', height: '2px', background: 'rgba(148,163,184,0.1)', borderRadius: '2px', overflow: 'hidden' }}>
                        <div style={{ width: `${Math.max(5, acc)}%`, height: '100%', background: accColor(acc), borderRadius: '2px' }} />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        {mainOdds && <span style={{ fontSize: '9px', color: '#fbbf24', fontWeight: '700', fontFamily: "'JetBrains Mono', monospace" }}>@{mainOdds.toFixed(2)}</span>}
                        <span style={{ fontSize: '8px', color: mainPickEdge > 0 ? '#fbbf24' : '#f87171', fontWeight: '800' }}>{mainPickEdge > 0 ? `🎯+${mainPickEdgePct}%` : `⚠️${mainPickEdgePct}%`}</span>
                    </div>
                </div>

                {/* BOX 2: AI SCORE */}
                <div style={G({ bg: match.insufficient_data === 1 ? 'rgba(245,158,11,0.06)' : 'rgba(0,255,170,0.04)', border: `1px solid ${match.insufficient_data === 1 ? 'rgba(245,158,11,0.15)' : 'rgba(0,255,170,0.1)'}` })}>
                    <L c={match.insufficient_data === 1 ? '#f59e0b' : '#00ffaa'} s="7px" w="900">SCORE IA</L>
                    <V c={match.insufficient_data === 1 ? '#f59e0b' : '#00ffaa'} s="18px">{match.insufficient_data === 1 ? '⏳' : cs}</V>
                </div>

                {/* BOX 3: BTTS */}
                <div style={G({ bg: 'rgba(239,68,68,0.04)', border: `1px solid ${scoreBtts ? 'rgba(0,255,170,0.12)' : 'rgba(239,68,68,0.15)'}` })}>
                    <L c={scoreBtts ? '#00ffaa' : '#f87171'} s="7px" w="900">BTTS</L>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                        <V c={scoreBtts ? '#00ffaa' : '#f87171'} s="18px">{scoreBtts ? 'OUI' : 'NON'}</V>
                        <Pct v={scoreBtts ? bttsPct : 100 - bttsPct} c={scoreBtts ? '#00ffaa' : '#f87171'} />
                    </div>
                </div>

                {/* BOX 4: O/U 2.5 */}
                <div style={G({ bg: 'rgba(16,185,129,0.04)', border: `1px solid ${scoreTotal >= 3 ? 'rgba(16,185,129,0.15)' : 'rgba(148,163,184,0.1)'}` })}>
                    <L c={scoreTotal >= 3 ? '#10b981' : '#94a3b8'} s="7px" w="900">O/U 2.5</L>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                        <V c={scoreTotal >= 3 ? '#10b981' : '#94a3b8'} s="18px">{scoreTotal >= 3 ? 'OVER' : 'UNDER'}</V>
                        <Pct v={scoreTotal >= 3 ? over25Pct : 100 - over25Pct} c={scoreTotal >= 3 ? '#10b981' : '#94a3b8'} />
                    </div>
                </div>

                {/* BOX 5: HT +0.5 */}
                <div style={G({ bg: 'rgba(251,191,36,0.04)', border: `1px solid ${htGoalPct >= 65 ? 'rgba(0,255,170,0.12)' : htGoalPct >= 50 ? 'rgba(251,191,36,0.15)' : 'rgba(239,68,68,0.12)'}` })}>
                    <L c={htGoalPct >= 65 ? '#00ffaa' : htGoalPct >= 50 ? '#fbbf24' : '#f87171'} s="7px" w="900">HT +0.5</L>
                    <V c={htGoalPct >= 65 ? '#00ffaa' : htGoalPct >= 50 ? '#fbbf24' : '#f87171'} s="18px">{htGoalPct}%</V>
                </div>

                {/* BOX 6: RISK / EV / FORCE */}
                <div style={G({ bg: 'rgba(148,163,184,0.04)', border: `1px solid ${msColor}33`, gap: '2px' })}>
                    <L c="#64748b" s="7px" w="900">RISK / EV / FORCE</L>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ fontSize: '13px', fontWeight: '900', color: msColor || '#64748b', fontFamily: "'JetBrains Mono', monospace" }}>{msDesc}</span>
                        <span style={{ fontSize: '11px', fontWeight: '800', color: evNum > 0 ? '#10b981' : evNum < 0 ? '#ef4444' : '#94a3b8', fontFamily: "'JetBrains Mono', monospace" }}>EV {quantObj?.ev_score || '0.00'}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        {ms && <div style={{ width: '40px', height: '3px', background: 'rgba(148,163,184,0.1)', borderRadius: '3px', overflow: 'hidden' }}><div style={{ width: `${ms * 10}%`, height: '100%', background: msColor, borderRadius: '3px' }} /></div>}
                        <span style={{ fontSize: '9px', fontWeight: '900', color: parseFloat(valueScore) >= 5 ? '#00ffaa' : parseFloat(valueScore) >= 2 ? '#fbbf24' : '#64748b' }}>🏆{valueScore}</span>
                    </div>
                </div>
            </div>
