...
try {
  import db from '../stitch/core/database'
  ...
  const row3 = db.db
    .prepare('SELECT COUNT(*) as cnt FROM matches WHERE home_win_probability > 0')
    .get()
  ...
  const nullPred = db.db
    .prepare(
      "SELECT COUNT(*) as cnt FROM matches WHERE prediction IS NULL OR prediction = 'UNDER ANALYSIS'"
    )
    .get()
  ...
  const defaultScore = db.db
    .prepare("SELECT COUNT(*) as cnt FROM matches WHERE expected_score = '1 - 1'")
    .get()
    ...
    const req = http.get({ hostname: '127.0.0.1', port, path, timeout: 3000 }, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ok(`${label} → HTTP ${res.statusCode}`)
          resolve(true)
        } else {
          warn(`${label} → HTTP ${res.statusCode}`)
          resolve(false)
        }
      })
    })
...
async function runEnrichTest() {
  try {
    import ep from '../stitch/core/enriched_predictions'
    const mockMatch = {
      id: 'TEST-001',
    ...
    if (result && result.home_win_probability > 0) {
      ok(
        `fastEnrichMatch → Home:${result.home_win_probability}% Draw:${result.draw_probability}% Away:${result.away_win_probability}%`
      )
...
`,
    ...
    py.on('close', () => {
      ...
      if (errOut && !errOut.includes('Traceback'))
        warn(`Python stderr: ${errOut.substring(0, 200)}`)
      resolve()
    })
    py.on('error', () => {
