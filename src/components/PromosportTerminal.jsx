...
const PromosportTerminal = ({ matches, onGenerateReduced }) => {
  ...
  useEffect(() => {
    if (matches?.length > 0) {
      ...
      const real = matches.filter(
        (m) => m.home && m.away && m.home !== 'Home' && m.away !== 'Away'
      ).length
      logsRef.current = [
        ...logsRef.current.slice(-19),
  ...
  const logs =
    logsRef.current.length > 0
      ? logsRef.current
      : ['SYSTEME EN ATTENTE DE DONNEES...', 'Veuillez patienter pendant le chargement des matchs']
        ...
        <div className="score-matrix-overlay" onClick={() => setSelectedMatch(null)}>
          <div className="score-matrix-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h4>
                PROBABILITES: {selectedMatch.home} vs {selectedMatch.away}
              </h4>
            ...
            return (
              ...
              >
                ...
                <td style={{ fontWeight: 'bold' }}>
                  <span>
                    {m.home?.toUpperCase()} v {m.away?.toUpperCase()}
                  </span>
                </td>
                <td>
                  <div className="prob-cell">
                    ...
                    <div className="prob-bar-bg">
                      <div
                        ...
                        style={{ width: `${h}%`, background: h > 50 ? '#34d399' : '#fbbf24' }}
                      />
                    </div>
                  </div>
                </td>
                ...
                <td>
                  <div style={{ display: 'flex', gap: '5px' }}>{getSignalTag(m)}</div>
                </td>
              </tr>
            )
          })}
      ...
      <div className="terminal-controls">
        <div className="control-group">
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button className="pro-btn" onClick={() => onGenerateReduced('N-1')}>
              REDUIT N-1
            </button>
            <button className="pro-btn" onClick={() => onGenerateReduced('N-2')}>
              REDUIT N-2
            </button>
          </div>
        </div>
