...
const PerformanceHub = ({ matches }) => {
    const stats = useMemo(() => {
        ...
        finished.forEach(m => {
            ...
            if (
                pick.includes('risky bet') || pick.includes('skip') ||
                pick.includes('no bet') || pick.includes('البطاقات') ||
                pick.includes('cards') || pick.includes('corner') ||
            ...
            if (isVerifiable) {
                verifiableCount++;
                if (isWin) { wins++; totalProfit += 0.85; }
                else { totalProfit -= 1; }
        ...
        return {
            total: verifiableCount, wins, winRate, roi,
    ...
    const cardLabel = {
        ...
        textTransform: 'uppercase', color: '#2d3f56',
        display: 'block', marginBottom: '8px',
    };
    const bigNumber = (color) => ({
        fontSize: '30px', fontWeight: '900', color,
        lineHeight: 1, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '-1px',
    });
    ...
    const avgConf = Math.round(
        matches.reduce((acc, m) => acc + (m.v22_success_rate || m.confidence || 0), 0) / (matches.length || 1)
    );
    ...
    return (
        <div style={{
            ...
            <div className="onyx-stat-card" style={{ ...cardBase, border: '1px solid rgba(56,189,248,0.18)' }}>
                ...
                <div style={{
                    fontSize: '12.5px', fontWeight: '800', color: '#38bdf8',
                    marginTop: '8px', lineHeight: 1.35, textTransform: 'uppercase', letterSpacing: '0.3px',
                }}>
