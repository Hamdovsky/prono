...
async function fetchTeamStats(teamId, uniqueTournamentId, seasonId) {
  ...
  try {
    ...
    const statsObj = {
      ...
      ppdaProxy: (function () {
        const denom =
          (s.tackles || 0) + (s.interceptions || 0) + (s.clearances || 0) + (s.fouls || 0)
        return denom > 0 ? +((s.totalPasses || 0) / denom).toFixed(2) : 15.0
      })(),
      progressivePassesProxy: +(
        ((s.accurateLongBalls || 0) + (s.accurateOppositionHalfPasses || 0)) /
        mp
      ).toFixed(2),
      shotCreatingActionsProxy: +(
        ((s.bigChancesCreated || 0) + (s.accurateCrosses || 0) + (s.successfulDribbles || 0)) /
        mp
      ).toFixed(2),
    }
    ...
    console.warn(
      `⚠️ [V90 FALLBACK] Sofascore API failed for Team ${teamId}. Marking as insufficient data.`
    )
    return {
      insufficient_data: true,
      matchesPlayed: 0,
...
class Workflow {
  ...
  _httpFixtureToEvent(m) {
    const id =
      parseInt(String(m.id).replace(/\D/g, '').slice(0, 8)) || Math.floor(Math.random() * 1000000)
    return {
      id,
  ...
  async start() {
    ...
    try {
      ...
      if (!needsFullScan) {
        console.log(
          `📡 [INCREMENTAL] Recent full scan found (${Math.round((now - state.lastFullScan) / 1000 / 60)}m ago). Scanning only Yesterday/Today/Tomorrow.`
        )
      } else {
      ...
      if (needsFullScan && shouldUseSofa()) {
        ...
        for (const fl of forceLeagues) {
          try {
            ...
            if (seasonsRes && seasonsRes.seasons && seasonsRes.seasons.length > 0) {
              ...
              const nextRes = await SofaAPI.getTournamentEvents(
                fl.sofascoreId,
                currentSeasonId,
                'next'
              )
              const lastRes = await SofaAPI.getTournamentEvents(
                fl.sofascoreId,
                currentSeasonId,
                'last'
              )
        ...
        console.log(
          '🎯 [V54] Skipping forced priority sync (Incremental Mode) to save time/bandwidth...'
        )
      }
      ...
      for (const event of allEvents) {
        ...
        const lgConfig = Object.values(LEAGUE_MAP).find(
          (l) =>
      ...
      if (targetMatches.length < 10) {
        ...
        console.log(
          `📊 [V54] Total events: ${allEvents.length}, kept: ${targetMatches.length}, filtered out: ${skipped}`
        )
      }
      ...
      allEvents.forEach((event) => {
        ...
        if (cName.includes('morocco') || cName.includes('saudi')) {
          console.log(
            `🔍 [DEBUG] RAW DETECT: ${event.homeTeam?.name} vs ${event.awayTeam?.name} | Category: ${event.tournament?.category?.name} | Tournament: ${event.tournament?.name}`
          )
        }
      })
      ...
      if (process.env.RAPIDAPI_ENABLED === 'true') {
        console.log(
          '🏆 [PRIORITY] Sorting matches by League Tier and Start Time to optimize RapidAPI quota...'
        )
      ...
      const processMatch = async (match) => {
        ...
        try {
          ...
          if (existing) {
            console.log(
              `🔄 [REFRESH] ${match.homeTeam} vs ${match.awayTeam} (${match.status}) - Refreshing data...`
            )
          } else {
          ...
          const [refStats, oddsData, newsIntel] = await Promise.all([
            rootEvent?.referee?.id
              ? SofaAPI.getRefereeStats(rootEvent.referee.id)
          ...
          if (matchLineups) {
            ...
            if (match.lineups_confirmed) {
              console.log(
                `✅ [LINEUPS] Official confirmed squads detected for ${match.home} vs ${match.away}`
              )
            }
            try {
              if (matchLineups.home && matchLineups.home.players) {
                // ⚡ [SYNC FIX] Await props scraping to prevent hidden socket pileup
                await PlayerPropsScraper.processTeamLineupForProps(
                  matchLineups.home.players,
                  match.homeTeam?.name,
                  match._uniqueTournament,
                  match._seasonId
                )
              }
          ...
          if (
            (homeStats && homeStats.insufficient_data) ||
            (awayStats && awayStats.insufficient_data)
          ) {
            match.insufficient_data = 1
            console.log(
              `⚠️ [INTEGRITY] Marking ${match.home} vs ${match.away} as INSUFFICIENT DATA.`
            )
          } else {
          ...
          match.home_xg = homeStats
            ? homeStats.expectedGoals / (homeStats.matchesPlayed || 1)
            : null
          match.away_xg = awayStats
            ? awayStats.expectedGoals / (awayStats.matchesPlayed || 1)
            : null
          ...
          if (homeFormAvg) {
            console.log(
              `📈 [FORM] ${match.homeTeam}: xG=${homeFormAvg.tw_xG_scored} goals=${homeFormAvg.avgGoals} w%=${homeFormAvg.winRate}`
            )
          }
          if (awayFormAvg) {
            console.log(
              `📈 [FORM] ${match.awayTeam}: xG=${awayFormAvg.tw_xG_scored} goals=${awayFormAvg.avgGoals} w%=${awayFormAvg.winRate}`
            )
          }
          ...
          try {
            ...
            match.v70_analytics = {
              discipline: AdvancedAnalyticsEngine.calculateDisciplineConflict(
                homeStats,
                awayStats,
                match.referee_yellow_avg,
                match.referee_red_avg
              ),
              rest: AdvancedAnalyticsEngine.calculateRestMismatch(
                homeForm,
                awayForm,
                match.startTimestamp || Date.now() / 1000
              ),
              xg_regression: {
          ...
          if (oddsData?.featured) {
            ...
            if (market?.choices) {
              ...
              if (opening && opening.odds_home_open) {
                const diff =
                  ((match.odds_home - opening.odds_home_open) / opening.odds_home_open) * 100
                if (Math.abs(diff) >= 10) {
                  const direction = diff < 0 ? '⬇️ DROPPING' : '⬆️ RISING'
                  console.log(
                    `♨️  [STEAM] ${match.home} Odds: ${opening.odds_home_open.toFixed(2)} -> ${match.odds_home.toFixed(2)} (${direction} ${Math.abs(diff).toFixed(1)}%)`
                  )
              ...
              if (match.odds_home) {
                console.log(
                  `💰 [ODDS] ${match.home} vs ${match.away}: H=${match.odds_home} D=${match.odds_draw} A=${match.odds_away}`
                )
              }
            }
          }
          ...
          try {
            ...
            if (audit && audit.home_win_probability > 0) {
              ...
              const poissonProb = (lambda, k) =>
                (Math.pow(lambda, k) * Math.exp(-lambda)) / [1, 1, 2, 6, 24, 120][k]
              let pH = 0,
                pD = 0,
                pA = 0
              for (let h = 0; h <= 5; h++) {
              ...
              match.prediction =
                match.home_win_probability > match.away_win_probability
                  ? `${match.homeTeam} Win`
                  : match.away_win_probability > match.draw_probability
                    ? `${match.awayTeam} Win`
                    : 'Draw'
              match.power_score = Math.round(
                Math.max(match.home_win_probability, match.away_win_probability) * 0.8
              )
          ...
          try {
            const isCup = /cup|coupe|pokal|copa|trophy|final|play-off|relegation/i.test(
            ...
            if (lgStr.includes('premier league')) {
              hMkt = 500
              aMkt = 450
            } else if (lgStr.includes('championship')) {
              hMkt = 120
              aMkt = 100
            } else if (
              /league\s+one|league\s+1|\bL1\b/i.test(lgStr) ||
              /league\s+one|league\s+1|\bL1\b/i.test(trnStr)
            ) {
              hMkt = 30
              aMkt = 28
              console.log(
                `💎 [V47] League One Detected: ${match.home} vs ${match.away} | LG: ${match.league} | SETTING 30M€`
              )
            } else if (
              /(national|vanarama|eng.5)/gi.test(lgStr) ||
              /(national|vanarama|eng.5)/gi.test(trnStr)
            ) {
              hMkt = 15
              aMkt = 14
              console.log(
                `🛡️ [V47] National League Detected: ${match.home} vs ${match.away} | LG: ${match.league} | SETTING 15M€`
              )
            } else if (lgStr.includes('saudi')) {
              hMkt = 250
              aMkt = 200
            } else if (lgStr.includes('egyptian')) {
              hMkt = 80
              aMkt = 60
            }
          ...
          try {
            ...
            if (audit.isSuspicious) {
              console.log(
                `🕵️‍♂️ [INTEGRITY ALERT] ${match.homeTeam} vs ${match.awayTeam}: ${audit.recommendation} (Score: ${audit.score})`
              )
            }
          } catch (integrityErr) {
          ...
          try {
            ...
            console.log(
              `🧬 [EVOLUTION] Meta-Refined: H=${match.home_win_probability.toFixed(1)}% | Calibrated Conf: ${match.confidence.toFixed(1)}%`
            )
          } catch (evolutionErr) {
          ...
          console.log(
            `⏱️ [PERF] Traitement ${match.homeTeam || match.home} terminé en ${duration.toFixed(2)}ms`
          )
