...
const profile = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'data', 'crowd_profile.json'), 'utf-8'))
const voteHistory = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'data', 'tunisian_vote_history.json'), 'utf-8'))
...
╚══════════════════════════════════════════════╝
`)
console.log(`Global:  ${profile.tunisianCrowd.crowdRight}/${profile.tunisianCrowd.totalMatches} = ${profile.tunisianCrowd.crowdAccuracy}%`)
console.log(`
...
bins.forEach(([key, d]) => {
  ...
  console.log(`  ${key.padEnd(12)} ${String(d.right).padStart(2)}/${String(d.total).padStart(3)}${' '.repeat(10-String(d.total).length)} ${acc}%${' '.repeat(9)}${action}`)
})

// Key insight
const weak = bins.filter(([k]) => parseInt(k) < 70).reduce((s, [, d]) => ({ r: s.r + d.right, t: s.t + d.total }), { r: 0, t: 0 })
const strong = bins.filter(([k]) => parseInt(k) >= 70).reduce((s, [, d]) => ({ r: s.r + d.right, t: s.t + d.total }), { r: 0, t: 0 })
