    ...
    async () => {
      ...
      try {
        ...
        if (openligadbService.isAvailable()) {
          sources.push(
            timedFetch(openligadbService.fetchEvents(dateStr), 'openligadb').catch(() => [])
          )
        }
      } catch (_) {}
  ...
  await runTask(
    ...
    async () => {
      ...
      try {
        ...
        const MAIN_URL = process.env.MAIN_SERVER_URL || 'https://pronostico.onrender.com'
        await axios.post(
          `${MAIN_URL}/api/invalidate-cache`,
...
app.post('/sync/goalmodel', requireAuth, async (req, res) => {
  await runTask(
    ...
    async () => {
      ...
      const fastApiUrl =
        process.env.INFERENCE_URL || process.env.FASTAPI_URL || 'http://127.0.0.1:8000'
      ...
      try {
        ...
        if (db) {
          ...
          for (const tbl of tables) {
            const hasTbl = db
              .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='${tbl}'`)
              .get()
            if (!hasTbl) continue
            ...
            const leagues =
              leagueFilter.length > 0
                ...
                : db
                    .prepare(
                      `SELECT league FROM ${tbl} WHERE scoreHome IS NOT NULL AND scoreHome > 0 AND scoreAway IS NOT NULL GROUP BY league HAVING COUNT(*) >= 5 ORDER BY COUNT(*) DESC LIMIT 50`
                    )
                    .all()
                    .map((r) => r.league)
  ...
  try {
    ...
    try {
      ...
      const r3 = await conn.query(
        'SELECT COUNT(*) as cnt FROM matches WHERE "fullData"::text LIKE \'%TITANIUM_QUANT_V4%\''
      )
      const r4 = await conn.query(
        'SELECT COUNT(*) as cnt FROM matches WHERE "fullData"::text LIKE \'%TITANIUM_ELITE_V3%\''
      )
...
app.post('/test-update', requireAuth, async (req, res) => {
  try {
    ...
    // Enrich one match
    const enriched = await enrichedPredictions.enrichMatches([testMatch], {
      fastMode: false,
      force: true,
    })
    const m = enriched[0]
    ...
    const r = await conn.query(
      'SELECT "fullData", home_win_probability, ai_source FROM matches WHERE id = $1',
      [m.id]
    )
    const row = r.rows?.[0]
    let fullData_ai_source = null,
      fullData_hwp = null
    if (row?.fullData) {
      try {
