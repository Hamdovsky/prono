...
function loadOddsHistory() {
  if (fs.existsSync(ODDS_FILE)) {
    try {
      return JSON.parse(fs.readFileSync(ODDS_FILE, 'utf8'))
    } catch (_) {}
  }
  return {}
}
...
function detectBookmakerTrap(matchId, aiWinProb, expectedWinner, currentOdds) {
  ...
  return {
    isTrap,
    severity: severity ? parseFloat(severity.toFixed(1)) : 0,
    msg,
    shiftPct: severity,
  }
}

export = { snapshotOdds, getSteamForMatch, get24hMovement, detectBookmakerTrap }
