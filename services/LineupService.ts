...
class LineupService {
  constructor() {
    this.IMPACT_THRESHOLDS = {
      ...
      DEFENSIVE_PILLAR: 0.1, // Higher variance if missing
  ...
  async syncKeyPlayers(teamId, tournamentId, seasonId) {
    try {
      ...
      for (const p of players.slice(0, 15)) {
        // Check top 15 by default relevance
  ...
  async calculateLineupDeficit(matchId, homeTeamId, awayTeamId) {
    try {
      ...
      const existing = database.db
        .prepare(`SELECT * FROM match_lineups WHERE match_id = ?`)
        .get(matchId)
  ...
  async _getTeamDeficit(teamId, activePlayerIds) {
    ...
    keys.forEach((k) => {
      if (!activePlayerIds.includes(k.player_id)) {
        ...
        const p = (k.goals > 5 ? 0.2 : 0.1) + (k.rating > 7.3 ? 0.05 : 0)
        xgPenalty += p
        confidencePenalty += 5
      }
    })
