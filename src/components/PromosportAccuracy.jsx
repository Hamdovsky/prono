...
export default function PromosportAccuracy({ onClose }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/promosport/accuracy')
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/promosport/feedback/stats')
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/promosport/accuracy/matrix')
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/promosport/accuracy/roi?stake=10')
        .then((r) => r.json())
        .catch(() => null),
    ]).then(([s, c, m, r]) => {
  ...
  const overallPct =
    stats && stats.totalMatches > 0
      ? ((stats.totalCorrect / stats.totalMatches) * 100).toFixed(1)
      : '0.0'
  const color =
    parseFloat(overallPct) > 65 ? '#22c55e' : parseFloat(overallPct) > 50 ? '#f59e0b' : '#ef4444'
    ...
    <div style={{ margin: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{
              padding: '4px 12px',
              fontSize: '0.65rem',
              background: retraining ? '#1e293b' : 'rgba(59,130,246,0.2)',
              border: `1px solid ${retraining ? '#334155' : 'rgba(59,130,246,0.4)'}`,
              borderRadius: 6,
              color: retraining ? '#64748b' : '#60a5fa',
      ...
      {retrainResult && (
        <div
          style={{
            ...
            padding: '8px 12px',
            borderRadius: 8,
            background: retrainResult.success ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)',
            border: `1px solid ${retrainResult.success ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`,
          }}
        >
          <div
            style={{
              color: retrainResult.success ? '#22c55e' : '#ef4444',
              fontWeight: 600,
              fontSize: '0.75rem',
            }}
          >
          ...
          {retrainResult.steps?.map((s, i) => (
            <div key={i} style={{ color: '#94a3b8', fontSize: '0.65rem', marginTop: 2 }}>
              {s.step}:{' '}
              {s.accuracy ? `accuracy ${s.accuracy}%` : s.output || s.success ? 'OK' : 'FAIL'}
            </div>
          ))}
        </div>
      )}
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: '3px 10px',
              fontSize: '0.6rem',
              background: tab === t.key ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.03)',
              border: `1px solid ${tab === t.key ? 'rgba(251,191,36,0.3)' : 'rgba(255,255,255,0.06)'}`,
              borderRadius: 4,
      ...
      {tab === 'overview' && (
        <div>
          ...
          >
            ...
            <KPI
              label="Crowd"
              value={crowd?.crowdAccuracy ? `${crowd.crowdAccuracy}%` : 'N/A'}
              color="#f59e0b"
            />
            {roi && (
              <KPI
                label="ROI"
                value={roi.roi}
                color={parseFloat(roi.roi) > 0 ? '#22c55e' : '#ef4444'}
              />
            )}
          </div>
          {(!stats || stats.totalMatches === 0) && (
            ...
            >
              Aucune prédiction enregistrée. Les statistiques apparaîtront après avoir visité la
              page Promosport (les pronostics sont stockés automatiquement).
            </div>
          )}
          <div
            style={{
          ...
          >
            ...
            >
              Répartition par confidence (crowd)
            </div>
                ...
                >
                  <div
                    style={{
                      width: `${b.accuracy}%`,
                      height: '100%',
                      background: NUM_COLORS[i % NUM_COLORS.length],
                      borderRadius: 2,
                    }}
                  />
                </div>
        ...
        <div>
          <div style={{ marginBottom: 8, display: 'flex', gap: 8 }}>
            ...
            <KPI
              ...
              color={parseFloat(matrix.accuracy) > 50 ? '#22c55e' : '#f59e0b'}
            />
          ...
          >
            ...
            >
              Matrice de confusion — Prédit → Réel ↓
            </div>
                    ...
                    <tr key={row.actual}>
                      ...
                      <td
                        style={{
                          padding: '6px 8px',
                          textAlign: 'center',
                          background: row.actual === '1' ? 'rgba(59,130,246,0.1)' : 'transparent',
                          color: '#e2e8f0',
                        }}
                      >
                        {(d[1] || 0).toFixed(1)}
                      ...
                      <td
                        style={{
                          padding: '6px 8px',
                          textAlign: 'center',
                          background: row.actual === 'X' ? 'rgba(251,191,36,0.1)' : 'transparent',
                          color: '#e2e8f0',
                        }}
                      >
                        {(d['X'] || 0).toFixed(1)}
                      ...
                      <td
                        style={{
                          padding: '6px 8px',
                          textAlign: 'center',
                          background: row.actual === '2' ? 'rgba(239,68,68,0.1)' : 'transparent',
                          color: '#e2e8f0',
                        }}
                      >
                        {(d[2] || 0).toFixed(1)}
                      ...
                      <td
                        style={{
                          padding: '6px 8px',
                          textAlign: 'center',
                          color: total > 0 && d[row.actual] / total > 0.4 ? '#22c55e' : '#f59e0b',
                          fontWeight: 600,
                        }}
                      >
      ...
      {tab === 'roi' && roi && (
        <div>
          ...
          >
            ...
            <KPI
              label="Retour total"
              value={`${roi.totalReturned.toFixed(0)} DT`}
              color={roi.profit > 0 ? '#22c55e' : '#ef4444'}
            />
            <KPI
              label="Profit"
              value={`${roi.profit > 0 ? '+' : ''}${roi.profit.toFixed(0)} DT`}
              color={roi.profit > 0 ? '#22c55e' : '#ef4444'}
            />
            <KPI
              label="ROI"
            ...
            <KPI
              label="Win Rate"
              value={roi.winRate}
              color={parseFloat(roi.winRate) > 50 ? '#22c55e' : '#f59e0b'}
            />
          </div>
          <div
            style={{
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: '0.5rem', color: '#64748b' }}>
                    {g.wins}W/{g.losses}L · {g.profit > 0 ? '+' : ''}
                    {g.profit.toFixed(0)} DT
                  </div>
                </div>
              )
            })}
          ...
          >
            ...
            >
              Profit par concours (cumulé)
            </div>
            {roi.byConcours?.slice(-20).map((c, i) => {
              const cumul = roi.byConcours
                .slice(0, roi.byConcours.indexOf(c) + 1)
              ...
              return (
                ...
                >
                  <div style={{ fontSize: '0.5rem', color: '#64748b', minWidth: 28 }}>
                    #{c.concours}
                  </div>
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: '0.7rem',
                      color: pct > 65 ? '#22c55e' : pct > 50 ? '#f59e0b' : '#ef4444',
                      fontWeight: 700,
                    }}
                  >
                    {g.accuracy}%
                ...
                >
                  <div
                    style={{
                      ...
                      height: '100%',
                      background: pct > 65 ? '#22c55e' : pct > 50 ? '#f59e0b' : '#ef4444',
                      borderRadius: 3,
      ...
      {tab === 'crowd' && crowd && (
        <div>
          ...
          >
            ...
            >
              Confiance de la foule par palier
            </div>
            {crowd.byConfidence?.map((b, i) => {
      ...
      {tab === 'trend' && stats?.recentConcours && (
        ...
        >
          ...
          >
            Derniers {stats.recentConcours.length} concours
          </div>
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: '0.65rem',
                      fontWeight: 700,
                      color: parseFloat(c.accuracy) > 65 ? '#22c55e' : '#f59e0b',
                    }}
                  >
                    {c.accuracy}%
                  </div>
...
function KPI({ label, value, color }) {
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: '0.55rem', color: '#64748b', textTransform: 'uppercase', marginTop: 2 }}
      >
        {label}
      </div>
    </div>
