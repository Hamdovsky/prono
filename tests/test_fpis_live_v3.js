...
const s2 = e.processLive({
  ...
  odds_home: 1.1,
  odds_home_open: 1.4, // Strong drift
...
const s3 = e.processLive({
  ...
  dangerous_attacks_away: 45, // High pressure: >1.2/min
  corners_home: 5,
  corners_away: 4,
...
assert(
  s3.alternative_angle?.[0]?.angle === 'Over 0.5 Goals',
  'Pressure Layer flagged Over 0.5 Goals for 0-0 match'
)
...
assert(
  s4.adjusted_prediction.includes('The New Saints Win'),
  'Trend Alignment overrode model to favour leading team'
)
assert(
  parseFloat(s4.model_prediction.away) > 50,
  'Trend Alignment forced away probability to reflect the 2-goal lead'
)
