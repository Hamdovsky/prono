...
class MarketCorrelationEngine {
  ...
  async analyze(m) {
    ...
    const currentRes =
      e.winner === (m.homeTeam || 'Home') ? '1' : e.winner === (m.awayTeam || 'Away') ? '2' : 'X'
    const currentScore = (m.expected_score || e.expected_score || '0 - 0')
      .replace(/\s+/g, ' ')
      .trim()

    // [V20 DATA 1] Logic: Prioritize precise result match
    let profileMatch = MASTER_PROFILES.find(
      (p) => p.data1 === currentRes && p.score === currentScore
    )
    if (!profileMatch) {
    ...
    const v20 = v20Plus || {}
    const masterConfidence = v20Plus
      ? v20Plus.master_confidence
      : Math.min(99, Math.round((totalScore / 115) * 100 + nodeCount * 5))
    const verdict = v20Plus
      ? v20Plus.master_verdict
      : profileMatch
        ? profileMatch.verdict.toUpperCase()
        : 'NEUTRAL'
    ...
    return {
      ...
      v20_plus: v20Plus,
      pattern_score: v20Plus
        ? v20Plus.monte_carlo_mode_score
        : profileMatch
          ? profileMatch.score
          : null,
...
export = new MarketCorrelationEngine()
