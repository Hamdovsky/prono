...
if (process.env.DATABASE_URL) {
  ...
  import pgConnector from './pg_connector'
  import pgDb from './pg_database'
  import pgMigrations from './pg_migrations'
  pgConnector.getPool()
  ...
  setTimeout(async () => {
    try {
      import { query as pgQuery } from './pg_connector'
...
const database = {
  ...
  getTeamPromosportStats: (teamName) => {
    try {
      ...
      import ArchiveDB from 'better-sqlite3'
