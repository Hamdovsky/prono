  ...
  try {
    ...
    try {
      const matches = sqlite
        .prepare("SELECT * FROM matches WHERE status IN ('scheduled','NOT_STARTED','NS')")
        .all()
      if (matches.length > 0) {
        for (const m of matches) {
          await pool.query(
            ...
            [
              ...
              m.awayTeam || m.away_team,
              m.league,
              m.startTimestamp || m.start_timestamp,
              m.status,
              m.scoreHome || m.score_home,
              m.scoreAway || m.score_away,
            ]
          )
        }
