...
class FeatureEngineer {
  constructor() {
    this.homeAdvantageFactor = 1.2 // 20% boost for home teams (increased from 15%)
  ...
  _getLeagueHomeAdvantage(league) {
    ...
    for (const k of strong) {
      if (key.includes(k)) return 1.1
    }
    for (const k of weak) {
      if (key.includes(k)) return 0.9
    }
    for (const k of moderate) {
      if (key.includes(k)) return 1.0
    }
    return 1.0 // default
  }
  ...
  ensemblePredict(match, xgH, xgA) {
    ...
    // Blend probabilities
    const blendedHome =
      poissonWeight * poissonProbs.win.home + eloWeight * eloProb + dcWeight * dcAdj.home

    const blendedAway =
      poissonWeight * poissonProbs.win.away + eloWeight * (1 - eloProb) + dcWeight * dcAdj.away
