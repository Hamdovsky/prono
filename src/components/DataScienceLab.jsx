...
const DataScienceLab = ({ matches = [] }) => {
  ...
  const stats = useMemo(() => {
    const filtered =
      selectedLeague === 'ALL' ? matches : matches.filter((m) => m.league === selectedLeague)
    ...
    <div className="ds-lab">
      <header className="ds-header">
        <div className="ds-title">
          ...
          <h1>
            Data Science Lab <small>v22.0</small>
          </h1>
        </div>
            ...
            {globalFeatures.map((f, i) => (
              <div key={i} className="feature-row">
                ...
                <div className="f-bar-container">
                  <div
                    className="f-bar"
                    style={{ width: `${f.importance * 100 * 3}%`, backgroundColor: f.color }}
                  ></div>
            ...
            <>
              <div className="ds-stats-row">
                <div className="ds-stat">
                  <span className="ds-stat-val">
                    {overall.last7Days != null ? `${overall.last7Days}%` : 'N/A'}
                  </span>
                  <span className="ds-stat-label">Précision 7 jours</span>
                </div>
                <div className="ds-stat highlight">
                  <span className="ds-stat-val">
                    {overall.last30Days != null ? `${overall.last30Days}%` : 'N/A'}
                  </span>
              ...
              <div className="ds-stats-row" style={{ marginTop: '8px' }}>
                {Object.entries(overall.byMarket || {}).map(([key, m]) => (
                  <div key={key} className="ds-stat" style={{ flex: 1 }}>
                    <span className="ds-stat-val" style={{ fontSize: '16px' }}>
                      {m.accuracy != null ? `${m.accuracy}%` : 'N/A'}
                    </span>
                    <span className="ds-stat-label">
                      {key === '1x2' ? '1X2' : key === 'ou25' ? 'O/U 2.5' : 'BTTS'} ({m.total}{' '}
                      matchs)
                    </span>
                  </div>
                ))}
              ...
              {overall.currentStreak > 0 && (
                <div className="ds-insight">
                  <p>
                    <strong>🔥 Série en cours:</strong> {overall.currentStreak} prédictions
                    correctes consécutives (Record: {overall.recordStreak})
                  </p>
                </div>
              )}
            </>
