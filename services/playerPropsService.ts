...
class PlayerPropsService {
  ...
  getPlayersForTeam(teamName) {
    try {
      return database
        .prepare('SELECT * FROM player_stats WHERE team_name = ? ORDER BY rating_avg DESC LIMIT 15')
        .all(teamName)
    } catch (e) {
  ...
  async generatePlayerProps(m) {
    ...
    try {
      ...
      const news = m.news_data
        ? typeof m.news_data === 'string'
          ? JSON.parse(m.news_data)
          : m.news_data
        : {}
      const injuries = news.injuries || {}

      // Map injuries to positional codes (G, D, M, F)
      const mapPos = (players) =>
        players.map((p) => {
