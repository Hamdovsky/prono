...
async function processTeamLineupForProps(players, teamName, uniqueTournamentId, seasonId) {
  ...
  for (const p of starters) {
    ...
    if (stat) {
      ...
      const shotsOnTargetAvg = stat.shotsOnTarget
        ? (stat.shotsOnTarget / matchesPlayed).toFixed(2)
        : 0
  ...
  if (processedCount > 0) {
    console.log(
      `👟 [PropsScraper] Fetched stats for ${processedCount} players from ${teamName || 'Team'}`
    )
  }
}
