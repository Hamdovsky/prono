...
import adaptiveLearning from '../services/adaptiveLearningEngine'
...
router.get('/leagues', async (req, res) => {
  try {
    const rows = database.db
      .prepare(
        'SELECT DISTINCT TRIM(league) as league FROM learning_memory WHERE league IS NOT NULL AND league != "" ORDER BY league ASC'
      )
      .all()
...
router.get('/auto-process', async (req, res) => {
  try {
    ...
    const inputs = rows.map((r) => {
      let fullData = {}
      try {
        fullData = JSON.parse(r.fullData || '{}')
      } catch (_) {}
    ...
    res.json({
      success: true,
      message: `Processing ${inputs.length} matches in background.`,
