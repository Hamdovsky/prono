...
function assignDoubles(processedMatches) {
  ...
  for (const m of core) {
    ...
    for (const g of GRID_NAMES) {
      const top2 = [
        m.probs[0] > m.probs[2] ? '1' : '2',
        m.probs[1] > Math.max(m.probs[0], m.probs[2]) ? 'X' : m.probs[0] > m.probs[2] ? '2' : '1',
      ]
  ...
  for (let i = 0; i < mediums.length; i++) {
    ...
    const top2 =
      m.probs[0] > 0.5
        ? ['1', m.probs[1] > m.probs[2] ? 'X' : '2']
        : m.probs[2] > 0.5
          ? ['2', m.probs[1] > m.probs[0] ? 'X' : '1']
          : ['1', '2']
...
function backtest() {
  ...
  for (let ci = 0; ci < concoursList.length; ci++) {
    ...
    if ((ci + 1) % 20 === 0 || ci === 0 || ci === concoursList.length - 1) {
      ...
      console.log(
        `  Concours ${concours} (${ci + 1}/${concoursList.length}): Grid accs = ${results.map((r) => `${r.grid}:${r.accuracy.toFixed(1)}%`).join(', ')} | Running: ${acc.toFixed(1)}%`
      )
    }
  }
  ...
  for (const g of GRID_NAMES) {
    ...
    if (s && s.total > 0) {
      console.log(
        `    ${g}: ${((s.correct / s.total) * 100).toFixed(2)}% (${s.correct}/${s.total})`
      )
    }
  }
  ...
  for (let i = 0; i < allResults.length; i += 50 * 4) {
    ...
    console.log(
      `    Concours ${chunkConcours[0] || '?'}-${chunkConcours[chunkConcours.length - 1] || '?'}: ${chunkAcc.toFixed(2)}% (${chunkCorrect}/${chunkTotal})`
    )
  }
  ...
  for (const [b, s] of Object.entries(bands)) {
    if (s.total > 0)
      console.log(
        `    ${b} (${s.total}): ${((s.correct / s.total) * 100).toFixed(2)}% (${s.correct}/${s.total})`
      )
  }
...
function backtestGoldCoupon() {
  ...
  for (let ci = 0; ci < concoursList.length; ci++) {
    ...
    const matches = db
      .prepare(
        `SELECT * FROM promosport_archive WHERE concours = ? AND result IS NOT NULL AND result != 'N' ORDER BY match_idx`
      )
      .all(String(concours))
    if (matches.length < 10) continue
  ...
  const acc = totalMatches > 0 ? (totalCorrect / totalMatches) * 100 : 0
  console.log(
    `  Concours: ${concoursList.length} | Exact: ${totalCorrect}/${totalMatches} = ${acc.toFixed(2)}%`
  )
