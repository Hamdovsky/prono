...
function scoreMatchForDraw(m, leagueDrawRates) {
  ...
  if (dpNorm >= 0.35) {
    score += 30
    reasons.push(`Probabilité de Nul élevée (${(dpNorm * 100).toFixed(0)}%)`)
  } else if (dpNorm >= 0.28) {
    score += 18
    reasons.push(`Probabilité de Nul modérée (${(dpNorm * 100).toFixed(0)}%)`)
  } else if (dpNorm >= 0.22) {
    score += 8
    reasons.push(`Probabilité de Nul viable (${(dpNorm * 100).toFixed(0)}%)`)
  }
  ...
  if (balance < 0.08) {
    score += 25
    reasons.push('Équilibre de force parfait')
  } else if (balance < 0.15) {
    score += 15
    reasons.push('Très bon équilibre des forces')
  ...
  for (const [leagueName, data] of Object.entries(leagueDrawRates)) {
    if (league.toLowerCase().includes(leagueName.toLowerCase().substring(0, 15))) {
      if (data.rate >= 0.35) {
        score += 20
        reasons.push(`Ligue très propice aux nuls (${(data.rate * 100).toFixed(0)}%)`)
      } else if (data.rate >= 0.28) {
        score += 12
        reasons.push(`Ligue propice aux nuls (${(data.rate * 100).toFixed(0)}%)`)
      }
      break
    }
  }
  ...
  const avgLeagueOdds = leagueData?.avgOdds || 3.1
  ...
  if (ou < 0.4) {
    score += 12
    reasons.push(`Forte tendance Under 2.5 (${((1 - ou) * 100).toFixed(0)}%)`)
  } else if (ou < 0.5) {
    score += 6
  }
  ...
  if (chaos < 25) {
    score += 15
    reasons.push('Indice de Chaos très faible (Match verrouillé)')
  } else if (chaos < 35) {
    score += 8
    reasons.push('Indice de Chaos modéré (Match stable)')
  } else if (chaos > 60) {
    score -= 10
    reasons.push('Indice de Chaos élevé (Risque de buts tardifs)')
  }
  ...
  if (oddsX > 0 && dpNorm > 0) {
    ...
    if (ev > 0.15) {
      score += 15
      reasons.push(`Value Bet détecté (EV: +${(ev * 100).toFixed(0)}%)`)
    } else if (ev > 0.05) {
      score += 8
      reasons.push(`Léger Value Bet (EV: +${(ev * 100).toFixed(0)}%)`)
    }
  }
...
if (require.main === module) {
  ...
  if (candidates.length === 0) {
    ...
    candidates.forEach((m, i) => {
      ...
      console.log(
        `   🏆 ${m.league} | 🎯 ${m.drawScore}/100 | 📊 ${dpDisp}% | X=${m.odds_draw || 'N/A'}`
      )
