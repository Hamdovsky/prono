...
function _saveJson(p, data) {
  try {
    ...
    logger.warn(`[BACKTEST] Save failed: ${e.message}`)
  }
}
...
function computeCalibrationMetrics(match) {
  ...
  const loglossBTTS = -(
    actualBTTS * Math.log(pBTTS_clamped) +
    (1 - actualBTTS) * Math.log(1 - pBTTS_clamped)
  )
...
function _computeDynamicWeights(leagueStats) {
  ...
  for (const [lg, stats] of Object.entries(leagueStats)) {
    ...
    if (edge > 5) {
      ...
      xgbWeight = Math.max(0.4, DEFAULT_XGB + (edge / 100) * 0.5)
    }

    // If overall accuracy is very low, reduce trust
    if (acc < 35) {
      xgbWeight = Math.max(0.3, xgbWeight * 0.7)
    }
    ...
    const newsBoost = Math.max(0.1, Math.min(0.8, 0.3 + (50 - acc) / 200))
...
export = { runAutoBacktest, evaluatePrediction }
