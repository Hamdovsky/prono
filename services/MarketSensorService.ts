      ...
      try {
        ...
        if (isSteamH || isSteamA || trap.isTrap) {
          signals.push({
            ...
            severity: trap.isTrap
              ? trap.severity
              : Math.abs(isSteamH ? movement.h_pct : movement.a_pct),
            description: trap.isTrap
              ? trap.msg
              : `Significant money flow on ${isSteamH ? m.homeTeam : m.awayTeam}`,
...
export = new MarketSensorService()
