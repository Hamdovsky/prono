...
export default function MarketLab() {
  ...
  const filteredDeals = opportunities.filter(
    (o) => filter === 'ALL' || o.integrity.status === filter
  )
  ...
  return (
    <div className="market-lab">
      ...
      <div className="market-filters">
        ...
        <button className={filter === 'GREEN' ? 'active' : ''} onClick={() => setFilter('GREEN')}>
          🛡️ آمن (نزاهة عالية)
        </button>
        <button className={filter === 'YELLOW' ? 'active' : ''} onClick={() => setFilter('YELLOW')}>
          ⚠️ حذر
        </button>
      </div>
