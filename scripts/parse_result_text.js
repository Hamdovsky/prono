        ...
        for (let i = 1; i < cells.length; i++) {
          if (cells[i].match(/^\d+\s*-\s*\d+$/)) {
            ...
            for (let j = i + 3; j < cells.length; j++) {
              ...
              if (pctMatch) {
                pct = {
                  p1: parseInt(pctMatch[1]),
                  px: parseInt(pctMatch[2]),
                  p2: parseInt(pctMatch[3]),
                }
                break
              }
            }
  ...
  if (result && result.matches.length > 0) {
    ...
    result.matches.forEach((m) => {
      const vote = m.publicVote
        ? `Pub: ${m.publicVote.p1}%/${m.publicVote.px}%/${m.publicVote.p2}%`
        : 'No vote'
      console.log(
        `  #${m.idx} ${m.home} vs ${m.away} [${m.scoreHome}-${m.scoreAway}] → ${m.result} | ${vote}`
      )
    })
  } else {
